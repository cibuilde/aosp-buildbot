...
mkdir -p out && echo '1735664461' > out/build_date.txt
...
echo "building superimage"
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/kati/build.ninja superimage

du -ah -d1 $GITHUB_WORKSPACE
du -ah -d1 $GITHUB_WORKSPACE/aosp
du -ah -d1 out/target/product/vsoc_x86_64/

cd $GITHUB_WORKSPACE/
tar -cf odm.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/out/target/product/vsoc_x86_64/ odm
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_kati odm.tar.zst --clobber

tar -cf odm_dlkm.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/out/target/product/vsoc_x86_64/ odm_dlkm
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_kati odm_dlkm.tar.zst --clobber

tar -cf product.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/out/target/product/vsoc_x86_64/ product
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_kati product.tar.zst --clobber

tar -cf root.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/out/target/product/vsoc_x86_64/ root
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_kati root.tar.zst --clobber

tar -cf system.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/out/target/product/vsoc_x86_64/ system
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_kati system.tar.zst --clobber

tar -cf system_ext.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/out/target/product/vsoc_x86_64/ system_ext
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_kati system_ext.tar.zst --clobber

tar -cf system_other.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/out/target/product/vsoc_x86_64/ system_other
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_kati system_other.tar.zst --clobber

tar -cf vendor.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/out/target/product/vsoc_x86_64/ vendor
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_kati vendor.tar.zst --clobber

tar -cf vendor_dlkm.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/out/target/product/vsoc_x86_64/ vendor_dlkm
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_kati vendor_dlkm.tar.zst --clobber
