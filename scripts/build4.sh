...
df -h
clean_out_intermediates
mkdir -p prebuiltlibs/frameworks/compile/mclinker/ninja && rsync -ar out/soong/ninja/frameworks/compile/mclinker/ prebuiltlibs/frameworks/compile/mclinker/ninja/frameworks_compile_mclinker-4
touch prebuiltlibs/frameworks/compile/mclinker/ninja/.find-ignore
tar cfJ frameworks_compile_mclinker-4.tar.xz -C prebuiltlibs/frameworks/compile/mclinker/ .
ls -l frameworks_compile_mclinker-4.tar.xz
end=`date +%s`
echo $((end-start))
start=`date +%s`
./prebuilts/build-tools/linux-x86/bin/ninja -f out/soong/build.ninja \
...
df -h
clean_out_intermediates
mkdir -p prebuiltlibs/frameworks/compile/slang/ninja && rsync -ar out/soong/ninja/frameworks/compile/slang/ prebuiltlibs/frameworks/compile/slang/ninja/frameworks_compile_slang-4
touch prebuiltlibs/frameworks/compile/slang/ninja/.find-ignore
tar cfJ frameworks_compile_slang-4.tar.xz -C prebuiltlibs/frameworks/compile/slang/ .
ls -l frameworks_compile_slang-4.tar.xz
end=`date +%s`
echo $((end-start))
start=`date +%s`
./prebuilts/build-tools/linux-x86/bin/ninja -f out/soong/build.ninja \
