...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libopencsd_decoder,android_x86_64_static
...
cd $GITHUB_WORKSPACE/
tar cfJ external_OpenCSD.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/OpenCSD/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_OpenCSD.tar.xz" ]; then
  echo "Compressing external/OpenCSD -> external_OpenCSD.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_OpenCSD.tar.xz -C $GITHUB_WORKSPACE/aosp/external/OpenCSD/ .
fi
