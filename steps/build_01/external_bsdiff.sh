...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libbspatch,android_recovery_x86_64_static
...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libbspatch,android_x86_64_static
...
cd $GITHUB_WORKSPACE/
tar cfJ external_bsdiff.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/bsdiff/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_bsdiff.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_bsdiff.tar.zst -C $GITHUB_WORKSPACE/aosp/external/bsdiff/ .
fi
