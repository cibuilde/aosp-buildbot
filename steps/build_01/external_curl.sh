...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libcurl,android_x86_64_static
...
cd $GITHUB_WORKSPACE/
tar cfJ external_curl.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/curl/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_curl.tar.xz" ]; then
  echo "Compressing external/curl -> external_curl.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_curl.tar.xz -C $GITHUB_WORKSPACE/aosp/external/curl/ .
fi
