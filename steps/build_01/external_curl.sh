...
cd $GITHUB_WORKSPACE/
tar cfJ external_curl.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/curl/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_curl.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_curl.tar.zst -C $GITHUB_WORKSPACE/aosp/external/curl/ .
fi
