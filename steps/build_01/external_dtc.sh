...
prebuilts/build-tools/linux-x86/bin/ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libfdt,android_x86_64_static
...
prebuilts/build-tools/linux-x86/bin/ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libfdt,android_x86_64_static_apex10000
...
cd $GITHUB_WORKSPACE/
tar cfJ external_dtc.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/dtc/ .
...
mkdir -p $GITHUB_WORKSPACE/cache
if [ ! -f "$GITHUB_WORKSPACE/cache/external_dtc.tar.xz" ]; then
  tar cfJ $GITHUB_WORKSPACE/cache/external_dtc.tar.xz -C $GITHUB_WORKSPACE/aosp/external/dtc/ .
fi
mkdir -p $GITHUB_WORKSPACE/cache
