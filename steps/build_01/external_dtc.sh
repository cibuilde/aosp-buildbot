...
cd $GITHUB_WORKSPACE/
tar cfJ external_dtc.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/dtc/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_dtc.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_dtc.tar.zst -C $GITHUB_WORKSPACE/aosp/external/dtc/ .
fi
