...
cd $GITHUB_WORKSPACE/
tar cfJ external_dynamic_depth.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/dynamic_depth/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_01 external_dynamic_depth.tar.zst --clobber
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_dynamic_depth.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_dynamic_depth.tar.zst -C $GITHUB_WORKSPACE/aosp/external/dynamic_depth/ .
fi
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_libxml2.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_libxml2.tar.zst -C $GITHUB_WORKSPACE/aosp/external/libxml2/ .
fi
