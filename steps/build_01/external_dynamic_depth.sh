...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libdynamic_depth,android_x86_64_static
...
cd $GITHUB_WORKSPACE/
tar cfJ external_dynamic_depth.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/dynamic_depth/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_dynamic_depth.tar.xz" ]; then
  echo "Compressing external/dynamic_depth -> external_dynamic_depth.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_dynamic_depth.tar.xz -C $GITHUB_WORKSPACE/aosp/external/dynamic_depth/ .
fi
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_libxml2.tar.xz" ]; then
  echo "Compressing external/libxml2 -> external_libxml2.tar.xz"
