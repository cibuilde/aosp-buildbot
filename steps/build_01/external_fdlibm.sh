...
cd $GITHUB_WORKSPACE/
tar cfJ external_fdlibm.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/fdlibm/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_fdlibm.tar.xz" ]; then
  echo "Compressing external/fdlibm -> external_fdlibm.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_fdlibm.tar.xz -C $GITHUB_WORKSPACE/aosp/external/fdlibm/ .
fi
