...
echo "Preparing for external/fdlibm"
...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_01.ninja libfdlibm,android_x86_64_static_apex31
...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_01.ninja libfdlibm,android_x86_x86_64_static_apex31
...
cd $GITHUB_WORKSPACE/
tar -cf external_fdlibm.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/fdlibm/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_01 external_fdlibm.tar.zst --clobber

du -ah -d1 external_fdlibm*.tar.zst | sort -h
