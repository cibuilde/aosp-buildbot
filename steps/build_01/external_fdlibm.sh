...
cd $GITHUB_WORKSPACE/
tar cfJ external_fdlibm.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/fdlibm/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_fdlibm.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_fdlibm.tar.zst -C $GITHUB_WORKSPACE/aosp/external/fdlibm/ .
fi
