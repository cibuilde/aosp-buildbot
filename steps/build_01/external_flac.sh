...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libFLAC,android_x86_64_static_cfi_apex29
...
cd $GITHUB_WORKSPACE/
tar cfJ external_flac.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/flac/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_flac.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_flac.tar.zst -C $GITHUB_WORKSPACE/aosp/external/flac/ .
fi
