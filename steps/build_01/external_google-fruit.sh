...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libfruit,android_x86_64_static
...
cd $GITHUB_WORKSPACE/
tar cfJ external_google-fruit.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/google-fruit/ .
...
mkdir -p $GITHUB_WORKSPACE/cache
if [ ! -f "$GITHUB_WORKSPACE/cache/external_google-fruit.tar.xz" ]; then
  echo "Compressing external/google-fruit -> external_google-fruit.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_google-fruit.tar.xz -C $GITHUB_WORKSPACE/aosp/external/google-fruit/ .
fi
mkdir -p $GITHUB_WORKSPACE/cache
