...
cd $GITHUB_WORKSPACE/
tar cfJ external_gptfdisk.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/gptfdisk/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_gptfdisk.tar.xz" ]; then
  echo "Compressing external/gptfdisk -> external_gptfdisk.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_gptfdisk.tar.xz -C $GITHUB_WORKSPACE/aosp/external/gptfdisk/ .
fi
