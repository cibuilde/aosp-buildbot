...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libimage_io,android_x86_64_static
...
cd $GITHUB_WORKSPACE/
tar cfJ external_image_io.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/image_io/ .
...
mkdir -p $GITHUB_WORKSPACE/cache
if [ ! -f "$GITHUB_WORKSPACE/cache/external_image_io.tar.xz" ]; then
  echo "Compressing external/image_io -> external_image_io.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_image_io.tar.xz -C $GITHUB_WORKSPACE/aosp/external/image_io/ .
fi
mkdir -p $GITHUB_WORKSPACE/cache
