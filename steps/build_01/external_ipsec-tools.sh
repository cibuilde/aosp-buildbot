...
cd $GITHUB_WORKSPACE/
tar cfJ external_ipsec-tools.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/ipsec-tools/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_ipsec-tools.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_ipsec-tools.tar.zst -C $GITHUB_WORKSPACE/aosp/external/ipsec-tools/ .
fi
