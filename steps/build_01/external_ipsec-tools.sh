...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libipsec,android_x86_64_static
...
cd $GITHUB_WORKSPACE/
tar cfJ external_ipsec-tools.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/ipsec-tools/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_ipsec-tools.tar.xz" ]; then
  echo "Compressing external/ipsec-tools -> external_ipsec-tools.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_ipsec-tools.tar.xz -C $GITHUB_WORKSPACE/aosp/external/ipsec-tools/ .
fi
