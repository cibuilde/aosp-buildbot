...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_iptables.tar.xz" ]; then
  echo "Compressing external/iptables -> external_iptables.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_iptables.tar.xz -C $GITHUB_WORKSPACE/aosp/external/iptables/ .
fi
