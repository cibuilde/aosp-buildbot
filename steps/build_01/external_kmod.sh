...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libkmod,linux_glibc_x86_64_static
...
cd $GITHUB_WORKSPACE/
tar cfJ external_kmod.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/kmod/ .

du -ah -d1

if [ ! -f "$GITHUB_WORKSPACE/cache/external_kmod.tar.xz" ]; then
  echo "Compressing external/kmod -> external_kmod.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_kmod.tar.xz -C $GITHUB_WORKSPACE/aosp/external/kmod/ .
fi
