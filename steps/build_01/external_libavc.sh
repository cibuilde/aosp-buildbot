...
prebuilts/build-tools/linux-x86/bin/ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libavcdec,android_x86_64_static_cfi_apex29
...
prebuilts/build-tools/linux-x86/bin/ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libavcenc,android_x86_64_static_cfi_apex29
...
cd $GITHUB_WORKSPACE/
tar cfJ external_libavc.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/libavc/ .
...
mkdir -p $GITHUB_WORKSPACE/cache
if [ ! -f "$GITHUB_WORKSPACE/cache/external_libavc.tar.xz" ]; then
  tar cfJ $GITHUB_WORKSPACE/cache/external_libavc.tar.xz -C $GITHUB_WORKSPACE/aosp/external/libavc/ .
fi
mkdir -p $GITHUB_WORKSPACE/cache
