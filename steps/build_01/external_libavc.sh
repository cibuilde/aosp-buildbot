...
cd $GITHUB_WORKSPACE/
tar cfJ external_libavc.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/libavc/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_libavc.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_libavc.tar.zst -C $GITHUB_WORKSPACE/aosp/external/libavc/ .
fi
