...
cd $GITHUB_WORKSPACE/
tar cfJ external_libgav1.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/libgav1/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_libgav1.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_libgav1.tar.zst -C $GITHUB_WORKSPACE/aosp/external/libgav1/ .
fi
