...
cd $GITHUB_WORKSPACE/
tar cfJ external_libgsm.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/libgsm/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_libgsm.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_libgsm.tar.zst -C $GITHUB_WORKSPACE/aosp/external/libgsm/ .
fi
