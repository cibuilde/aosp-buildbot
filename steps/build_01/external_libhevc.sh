...
cd $GITHUB_WORKSPACE/
tar cfJ external_libhevc.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/libhevc/ .
...
mkdir -p $GITHUB_WORKSPACE/cache
if [ ! -f "$GITHUB_WORKSPACE/cache/external_libhevc.tar.xz" ]; then
  echo "Compressing external/libhevc -> external_libhevc.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_libhevc.tar.xz -C $GITHUB_WORKSPACE/aosp/external/libhevc/ .
fi
mkdir -p $GITHUB_WORKSPACE/cache
