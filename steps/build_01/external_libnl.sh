...
cd $GITHUB_WORKSPACE/
tar cfJ external_libnl.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/libnl/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_libnl.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_libnl.tar.zst -C $GITHUB_WORKSPACE/aosp/external/libnl/ .
fi
