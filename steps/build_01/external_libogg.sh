...
cd $GITHUB_WORKSPACE/
tar -cf external_libogg.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/libogg/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_01 external_libogg.tar.zst --clobber

du -ah -d1 external_libogg*.tar.zst | sort -h
