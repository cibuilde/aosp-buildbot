...
mkdir -p $GITHUB_WORKSPACE/cache
if [ ! -f "$GITHUB_WORKSPACE/cache/external_libopus.tar.xz" ]; then
  echo "Compressing external/libopus -> external_libopus.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_libopus.tar.xz -C $GITHUB_WORKSPACE/aosp/external/libopus/ .
fi
mkdir -p $GITHUB_WORKSPACE/cache
