...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libopus,android_x86_64_static_apex29
...
cd $GITHUB_WORKSPACE/
tar cfJ external_libopus.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/libopus/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_libopus.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_libopus.tar.zst -C $GITHUB_WORKSPACE/aosp/external/libopus/ .
fi
