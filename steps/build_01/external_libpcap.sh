...
cd $GITHUB_WORKSPACE/
tar cfJ external_libpcap.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/libpcap/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_libpcap.tar.xz" ]; then
  echo "Compressing external/libpcap -> external_libpcap.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_libpcap.tar.xz -C $GITHUB_WORKSPACE/aosp/external/libpcap/ .
fi
