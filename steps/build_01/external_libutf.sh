...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libutf,android_x86_64_static_apex10000
...
cd $GITHUB_WORKSPACE/
tar cfJ external_libutf.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/libutf/ .
...
mkdir -p $GITHUB_WORKSPACE/cache
if [ ! -f "$GITHUB_WORKSPACE/cache/external_libutf.tar.xz" ]; then
  echo "Compressing external/libutf -> external_libutf.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_libutf.tar.xz -C $GITHUB_WORKSPACE/aosp/external/libutf/ .
fi
mkdir -p $GITHUB_WORKSPACE/cache
