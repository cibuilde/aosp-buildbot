...
prebuilts/build-tools/linux-x86/bin/ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libLLVMSupport,android_x86_64_static
...
prebuilts/build-tools/linux-x86/bin/ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libLLVMSupport,android_x86_x86_64_static
...
prebuilts/build-tools/linux-x86/bin/ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libLLVMSupport,linux_glibc_x86_64_static
...
prebuilts/build-tools/linux-x86/bin/ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libLLVMTableGen,linux_glibc_x86_64_static
...
cd $GITHUB_WORKSPACE/
tar cfJ external_llvm.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/llvm/ .
...
mkdir -p $GITHUB_WORKSPACE/cache
if [ ! -f "$GITHUB_WORKSPACE/cache/external_llvm.tar.xz" ]; then
  tar cfJ $GITHUB_WORKSPACE/cache/external_llvm.tar.xz -C $GITHUB_WORKSPACE/aosp/external/llvm/ .
fi
mkdir -p $GITHUB_WORKSPACE/cache
