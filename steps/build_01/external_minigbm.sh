...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libgbm,android_x86_64_static_apex10000
...
cd $GITHUB_WORKSPACE/
tar cfJ external_minigbm.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/minigbm/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_minigbm.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_minigbm.tar.zst -C $GITHUB_WORKSPACE/aosp/external/minigbm/ .
fi
