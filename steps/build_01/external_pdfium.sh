...
cd $GITHUB_WORKSPACE/
tar cfJ external_pdfium.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/pdfium/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_pdfium.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_pdfium.tar.zst -C $GITHUB_WORKSPACE/aosp/external/pdfium/ .
fi
