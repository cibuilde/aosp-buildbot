...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libcfg_if,linux_glibc_x86_64_rlib_rlib-std
...
cd $GITHUB_WORKSPACE/
tar cfJ external_rust_crates_cfg-if.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/rust/crates/cfg-if/ .

du -ah -d1

if [ ! -f "$GITHUB_WORKSPACE/cache/external_rust_crates_cfg-if.tar.xz" ]; then
  echo "Compressing external/rust/crates/cfg-if -> external_rust_crates_cfg-if.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_rust_crates_cfg-if.tar.xz -C $GITHUB_WORKSPACE/aosp/external/rust/crates/cfg-if/ .
fi
