...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libglob,linux_glibc_x86_64_rlib_rlib-std
...
cd $GITHUB_WORKSPACE/
tar cfJ external_rust_crates_glob.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/rust/crates/glob/ .

du -ah -d1

mkdir -p $GITHUB_WORKSPACE/cache
if [ ! -f "$GITHUB_WORKSPACE/cache/external_rust_crates_glob.tar.xz" ]; then
  echo "Compressing external/rust/crates/glob -> external_rust_crates_glob.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_rust_crates_glob.tar.xz -C $GITHUB_WORKSPACE/aosp/external/rust/crates/glob/ .
fi
mkdir -p $GITHUB_WORKSPACE/cache
