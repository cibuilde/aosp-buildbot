...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libgrpc_wrap,android_x86_x86_64_static
...
cd $GITHUB_WORKSPACE/
tar cfJ external_rust_crates_grpcio-sys.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/rust/crates/grpcio-sys/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_grpc-grpc.tar.xz" ]; then
  echo "Compressing external/grpc-grpc -> external_grpc-grpc.tar.xz"
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_rust_crates_grpcio.tar.xz" ]; then
  echo "Compressing external/rust/crates/grpcio -> external_rust_crates_grpcio.tar.xz"
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_rust_crates_grpcio-sys.tar.xz" ]; then
  echo "Compressing external/rust/crates/grpcio-sys -> external_rust_crates_grpcio-sys.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_rust_crates_grpcio-sys.tar.xz -C $GITHUB_WORKSPACE/aosp/external/rust/crates/grpcio-sys/ .
fi
