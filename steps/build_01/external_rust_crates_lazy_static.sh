...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja liblazy_static,linux_glibc_x86_64_rlib_rlib-std
...
cd $GITHUB_WORKSPACE/
tar cfJ external_rust_crates_lazy_static.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/rust/crates/lazy_static/ .

du -ah -d1

mkdir -p $GITHUB_WORKSPACE/cache
if [ ! -f "$GITHUB_WORKSPACE/cache/external_rust_crates_lazy_static.tar.xz" ]; then
  echo "Compressing external/rust/crates/lazy_static -> external_rust_crates_lazy_static.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_rust_crates_lazy_static.tar.xz -C $GITHUB_WORKSPACE/aosp/external/rust/crates/lazy_static/ .
fi
mkdir -p $GITHUB_WORKSPACE/cache
