...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libmemchr,linux_glibc_x86_64_rlib_rlib-std
...
cd $GITHUB_WORKSPACE/
tar cfJ external_rust_crates_memchr.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/rust/crates/memchr/ .

du -ah -d1

mkdir -p $GITHUB_WORKSPACE/cache
if [ ! -f "$GITHUB_WORKSPACE/cache/external_rust_crates_memchr.tar.xz" ]; then
  echo "Compressing external/rust/crates/memchr -> external_rust_crates_memchr.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_rust_crates_memchr.tar.xz -C $GITHUB_WORKSPACE/aosp/external/rust/crates/memchr/ .
fi
mkdir -p $GITHUB_WORKSPACE/cache
