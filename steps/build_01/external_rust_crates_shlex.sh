...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libshlex,linux_glibc_x86_64_rlib_rlib-std
...
cd $GITHUB_WORKSPACE/
tar cfJ external_rust_crates_shlex.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/rust/crates/shlex/ .

du -ah -d1

if [ ! -f "$GITHUB_WORKSPACE/cache/external_rust_crates_shlex.tar.xz" ]; then
  echo "Compressing external/rust/crates/shlex -> external_rust_crates_shlex.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_rust_crates_shlex.tar.xz -C $GITHUB_WORKSPACE/aosp/external/rust/crates/shlex/ .
fi
