...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_rust_crates_unicode-width.tar.xz" ]; then
  echo "Compressing external/rust/crates/unicode-width -> external_rust_crates_unicode-width.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_rust_crates_unicode-width.tar.xz -C $GITHUB_WORKSPACE/aosp/external/rust/crates/unicode-width/ .
fi
mkdir -p $GITHUB_WORKSPACE/cache
