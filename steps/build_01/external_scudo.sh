...
cd $GITHUB_WORKSPACE/
tar cfJ external_scudo.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/scudo/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_scudo.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_scudo.tar.zst -C $GITHUB_WORKSPACE/aosp/external/scudo/ .
fi
