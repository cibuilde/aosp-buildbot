...
cd $GITHUB_WORKSPACE/
tar cfJ external_tensorflow.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/tensorflow/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_ruy.tar.xz" ]; then
  echo "Compressing external/ruy -> external_ruy.tar.xz"
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_tensorflow.tar.xz" ]; then
  echo "Compressing external/tensorflow -> external_tensorflow.tar.xz"
