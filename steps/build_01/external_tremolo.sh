...
mkdir -p $GITHUB_WORKSPACE/cache
if [ ! -f "$GITHUB_WORKSPACE/cache/external_tremolo.tar.xz" ]; then
  echo "Compressing external/tremolo -> external_tremolo.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_tremolo.tar.xz -C $GITHUB_WORKSPACE/aosp/external/tremolo/ .
fi
mkdir -p $GITHUB_WORKSPACE/cache
