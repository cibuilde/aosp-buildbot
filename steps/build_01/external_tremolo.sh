...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libvorbisidec,android_x86_64_static_cfi_apex29
...
cd $GITHUB_WORKSPACE/
tar cfJ external_tremolo.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/tremolo/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_tremolo.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_tremolo.tar.zst -C $GITHUB_WORKSPACE/aosp/external/tremolo/ .
fi
