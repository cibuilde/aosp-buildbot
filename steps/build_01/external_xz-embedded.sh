...
cd $GITHUB_WORKSPACE/
tar cfJ external_xz-embedded.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/xz-embedded/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_xz-embedded.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_xz-embedded.tar.zst -C $GITHUB_WORKSPACE/aosp/external/xz-embedded/ .
fi
