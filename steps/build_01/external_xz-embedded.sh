...
mkdir -p $GITHUB_WORKSPACE/cache
if [ ! -f "$GITHUB_WORKSPACE/cache/external_xz-embedded.tar.xz" ]; then
  echo "Compressing external/xz-embedded -> external_xz-embedded.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/external_xz-embedded.tar.xz -C $GITHUB_WORKSPACE/aosp/external/xz-embedded/ .
fi
mkdir -p $GITHUB_WORKSPACE/cache
