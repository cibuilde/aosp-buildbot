...
echo "Preparing for external/zopfli"
...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_01.ninja libzopfli,linux_glibc_x86_64_static
...
python3 $GITHUB_WORKSPACE/copy_symlink.py $GITHUB_WORKSPACE/steps/outputs_01/external/zopfli/libzopfli^linux_glibc_x86_64_static.output $GITHUB_WORKSPACE/artifacts/external/zopfli/libzopfli^linux_glibc_x86_64_static $GITHUB_WORKSPACE/artifacts/external/zopfli/libzopfli^linux_glibc_x86_64_static/addition_copy_files.output


rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_zopfli.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/zopfli/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_01 external_zopfli.tar.zst --clobber

du -ah -d1 external_zopfli*.tar.zst | sort -h
