...
prebuilts/build-tools/linux-x86/bin/ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libzopfli,linux_glibc_x86_64_static
...
cd $GITHUB_WORKSPACE/
tar cfJ external_zopfli.tar.xz -C $GITHUB_WORKSPACE/artifacts/external/zopfli/ .
...
mkdir -p $GITHUB_WORKSPACE/cache
if [ ! -f "$GITHUB_WORKSPACE/cache/external_zopfli.tar.xz" ]; then
  tar cfJ $GITHUB_WORKSPACE/cache/external_zopfli.tar.xz -C $GITHUB_WORKSPACE/aosp/external/zopfli/ .
fi
mkdir -p $GITHUB_WORKSPACE/cache
