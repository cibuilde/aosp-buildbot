...
cd $GITHUB_WORKSPACE/
tar cfJ frameworks_av.tar.zst -C $GITHUB_WORKSPACE/artifacts/frameworks/av/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_parameter-framework.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_parameter-framework.tar.zst -C $GITHUB_WORKSPACE/aosp/external/parameter-framework/ .
fi
