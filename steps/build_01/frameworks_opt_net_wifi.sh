...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libwifi-system-iface,android_x86_64_static
...
cd $GITHUB_WORKSPACE/
tar cfJ frameworks_opt_net_wifi.tar.xz -C $GITHUB_WORKSPACE/artifacts/frameworks/opt/net/wifi/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/frameworks_opt_net_wifi.tar.xz" ]; then
  echo "Compressing frameworks/opt/net/wifi -> frameworks_opt_net_wifi.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/frameworks_opt_net_wifi.tar.xz -C $GITHUB_WORKSPACE/aosp/frameworks/opt/net/wifi/ .
fi
