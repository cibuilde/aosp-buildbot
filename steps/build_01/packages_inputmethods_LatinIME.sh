...
prebuilts/build-tools/linux-x86/bin/ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libjni_latinime,android_x86_64_static
...
cd $GITHUB_WORKSPACE/
tar cfJ packages_inputmethods_LatinIME.tar.xz -C $GITHUB_WORKSPACE/artifacts/packages/inputmethods/LatinIME/ .
...
mkdir -p $GITHUB_WORKSPACE/cache
if [ ! -f "$GITHUB_WORKSPACE/cache/packages_inputmethods_LatinIME.tar.xz" ]; then
  tar cfJ $GITHUB_WORKSPACE/cache/packages_inputmethods_LatinIME.tar.xz -C $GITHUB_WORKSPACE/aosp/packages/inputmethods/LatinIME/ .
fi
mkdir -p $GITHUB_WORKSPACE/cache
