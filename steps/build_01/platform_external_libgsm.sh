...
clone_depth_platform platform/external/libgsm
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_01/platform/external/libgsm/libgsm^android_x86_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/external/libgsm/libgsm^android_x86_x86_64_static
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_01/platform/external/libgsm/libgsm^android_x86_64_static_cfi_apex29.output . $GITHUB_WORKSPACE/artifacts/external/libgsm/libgsm^android_x86_64_static_cfi_apex29
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_01/platform/external/libgsm/libgsm^android_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/external/libgsm/libgsm^android_x86_64_static

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_libgsm.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/libgsm/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_01 platform_external_libgsm.tar.zst --clobber

du -ah -d1 platform_external_libgsm*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_libgsm.tar.zst" ]; then
  echo "Compressing platform/external/libgsm -> platform_external_libgsm.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_libgsm.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/libgsm/ .
fi
