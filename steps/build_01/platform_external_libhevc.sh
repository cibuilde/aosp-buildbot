...
clone_depth_platform platform/external/libhevc
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_01/platform/external/libhevc/libhevcenc^android_x86_64_static_cfi_apex29.output . $GITHUB_WORKSPACE/artifacts/external/libhevc/libhevcenc^android_x86_64_static_cfi_apex29
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_01/platform/external/libhevc/libhevcdec^android_x86_64_static_cfi_apex29.output . $GITHUB_WORKSPACE/artifacts/external/libhevc/libhevcdec^android_x86_64_static_cfi_apex29

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_libhevc.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/libhevc/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_01 platform_external_libhevc.tar.zst --clobber

du -ah -d1 platform_external_libhevc*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_libhevc.tar.zst" ]; then
  echo "Compressing platform/external/libhevc -> platform_external_libhevc.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_libhevc.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/libhevc/ .
fi
