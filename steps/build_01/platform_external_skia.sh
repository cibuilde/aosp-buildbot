...
clone_depth_platform platform/external/skia
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_01/platform/external/skia/libskia_renderengine^android_x86_64_static_lto-thin.output . $GITHUB_WORKSPACE/artifacts/external/skia/libskia_renderengine^android_x86_64_static_lto-thin
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_01/platform/external/skia/libskia^android_x86_x86_64_static_lto-thin.output . $GITHUB_WORKSPACE/artifacts/external/skia/libskia^android_x86_x86_64_static_lto-thin
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_01/platform/external/skia/libskia^android_x86_64_static_lto-thin.output . $GITHUB_WORKSPACE/artifacts/external/skia/libskia^android_x86_64_static_lto-thin

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_skia.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/skia/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_01 platform_external_skia.tar.zst --clobber

du -ah -d1 platform_external_skia*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_skia.tar.zst" ]; then
  echo "Compressing platform/external/skia -> platform_external_skia.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_skia.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/skia/ .
fi
