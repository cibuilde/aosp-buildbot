...
clone_depth_platform platform/external/virglrenderer
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_01.ninja libvirglrenderer,android_x86_64_static_apex10000
mkdir -p $GITHUB_WORKSPACE/artifacts/external/virglrenderer/libvirglrenderer^android_x86_64_static_apex10000
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_01/platform/external/virglrenderer/libvirglrenderer^android_x86_64_static_apex10000.output . $GITHUB_WORKSPACE/artifacts/external/virglrenderer/libvirglrenderer^android_x86_64_static_apex10000

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_virglrenderer.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/virglrenderer/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_01 platform_external_virglrenderer.tar.zst --clobber

du -ah -d1 platform_external_virglrenderer*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_virglrenderer.tar.zst" ]; then
  echo "Compressing platform/external/virglrenderer -> platform_external_virglrenderer.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_virglrenderer.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/virglrenderer/ .
fi
