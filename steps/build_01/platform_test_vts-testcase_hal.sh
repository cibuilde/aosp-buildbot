...
clone_depth_platform platform/test/vts-testcase/hal
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_01.ninja libvts_vintf_test_common,linux_glibc_x86_64_static
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_01/platform/test/vts-testcase/hal/libvts_vintf_test_common^linux_glibc_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/test/vts-testcase/hal/treble/vintf/libvts_vintf_test_common/libvts_vintf_test_common^linux_glibc_x86_64_static

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_test_vts-testcase_hal.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/test/vts-testcase/hal/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_01 platform_test_vts-testcase_hal.tar.zst --clobber

du -ah -d1 platform_test_vts-testcase_hal*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_test_vts-testcase_hal.tar.zst" ]; then
  echo "Compressing platform/test/vts-testcase/hal -> platform_test_vts-testcase_hal.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_test_vts-testcase_hal.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/test/vts-testcase/hal/ .
fi
