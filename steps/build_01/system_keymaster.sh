...
prebuilts/build-tools/linux-x86/bin/ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libcppcose_rkp,android_x86_64_static
...
prebuilts/build-tools/linux-x86/bin/ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libkeymaster_portable,android_x86_64_static
...
prebuilts/build-tools/linux-x86/bin/ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libpuresoftkeymasterdevice,android_x86_64_static
...
prebuilts/build-tools/linux-x86/bin/ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libsoft_attestation_cert,android_x86_64_static
...
cd $GITHUB_WORKSPACE/
tar cfJ system_keymaster.tar.xz -C $GITHUB_WORKSPACE/artifacts/system/keymaster/ .
...
mkdir -p $GITHUB_WORKSPACE/cache
if [ ! -f "$GITHUB_WORKSPACE/cache/external_libcppbor.tar.xz" ]; then
  tar cfJ $GITHUB_WORKSPACE/cache/external_libcppbor.tar.xz -C $GITHUB_WORKSPACE/aosp/external/libcppbor/ .
fi
mkdir -p $GITHUB_WORKSPACE/cache
...
mkdir -p $GITHUB_WORKSPACE/cache
if [ ! -f "$GITHUB_WORKSPACE/cache/system_keymaster.tar.xz" ]; then
  tar cfJ $GITHUB_WORKSPACE/cache/system_keymaster.tar.xz -C $GITHUB_WORKSPACE/aosp/system/keymaster/ .
fi
mkdir -p $GITHUB_WORKSPACE/cache
