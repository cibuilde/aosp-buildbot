...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libdmabufheap,android_x86_64_static
...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libdmabufheap,android_x86_64_static_apex29
...
ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libdmabufheap,android_x86_x86_64_static
...
cd $GITHUB_WORKSPACE/
tar cfJ system_memory_libdmabufheap.tar.xz -C $GITHUB_WORKSPACE/artifacts/system/memory/libdmabufheap/ .
...
mkdir -p $GITHUB_WORKSPACE/cache
if [ ! -f "$GITHUB_WORKSPACE/cache/system_memory_libdmabufheap.tar.xz" ]; then
  echo "Compressing system/memory/libdmabufheap -> system_memory_libdmabufheap.tar.xz"
  tar cfJ $GITHUB_WORKSPACE/cache/system_memory_libdmabufheap.tar.xz -C $GITHUB_WORKSPACE/aosp/system/memory/libdmabufheap/ .
fi
mkdir -p $GITHUB_WORKSPACE/cache
