...
prebuilts/build-tools/linux-x86/bin/ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libnfc-nci.conf-default,android_x86_64
...
prebuilts/build-tools/linux-x86/bin/ninja -f $GITHUB_WORKSPACE/steps/build_01.ninja libnfcutils,android_x86_64_static
...
cd $GITHUB_WORKSPACE/
tar cfJ system_nfc.tar.xz -C $GITHUB_WORKSPACE/artifacts/system/nfc/ .
...
mkdir -p $GITHUB_WORKSPACE/cache
if [ ! -f "$GITHUB_WORKSPACE/cache/system_nfc.tar.xz" ]; then
  tar cfJ $GITHUB_WORKSPACE/cache/system_nfc.tar.xz -C $GITHUB_WORKSPACE/aosp/system/nfc/ .
fi
rm -rf aosp