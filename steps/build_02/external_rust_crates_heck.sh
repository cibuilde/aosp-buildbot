...
cd $GITHUB_WORKSPACE/
tar cfJ external_rust_crates_heck.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/rust/crates/heck/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_rust_crates_heck.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_rust_crates_heck.tar.zst -C $GITHUB_WORKSPACE/aosp/external/rust/crates/heck/ .
fi
