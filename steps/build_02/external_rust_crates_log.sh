...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_01 --pattern external_rust_crates_cfg-if.tar.zst --skip-existing
...
ninja -f $GITHUB_WORKSPACE/steps/build_02.ninja liblog_rust,linux_glibc_x86_64_rlib_rlib-std
...
cd $GITHUB_WORKSPACE/
tar cfJ external_rust_crates_log.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/rust/crates/log/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_rust_crates_log.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_rust_crates_log.tar.zst -C $GITHUB_WORKSPACE/aosp/external/rust/crates/log/ .
fi
