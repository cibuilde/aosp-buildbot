...
clone_depth_platform platform/external/rust/crates/cfg-if
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_02.ninja libcfg_if,android_x86_x86_64_rlib_rlib-std
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/cfg-if/libcfg_if^android_x86_x86_64_rlib_rlib-std
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_02/platform/external/rust/crates/cfg-if/libcfg_if^android_x86_x86_64_rlib_rlib-std.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/cfg-if/libcfg_if^android_x86_x86_64_rlib_rlib-std

echo "building libcfg_if^android_x86_64_rlib_rlib-std_apex10000"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_02.ninja libcfg_if,android_x86_64_rlib_rlib-std_apex10000
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/cfg-if/libcfg_if^android_x86_64_rlib_rlib-std_apex10000
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_02/platform/external/rust/crates/cfg-if/libcfg_if^android_x86_64_rlib_rlib-std_apex10000.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/cfg-if/libcfg_if^android_x86_64_rlib_rlib-std_apex10000

echo "building libcfg_if^android_x86_64_rlib_rlib-std"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_02.ninja libcfg_if,android_x86_64_rlib_rlib-std
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/cfg-if/libcfg_if^android_x86_64_rlib_rlib-std
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_02/platform/external/rust/crates/cfg-if/libcfg_if^android_x86_64_rlib_rlib-std.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/cfg-if/libcfg_if^android_x86_64_rlib_rlib-std

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_rust_crates_cfg-if.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/rust/crates/cfg-if/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_02 platform_external_rust_crates_cfg-if.tar.zst --clobber

du -ah -d1 platform_external_rust_crates_cfg-if*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_rust_crates_cfg-if.tar.zst" ]; then
  echo "Compressing platform/external/rust/crates/cfg-if -> platform_external_rust_crates_cfg-if.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_rust_crates_cfg-if.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/rust/crates/cfg-if/ .
fi
