...
clone_depth_platform platform/external/rust/crates/codespan-reporting
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_02.ninja libcodespan_reporting,linux_glibc_x86_64_rlib_rlib-std
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_02/platform/external/rust/crates/codespan-reporting/libcodespan_reporting^linux_glibc_x86_64_rlib_rlib-std.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/codespan-reporting/libcodespan_reporting^linux_glibc_x86_64_rlib_rlib-std

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_rust_crates_codespan-reporting.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/rust/crates/codespan-reporting/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_02 platform_external_rust_crates_codespan-reporting.tar.zst --clobber

du -ah -d1 platform_external_rust_crates_codespan-reporting*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_rust_crates_codespan-reporting.tar.zst" ]; then
  echo "Compressing platform/external/rust/crates/codespan-reporting -> platform_external_rust_crates_codespan-reporting.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_rust_crates_codespan-reporting.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/rust/crates/codespan-reporting/ .
fi
