...
clone_depth_platform platform/external/rust/crates/downcast-rs
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_02.ninja libdowncast_rs,android_x86_64_rlib_rlib-std_apex10000
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_02/platform/external/rust/crates/downcast-rs/libdowncast_rs^android_x86_64_rlib_rlib-std_apex10000.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/downcast-rs/libdowncast_rs^android_x86_64_rlib_rlib-std_apex10000

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_rust_crates_downcast-rs.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/rust/crates/downcast-rs/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_02 platform_external_rust_crates_downcast-rs.tar.zst --clobber

du -ah -d1 platform_external_rust_crates_downcast-rs*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_rust_crates_downcast-rs.tar.zst" ]; then
  echo "Compressing platform/external/rust/crates/downcast-rs -> platform_external_rust_crates_downcast-rs.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_rust_crates_downcast-rs.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/rust/crates/downcast-rs/ .
fi
