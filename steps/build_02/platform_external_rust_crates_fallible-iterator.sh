...
clone_depth_platform platform/external/rust/crates/fallible-iterator
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_02.ninja libfallible_iterator,android_x86_64_rlib_rlib-std
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_02/platform/external/rust/crates/fallible-iterator/libfallible_iterator^android_x86_64_rlib_rlib-std.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/fallible-iterator/libfallible_iterator^android_x86_64_rlib_rlib-std

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_rust_crates_fallible-iterator.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/rust/crates/fallible-iterator/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_02 platform_external_rust_crates_fallible-iterator.tar.zst --clobber

du -ah -d1 platform_external_rust_crates_fallible-iterator*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_rust_crates_fallible-iterator.tar.zst" ]; then
  echo "Compressing platform/external/rust/crates/fallible-iterator -> platform_external_rust_crates_fallible-iterator.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_rust_crates_fallible-iterator.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/rust/crates/fallible-iterator/ .
fi
