...
clone_depth_platform platform/external/selinux
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_02/platform/external/selinux/libselinux^linux_glibc_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/external/selinux/libselinux/libselinux^linux_glibc_x86_64_static
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_02/platform/external/selinux/sefcontext_compile^linux_glibc_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/selinux/libselinux/sefcontext_compile^linux_glibc_x86_64
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_02/platform/external/selinux/libselinux^linux_glibc_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/selinux/libselinux/libselinux^linux_glibc_x86_64_shared
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_02/platform/external/selinux/libselinux^android_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/external/selinux/libselinux/libselinux^android_x86_64_static

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_selinux.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/selinux/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_02 platform_external_selinux.tar.zst --clobber

du -ah -d1 platform_external_selinux*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_selinux.tar.zst" ]; then
  echo "Compressing platform/external/selinux -> platform_external_selinux.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_selinux.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/selinux/ .
fi
