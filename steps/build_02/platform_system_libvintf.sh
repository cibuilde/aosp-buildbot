...
clone_depth_platform platform/system/libvintf
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_02.ninja libvintf,linux_glibc_x86_64_static
mkdir -p $GITHUB_WORKSPACE/artifacts/system/libvintf/libvintf^linux_glibc_x86_64_static
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_02/platform/system/libvintf/libvintf^linux_glibc_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/system/libvintf/libvintf^linux_glibc_x86_64_static

echo "building libvintf^android_x86_x86_64_static"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_02.ninja libvintf,android_x86_x86_64_static
mkdir -p $GITHUB_WORKSPACE/artifacts/system/libvintf/libvintf^android_x86_x86_64_static
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_02/platform/system/libvintf/libvintf^android_x86_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/system/libvintf/libvintf^android_x86_x86_64_static

echo "building libvintf^android_x86_64_static"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_02.ninja libvintf,android_x86_64_static
mkdir -p $GITHUB_WORKSPACE/artifacts/system/libvintf/libvintf^android_x86_64_static
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_02/platform/system/libvintf/libvintf^android_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/system/libvintf/libvintf^android_x86_64_static

echo "building libvintf^android_recovery_x86_64_static"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_02.ninja libvintf,android_recovery_x86_64_static
mkdir -p $GITHUB_WORKSPACE/artifacts/system/libvintf/libvintf^android_recovery_x86_64_static
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_02/platform/system/libvintf/libvintf^android_recovery_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/system/libvintf/libvintf^android_recovery_x86_64_static

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_system_libvintf.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/system/libvintf/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_02 platform_system_libvintf.tar.zst --clobber

du -ah -d1 platform_system_libvintf*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_system_libvintf.tar.zst" ]; then
  echo "Compressing platform/system/libvintf -> platform_system_libvintf.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_system_libvintf.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/system/libvintf/ .
fi
