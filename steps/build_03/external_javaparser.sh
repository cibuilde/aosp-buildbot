...
echo "Preparing for external/javaparser"
...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_03.ninja javaparser,linux_glibc_common
...
python3 $GITHUB_WORKSPACE/copy_symlink.py $GITHUB_WORKSPACE/steps/outputs_03/external/javaparser/javaparser^linux_glibc_common.output $GITHUB_WORKSPACE/artifacts/external/javaparser/javaparser^linux_glibc_common $GITHUB_WORKSPACE/artifacts/external/javaparser/javaparser^linux_glibc_common/addition_copy_files.output
