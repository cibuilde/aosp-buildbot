...
ninja -f $GITHUB_WORKSPACE/steps/build_03.ninja libselinux,linux_glibc_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/external/selinux/libselinux/libselinux^linux_glibc_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_03/external/selinux/libselinux^linux_glibc_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/selinux/libselinux/libselinux^linux_glibc_x86_64_shared

echo "building sefcontext_compile^linux_glibc_x86_64"
ninja -f $GITHUB_WORKSPACE/steps/build_03.ninja sefcontext_compile,linux_glibc_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/external/selinux/libselinux/sefcontext_compile^linux_glibc_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_03/external/selinux/sefcontext_compile^linux_glibc_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/selinux/libselinux/sefcontext_compile^linux_glibc_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar cfJ external_selinux.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/selinux/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_03 external_selinux.tar.zst --clobber
