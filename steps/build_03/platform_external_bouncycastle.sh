...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_03.ninja bouncycastle-unbundled,linux_glibc_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/bouncycastle/bouncycastle-unbundled^linux_glibc_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_03/platform/external/bouncycastle/bouncycastle-unbundled^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/bouncycastle/bouncycastle-unbundled^linux_glibc_common

echo "building bouncycastle-bcpkix-unbundled^linux_glibc_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_03.ninja bouncycastle-bcpkix-unbundled,linux_glibc_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/bouncycastle/bouncycastle-bcpkix-unbundled^linux_glibc_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_03/platform/external/bouncycastle/bouncycastle-bcpkix-unbundled^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/bouncycastle/bouncycastle-bcpkix-unbundled^linux_glibc_common
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_03 platform_external_bouncycastle.tar.zst --clobber
