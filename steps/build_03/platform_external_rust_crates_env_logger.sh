...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_03.ninja libenv_logger,linux_glibc_x86_64_rlib_rlib-std
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/env_logger/libenv_logger^linux_glibc_x86_64_rlib_rlib-std
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_03/platform/external/rust/crates/env_logger/libenv_logger^linux_glibc_x86_64_rlib_rlib-std.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/env_logger/libenv_logger^linux_glibc_x86_64_rlib_rlib-std
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_03 platform_external_rust_crates_env_logger.tar.zst --clobber
