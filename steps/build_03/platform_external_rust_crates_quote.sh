...
clone_depth_platform platform/external/rust/crates/quote
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_03/platform/external/rust/crates/quote/libquote^linux_glibc_x86_64_rlib_rlib-std.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/quote/libquote^linux_glibc_x86_64_rlib_rlib-std

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_rust_crates_quote.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/rust/crates/quote/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_03 platform_external_rust_crates_quote.tar.zst --clobber

du -ah -d1 platform_external_rust_crates_quote*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_rust_crates_quote.tar.zst" ]; then
  echo "Compressing platform/external/rust/crates/quote -> platform_external_rust_crates_quote.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_rust_crates_quote.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/rust/crates/quote/ .
fi
