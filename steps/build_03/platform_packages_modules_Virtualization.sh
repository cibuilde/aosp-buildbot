...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_03.ninja microdroid_uboot_env_gen_x86_64,
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/modules/Virtualization/microdroid/microdroid_uboot_env_gen_x86_64^
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_03/platform/packages/modules/Virtualization/microdroid_uboot_env_gen_x86_64^.output . $GITHUB_WORKSPACE/artifacts/packages/modules/Virtualization/microdroid/microdroid_uboot_env_gen_x86_64^

echo "building microdroid_uboot_env^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_03.ninja microdroid_uboot_env,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/modules/Virtualization/microdroid/microdroid_uboot_env^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_03/platform/packages/modules/Virtualization/microdroid_uboot_env^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/packages/modules/Virtualization/microdroid/microdroid_uboot_env^android_x86_64
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_03 platform_packages_modules_Virtualization.tar.zst --clobber
