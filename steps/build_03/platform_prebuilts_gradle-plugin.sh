...
clone_depth_platform platform/prebuilts/gradle-plugin
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_03.ninja metalava-gradle-plugin-deps,linux_glibc_common
mkdir -p $GITHUB_WORKSPACE/artifacts/prebuilts/gradle-plugin/metalava-gradle-plugin-deps^linux_glibc_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_03/platform/prebuilts/gradle-plugin/metalava-gradle-plugin-deps^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/prebuilts/gradle-plugin/metalava-gradle-plugin-deps^linux_glibc_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_prebuilts_gradle-plugin.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/prebuilts/gradle-plugin/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_03 platform_prebuilts_gradle-plugin.tar.zst --clobber

du -ah -d1 platform_prebuilts_gradle-plugin*.tar.zst | sort -h

if [ ! -f "$GITHUB_WORKSPACE/cache/platform_prebuilts_gradle-plugin.tar.zst" ]; then
  echo "Compressing platform/prebuilts/gradle-plugin -> platform_prebuilts_gradle-plugin.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_prebuilts_gradle-plugin.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/prebuilts/gradle-plugin/ .
fi
