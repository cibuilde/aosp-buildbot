...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_03.ninja bluetooth_packetgen,linux_glibc_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/bt/gd/packet/parser/bluetooth_packetgen^linux_glibc_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_03/platform/system/bt/bluetooth_packetgen^linux_glibc_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/bt/gd/packet/parser/bluetooth_packetgen^linux_glibc_x86_64
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_03/platform/system/bt/RootCanalGeneratedPackets_h^.output . $GITHUB_WORKSPACE/artifacts/system/bt/vendor_libs/test_vendor_lib/RootCanalGeneratedPackets_h^
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_03/platform/system/bt/BluetoothGeneratedPackets_rust^.output . $GITHUB_WORKSPACE/artifacts/system/bt/gd/BluetoothGeneratedPackets_rust^
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_03/platform/system/bt/BluetoothGeneratedPackets_h^.output . $GITHUB_WORKSPACE/artifacts/system/bt/gd/BluetoothGeneratedPackets_h^

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_system_bt.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/system/bt/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_03 platform_system_bt.tar.zst --clobber
