...
ninja -f $GITHUB_WORKSPACE/steps/build_04.ninja icing-c-proto,android_x86_64_static_apex10000
...
cd $GITHUB_WORKSPACE/
tar cfJ external_icing.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/icing/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_icing.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_icing.tar.zst -C $GITHUB_WORKSPACE/aosp/external/icing/ .
fi
