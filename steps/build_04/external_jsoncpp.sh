...
ninja -f $GITHUB_WORKSPACE/steps/build_04.ninja libjsoncpp,linux_glibc_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/external/jsoncpp/libjsoncpp^linux_glibc_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/external/jsoncpp/libjsoncpp^linux_glibc_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/jsoncpp/libjsoncpp^linux_glibc_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar cfJ external_jsoncpp.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/jsoncpp/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_jsoncpp.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_jsoncpp.tar.zst -C $GITHUB_WORKSPACE/aosp/external/jsoncpp/ .
fi
