...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_04.ninja copy_clang-sys_build_out,
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/clang-sys/copy_clang-sys_build_out^
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/external/rust/crates/clang-sys/copy_clang-sys_build_out^.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/clang-sys/copy_clang-sys_build_out^
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_04 external_rust_crates_clang-sys.tar.zst --clobber
