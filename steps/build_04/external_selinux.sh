...
ninja -f $GITHUB_WORKSPACE/steps/build_04.ninja checkpolicy,linux_glibc_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/external/selinux/checkpolicy/checkpolicy^linux_glibc_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/external/selinux/checkpolicy^linux_glibc_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/selinux/checkpolicy/checkpolicy^linux_glibc_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar cfJ external_selinux.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/selinux/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_04 external_selinux.tar.zst --clobber
