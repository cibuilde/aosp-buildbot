...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/build/ota_metadata_proto_cc^android_recovery_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/build/make/tools/releasetools/ota_metadata_proto_cc^android_recovery_x86_64_static

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_build.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/build/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_04 platform_build.tar.zst --clobber
