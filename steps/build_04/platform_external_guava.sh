...
clone_depth_platform platform/external/guava
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_04.ninja guava,android_common_apex30
mkdir -p $GITHUB_WORKSPACE/artifacts/external/guava/guava^android_common_apex30
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/guava/guava^android_common_apex30.output . $GITHUB_WORKSPACE/artifacts/external/guava/guava^android_common_apex30

echo "building guava^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_04.ninja guava,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/guava/guava^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/guava/guava^android_common.output . $GITHUB_WORKSPACE/artifacts/external/guava/guava^android_common

echo "building guava-both^linux_glibc_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_04.ninja guava-both,linux_glibc_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/guava/guava-both^linux_glibc_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/guava/guava-both^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/guava/guava-both^linux_glibc_common

echo "building guava-jre^linux_glibc_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_04.ninja guava-jre,linux_glibc_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/guava/guava-jre^linux_glibc_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/guava/guava-jre^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/guava/guava-jre^linux_glibc_common

echo "building guava-android-host^linux_glibc_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_04.ninja guava-android-host,linux_glibc_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/guava/guava-android-host^linux_glibc_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/guava/guava-android-host^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/guava/guava-android-host^linux_glibc_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_guava.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/guava/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_04 platform_external_guava.tar.zst --clobber

du -ah -d1 platform_external_guava*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_guava.tar.zst" ]; then
  echo "Compressing platform/external/guava -> platform_external_guava.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_guava.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/guava/ .
fi
