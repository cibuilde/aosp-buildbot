...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/junit/junit^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/junit/junit^linux_glibc_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_junit.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/junit/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_04 platform_external_junit.tar.zst --clobber
