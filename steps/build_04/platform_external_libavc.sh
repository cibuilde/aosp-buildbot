...
clone_depth_platform platform/external/libavc
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/libavc/libavcenc^android_vendor.31_x86_x86_64_static_cfi.output . $GITHUB_WORKSPACE/artifacts/external/libavc/libavcenc^android_vendor.31_x86_x86_64_static_cfi
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/libavc/libavcdec^android_vendor.31_x86_x86_64_static_cfi.output . $GITHUB_WORKSPACE/artifacts/external/libavc/libavcdec^android_vendor.31_x86_x86_64_static_cfi

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_libavc.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/libavc/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_04 platform_external_libavc.tar.zst --clobber

du -ah -d1 platform_external_libavc*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_libavc.tar.zst" ]; then
  echo "Compressing platform/external/libavc -> platform_external_libavc.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_libavc.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/libavc/ .
fi
