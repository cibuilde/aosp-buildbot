...
clone_depth_platform platform/external/libbrillo
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/libbrillo/libbrillo^android_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/external/libbrillo/libbrillo^android_x86_64_static
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/libbrillo/libbrillo^android_recovery_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/external/libbrillo/libbrillo^android_recovery_x86_64_static
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/libbrillo/libbrillo-stream^android_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/external/libbrillo/libbrillo-stream^android_x86_64_static
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/libbrillo/libbrillo-stream^android_recovery_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/external/libbrillo/libbrillo-stream^android_recovery_x86_64_static

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_libbrillo.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/libbrillo/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_04 platform_external_libbrillo.tar.zst --clobber

du -ah -d1 platform_external_libbrillo*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_libbrillo.tar.zst" ]; then
  echo "Compressing platform/external/libbrillo -> platform_external_libbrillo.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_libbrillo.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/libbrillo/ .
fi
