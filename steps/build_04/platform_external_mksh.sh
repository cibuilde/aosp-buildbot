...
clone_depth_platform platform/external/mksh
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/mksh/sh_vendor^android_vendor.31_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/mksh/sh_vendor^android_vendor.31_x86_64
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/mksh/sh^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/mksh/sh^android_x86_64
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/mksh/sh^android_recovery_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/mksh/sh^android_recovery_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_mksh.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/mksh/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_04 platform_external_mksh.tar.zst --clobber

du -ah -d1 platform_external_mksh*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_mksh.tar.zst" ]; then
  echo "Compressing platform/external/mksh -> platform_external_mksh.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_mksh.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/mksh/ .
fi
