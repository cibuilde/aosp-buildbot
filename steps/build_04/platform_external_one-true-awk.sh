...
rsync -a -r $GITHUB_WORKSPACE/downloads/external/one-true-awk/awkgram.tab.c^/ .
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/one-true-awk/awk_vendor^android_vendor.31_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/one-true-awk/awk_vendor^android_vendor.31_x86_64
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/one-true-awk/awk^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/one-true-awk/awk^android_x86_64
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_04 platform_external_one-true-awk.tar.zst --clobber
