...
clone_depth_platform platform/external/pcre
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/pcre/libpcre2^android_x86_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/pcre/libpcre2^android_x86_x86_64_shared
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/pcre/libpcre2^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/pcre/libpcre2^android_x86_64_shared
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/pcre/libpcre2^android_vendor.31_x86_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/external/pcre/libpcre2^android_vendor.31_x86_x86_64_static
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/pcre/libpcre2^android_vendor.31_x86_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/pcre/libpcre2^android_vendor.31_x86_x86_64_shared
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/pcre/libpcre2^android_vendor.31_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/external/pcre/libpcre2^android_vendor.31_x86_64_static

echo "building libpcre2^android_vendor.31_x86_64_shared"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_04.ninja libpcre2,android_vendor.31_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/external/pcre/libpcre2^android_vendor.31_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/pcre/libpcre2^android_vendor.31_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/pcre/libpcre2^android_vendor.31_x86_64_shared
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/pcre/libpcre2^android_recovery_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/pcre/libpcre2^android_recovery_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_pcre.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/pcre/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_04 platform_external_pcre.tar.zst --clobber

du -ah -d1 platform_external_pcre*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_pcre.tar.zst" ]; then
  echo "Compressing platform/external/pcre -> platform_external_pcre.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_pcre.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/pcre/ .
fi
