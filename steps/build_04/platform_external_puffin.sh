...
clone_depth_platform platform/external/puffin
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/puffin/libpuffpatch^android_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/external/puffin/libpuffpatch^android_x86_64_static
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/puffin/libpuffpatch^android_recovery_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/external/puffin/libpuffpatch^android_recovery_x86_64_static

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_puffin.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/puffin/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_04 platform_external_puffin.tar.zst --clobber

du -ah -d1 platform_external_puffin*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_puffin.tar.zst" ]; then
  echo "Compressing platform/external/puffin -> platform_external_puffin.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_puffin.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/puffin/ .
fi
