...
clone_depth_platform platform/external/rust/crates/parking_lot_core
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/rust/crates/parking_lot_core/libparking_lot_core^android_x86_x86_64_rlib_rlib-std.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/parking_lot_core/libparking_lot_core^android_x86_x86_64_rlib_rlib-std
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/rust/crates/parking_lot_core/libparking_lot_core^android_x86_64_rlib_rlib-std.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/parking_lot_core/libparking_lot_core^android_x86_64_rlib_rlib-std

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_rust_crates_parking_lot_core.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/rust/crates/parking_lot_core/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_04 platform_external_rust_crates_parking_lot_core.tar.zst --clobber

du -ah -d1 platform_external_rust_crates_parking_lot_core*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_rust_crates_parking_lot_core.tar.zst" ]; then
  echo "Compressing platform/external/rust/crates/parking_lot_core -> platform_external_rust_crates_parking_lot_core.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_rust_crates_parking_lot_core.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/rust/crates/parking_lot_core/ .
fi
