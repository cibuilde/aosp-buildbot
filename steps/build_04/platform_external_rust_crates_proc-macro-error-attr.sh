...
clone_depth_platform platform/external/rust/crates/proc-macro-error-attr
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_04.ninja libproc_macro_error_attr,linux_glibc_x86_64
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/rust/crates/proc-macro-error-attr/libproc_macro_error_attr^linux_glibc_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/proc-macro-error-attr/libproc_macro_error_attr^linux_glibc_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_rust_crates_proc-macro-error-attr.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/rust/crates/proc-macro-error-attr/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_04 platform_external_rust_crates_proc-macro-error-attr.tar.zst --clobber

du -ah -d1 platform_external_rust_crates_proc-macro-error-attr*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_rust_crates_proc-macro-error-attr.tar.zst" ]; then
  echo "Compressing platform/external/rust/crates/proc-macro-error-attr -> platform_external_rust_crates_proc-macro-error-attr.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_rust_crates_proc-macro-error-attr.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/rust/crates/proc-macro-error-attr/ .
fi
