...
clone_depth_platform platform/external/rust/crates/syn
...
echo "building libsyn^linux_glibc_x86_64_rlib_rlib-std"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_04.ninja libsyn,linux_glibc_x86_64_rlib_rlib-std
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/syn/libsyn^linux_glibc_x86_64_rlib_rlib-std
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/rust/crates/syn/libsyn^linux_glibc_x86_64_rlib_rlib-std.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/syn/libsyn^linux_glibc_x86_64_rlib_rlib-std

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_rust_crates_syn.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/rust/crates/syn/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_04 platform_external_rust_crates_syn.tar.zst --clobber

du -ah -d1 platform_external_rust_crates_syn*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_rust_crates_syn.tar.zst" ]; then
  echo "Compressing platform/external/rust/crates/syn -> platform_external_rust_crates_syn.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_rust_crates_syn.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/rust/crates/syn/ .
fi
