...
clone_depth_platform platform/external/tflite-support
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/tflite-support/tflite_support_spm_encoder_config^.output . $GITHUB_WORKSPACE/artifacts/external/tflite-support/tflite_support_spm_encoder_config^
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/external/tflite-support/tflite_support_spm_config^.output . $GITHUB_WORKSPACE/artifacts/external/tflite-support/tflite_support_spm_config^

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_tflite-support.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/tflite-support/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_04 platform_external_tflite-support.tar.zst --clobber

du -ah -d1 platform_external_tflite-support*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_tflite-support.tar.zst" ]; then
  echo "Compressing platform/external/tflite-support -> platform_external_tflite-support.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_tflite-support.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/tflite-support/ .
fi
