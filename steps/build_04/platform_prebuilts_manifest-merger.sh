...
clone_depth_platform platform/prebuilts/manifest-merger
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_04.ninja manifest-merger,linux_glibc_common
mkdir -p $GITHUB_WORKSPACE/artifacts/prebuilts/manifest-merger/manifest-merger^linux_glibc_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/prebuilts/manifest-merger/manifest-merger^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/prebuilts/manifest-merger/manifest-merger^linux_glibc_common

echo "building manifest-merger^linux_glibc_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_04.ninja manifest-merger,linux_glibc_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/prebuilts/manifest-merger/manifest-merger^linux_glibc_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/prebuilts/manifest-merger/manifest-merger^linux_glibc_x86_64.output . $GITHUB_WORKSPACE/artifacts/prebuilts/manifest-merger/manifest-merger^linux_glibc_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_prebuilts_manifest-merger.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/prebuilts/manifest-merger/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_04 platform_prebuilts_manifest-merger.tar.zst --clobber

du -ah -d1 platform_prebuilts_manifest-merger*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_prebuilts_manifest-merger.tar.zst" ]; then
  echo "Compressing platform/prebuilts/manifest-merger -> platform_prebuilts_manifest-merger.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_prebuilts_manifest-merger.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/prebuilts/manifest-merger/ .
fi
