...
clone_depth_platform platform/system/tools/xsdc
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_04.ninja xsdc,linux_glibc_common
mkdir -p $GITHUB_WORKSPACE/artifacts/system/tools/xsdc/xsdc^linux_glibc_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/system/tools/xsdc/xsdc^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/system/tools/xsdc/xsdc^linux_glibc_common

echo "building xsdc^linux_glibc_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_04.ninja xsdc,linux_glibc_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/tools/xsdc/xsdc^linux_glibc_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/system/tools/xsdc/xsdc^linux_glibc_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/tools/xsdc/xsdc^linux_glibc_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_system_tools_xsdc.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/system/tools/xsdc/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_04 platform_system_tools_xsdc.tar.zst --clobber

du -ah -d1 platform_system_tools_xsdc*.tar.zst | sort -h

if [ ! -f "$GITHUB_WORKSPACE/cache/build_soong.tar.zst" ]; then
  echo "Compressing build/soong -> build_soong.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/build_soong.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/build/soong/ .
fi
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_system_tools_xsdc.tar.zst" ]; then
  echo "Compressing platform/system/tools/xsdc -> platform_system_tools_xsdc.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_system_tools_xsdc.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/system/tools/xsdc/ .
fi
