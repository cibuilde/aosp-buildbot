...
clone_depth_platform platform/tools/metalava
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_04.ninja metalava,linux_glibc_common
mkdir -p $GITHUB_WORKSPACE/artifacts/tools/metalava/metalava^linux_glibc_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/tools/metalava/metalava^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/tools/metalava/metalava^linux_glibc_common
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/tools/metalava/private-stub-annotations^.output . $GITHUB_WORKSPACE/artifacts/tools/metalava/private-stub-annotations^
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_04/platform/tools/metalava/metalava^linux_glibc_x86_64.output . $GITHUB_WORKSPACE/artifacts/tools/metalava/metalava^linux_glibc_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_tools_metalava.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/tools/metalava/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_04 platform_tools_metalava.tar.zst --clobber

du -ah -d1 platform_tools_metalava*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_tools_metalava.tar.zst" ]; then
  echo "Compressing platform/tools/metalava -> platform_tools_metalava.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_tools_metalava.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/tools/metalava/ .
fi
