...
ninja -f $GITHUB_WORKSPACE/steps/build_04.ninja update_metadata-protos,android_recovery_x86_64_static
...
ninja -f $GITHUB_WORKSPACE/steps/build_04.ninja update_metadata-protos,android_x86_64_static
...
cd $GITHUB_WORKSPACE/
tar cfJ system_update_engine.tar.zst -C $GITHUB_WORKSPACE/artifacts/system/update_engine/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/system_update_engine.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/system_update_engine.tar.zst -C $GITHUB_WORKSPACE/aosp/system/update_engine/ .
fi
