...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_05.ninja blueprint-bootstrap,linux_glibc_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/build/blueprint/blueprint-bootstrap^linux_glibc_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/build/blueprint/blueprint-bootstrap^linux_glibc_x86_64.output . $GITHUB_WORKSPACE/artifacts/build/blueprint/blueprint-bootstrap^linux_glibc_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf build_blueprint.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/build/blueprint/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_05 build_blueprint.tar.zst --clobber
