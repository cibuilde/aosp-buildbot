...
ninja -f $GITHUB_WORKSPACE/steps/build_05.ninja google_java_format_android_annotation_stubs,linux_glibc_common
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/external/google-java-format/google_java_format_android_annotation_stubs^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/google-java-format/google_java_format_android_annotation_stubs^linux_glibc_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar cfJ external_google-java-format.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/google-java-format/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_google-java-format.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_google-java-format.tar.zst -C $GITHUB_WORKSPACE/aosp/external/google-java-format/ .
fi
