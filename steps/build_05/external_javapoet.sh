...
ninja -f $GITHUB_WORKSPACE/steps/build_05.ninja javapoet,linux_glibc_common
...
cd $GITHUB_WORKSPACE/
tar cfJ external_javapoet.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/javapoet/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_javapoet.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_javapoet.tar.zst -C $GITHUB_WORKSPACE/aosp/external/javapoet/ .
fi
