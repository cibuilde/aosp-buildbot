...
ninja -f $GITHUB_WORKSPACE/steps/build_05.ninja jsr305,linux_glibc_common
...
cd $GITHUB_WORKSPACE/
tar cfJ external_jsr305.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/jsr305/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_jsr305.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_jsr305.tar.zst -C $GITHUB_WORKSPACE/aosp/external/jsr305/ .
fi
