...
ninja -f $GITHUB_WORKSPACE/steps/build_05.ninja jsr330,linux_glibc_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/jsr330/jsr330^linux_glibc_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/external/jsr330/jsr330^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/jsr330/jsr330^linux_glibc_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar cfJ external_jsr330.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/jsr330/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_05 external_jsr330.tar.zst --clobber

du -ah -d1| sort -h

if [ ! -f "$GITHUB_WORKSPACE/cache/external_jsr330.tar.zst" ]; then
  echo "Compressing external/jsr330 -> external_jsr330.tar.zst"
  tar cfJ $GITHUB_WORKSPACE/cache/external_jsr330.tar.zst -C $GITHUB_WORKSPACE/aosp/external/jsr330/ .
fi
