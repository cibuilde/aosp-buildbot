...
ninja -f $GITHUB_WORKSPACE/steps/build_05.ninja libasync_trait,linux_glibc_x86_64
...
cd $GITHUB_WORKSPACE/
tar cfJ external_rust_crates_async-trait.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/rust/crates/async-trait/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_rust_crates_async-trait.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_rust_crates_async-trait.tar.zst -C $GITHUB_WORKSPACE/aosp/external/rust/crates/async-trait/ .
fi
