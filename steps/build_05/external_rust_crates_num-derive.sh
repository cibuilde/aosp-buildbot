...
echo "Preparing for external/rust/crates/num-derive"
...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_05.ninja libnum_derive,linux_glibc_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/num-derive/libnum_derive^linux_glibc_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/external/rust/crates/num-derive/libnum_derive^linux_glibc_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/num-derive/libnum_derive^linux_glibc_x86_64
python3 $GITHUB_WORKSPACE/copy_symlink.py $GITHUB_WORKSPACE/steps/outputs_05/external/rust/crates/num-derive/libnum_derive^linux_glibc_x86_64.output $GITHUB_WORKSPACE/artifacts/external/rust/crates/num-derive/libnum_derive^linux_glibc_x86_64 $GITHUB_WORKSPACE/artifacts/external/rust/crates/num-derive/libnum_derive^linux_glibc_x86_64/addition_copy_files.output


rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_rust_crates_num-derive.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/rust/crates/num-derive/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_05 external_rust_crates_num-derive.tar.zst --clobber

du -ah -d1 external_rust_crates_num-derive*.tar.zst | sort -h
