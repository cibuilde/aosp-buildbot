...
cd $GITHUB_WORKSPACE/
tar -cf external_rust_crates_proc-macro-error.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/rust/crates/proc-macro-error/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_05 external_rust_crates_proc-macro-error.tar.zst --clobber

du -ah -d1 external_rust_crates_proc-macro-error*.tar.zst | sort -h
