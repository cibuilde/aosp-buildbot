...
ninja -f $GITHUB_WORKSPACE/steps/build_05.ninja libproc_macro_error,linux_glibc_x86_64_rlib_rlib-std
...
cd $GITHUB_WORKSPACE/
tar cfJ external_rust_crates_proc-macro-error.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/rust/crates/proc-macro-error/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_05 external_rust_crates_proc-macro-error.tar.zst --clobber
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_rust_crates_proc-macro-error.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_rust_crates_proc-macro-error.tar.zst -C $GITHUB_WORKSPACE/aosp/external/rust/crates/proc-macro-error/ .
fi
if [ ! -f "$GITHUB_WORKSPACE/cache/external_rust_crates_proc-macro-error-attr.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_rust_crates_proc-macro-error-attr.tar.zst -C $GITHUB_WORKSPACE/aosp/external/rust/crates/proc-macro-error-attr/ .
fi
