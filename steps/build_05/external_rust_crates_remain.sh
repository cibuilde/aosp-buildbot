...
cd $GITHUB_WORKSPACE/
tar cfJ external_rust_crates_remain.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/rust/crates/remain/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_rust_crates_remain.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_rust_crates_remain.tar.zst -C $GITHUB_WORKSPACE/aosp/external/rust/crates/remain/ .
fi
