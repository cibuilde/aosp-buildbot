...
ninja -f $GITHUB_WORKSPACE/steps/build_05.ninja libserde_derive,linux_glibc_x86_64
...
cd $GITHUB_WORKSPACE/
tar cfJ external_rust_crates_serde_derive.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/rust/crates/serde_derive/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_rust_crates_serde.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_rust_crates_serde.tar.zst -C $GITHUB_WORKSPACE/aosp/external/rust/crates/serde/ .
fi
if [ ! -f "$GITHUB_WORKSPACE/cache/external_rust_crates_serde_derive.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_rust_crates_serde_derive.tar.zst -C $GITHUB_WORKSPACE/aosp/external/rust/crates/serde_derive/ .
fi
