...
ninja -f $GITHUB_WORKSPACE/steps/build_05.ninja libthiserror_impl,linux_glibc_x86_64
...
cd $GITHUB_WORKSPACE/
tar cfJ external_rust_crates_thiserror-impl.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/rust/crates/thiserror-impl/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_rust_crates_thiserror.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_rust_crates_thiserror.tar.zst -C $GITHUB_WORKSPACE/aosp/external/rust/crates/thiserror/ .
fi
if [ ! -f "$GITHUB_WORKSPACE/cache/external_rust_crates_thiserror-impl.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_rust_crates_thiserror-impl.tar.zst -C $GITHUB_WORKSPACE/aosp/external/rust/crates/thiserror-impl/ .
fi
