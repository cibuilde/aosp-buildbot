...
ninja -f $GITHUB_WORKSPACE/steps/build_05.ninja libtokio_macros,linux_glibc_x86_64
...
cd $GITHUB_WORKSPACE/
tar cfJ external_rust_crates_tokio-macros.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/rust/crates/tokio-macros/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_rust_crates_tokio.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_rust_crates_tokio.tar.zst -C $GITHUB_WORKSPACE/aosp/external/rust/crates/tokio/ .
fi
if [ ! -f "$GITHUB_WORKSPACE/cache/external_rust_crates_tokio-macros.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_rust_crates_tokio-macros.tar.zst -C $GITHUB_WORKSPACE/aosp/external/rust/crates/tokio-macros/ .
fi
