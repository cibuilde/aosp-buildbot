...
cd $GITHUB_WORKSPACE/
tar cfJ external_tflite-support.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/tflite-support/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_tflite-support.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_tflite-support.tar.zst -C $GITHUB_WORKSPACE/aosp/external/tflite-support/ .
fi
