...
ninja -f $GITHUB_WORKSPACE/steps/build_05.ninja libwire_format_derive,linux_glibc_x86_64
...
cd $GITHUB_WORKSPACE/
tar cfJ external_vm_tools_p9.tar.zst -C $GITHUB_WORKSPACE/artifacts/external/vm_tools/p9/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_05 external_vm_tools_p9.tar.zst --clobber
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_vm_tools_p9.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/external_vm_tools_p9.tar.zst -C $GITHUB_WORKSPACE/aosp/external/vm_tools/p9/ .
fi
