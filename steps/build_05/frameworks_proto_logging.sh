...
ninja -f $GITHUB_WORKSPACE/steps/build_05.ninja libstats_proto_host,linux_glibc_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/frameworks/proto_logging/stats/libstats_proto_host^linux_glibc_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/frameworks/proto_logging/libstats_proto_host^linux_glibc_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/frameworks/proto_logging/stats/libstats_proto_host^linux_glibc_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar cfJ frameworks_proto_logging.tar.zst -C $GITHUB_WORKSPACE/artifacts/frameworks/proto_logging/ .
