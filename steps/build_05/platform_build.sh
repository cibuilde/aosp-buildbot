...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/build/verity_utils^linux_glibc_x86_64_PY2.output . $GITHUB_WORKSPACE/artifacts/build/make/tools/releasetools/verity_utils^linux_glibc_x86_64_PY2
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/build/sparse_img^linux_glibc_x86_64_PY2.output . $GITHUB_WORKSPACE/artifacts/build/make/tools/releasetools/sparse_img^linux_glibc_x86_64_PY2
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/build/signapk^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/build/make/tools/signapk/signapk^linux_glibc_common
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/build/build_image^linux_glibc_x86_64_PY2.output . $GITHUB_WORKSPACE/artifacts/build/make/tools/releasetools/build_image^linux_glibc_x86_64_PY2

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_build.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/build/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_05 platform_build.tar.zst --clobber
