...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/build/soong/merge_csv^linux_glibc_x86_64_PY3.output . $GITHUB_WORKSPACE/artifacts/build/soong/scripts/hiddenapi/merge_csv^linux_glibc_x86_64_PY3
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/build/soong/generate_hiddenapi_lists^linux_glibc_x86_64_PY3.output . $GITHUB_WORKSPACE/artifacts/build/soong/scripts/hiddenapi/generate_hiddenapi_lists^linux_glibc_x86_64_PY3
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/build/soong/conv_linker_config^linux_glibc_x86_64_PY3.output . $GITHUB_WORKSPACE/artifacts/build/soong/scripts/conv_linker_config^linux_glibc_x86_64_PY3

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_build_soong.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/build/soong/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_05 platform_build_soong.tar.zst --clobber
