...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/external/auto/auto_value_extension^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/auto/value/auto_value_extension^linux_glibc_common
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/external/auto/auto_common^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/auto/common/auto_common^linux_glibc_common
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/external/auto/auto_service_plugin^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/auto/service/auto_service_plugin^linux_glibc_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_auto.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/auto/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_05 platform_external_auto.tar.zst --clobber
