...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_05.ninja avbtool,linux_glibc_x86_64_PY3
mkdir -p $GITHUB_WORKSPACE/artifacts/external/avb/avbtool^linux_glibc_x86_64_PY3
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/external/avb/avbtool^linux_glibc_x86_64_PY3.output . $GITHUB_WORKSPACE/artifacts/external/avb/avbtool^linux_glibc_x86_64_PY3

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_avb.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/avb/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_05 platform_external_avb.tar.zst --clobber
