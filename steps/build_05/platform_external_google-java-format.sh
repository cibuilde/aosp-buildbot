...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_05.ninja google_java_format,linux_glibc_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/google-java-format/google_java_format^linux_glibc_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/external/google-java-format/google_java_format^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/google-java-format/google_java_format^linux_glibc_common
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_05 platform_external_google-java-format.tar.zst --clobber
