...
clone_depth_platform platform/external/guice
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/external/guice/guice_munge^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/guice/guice_munge^linux_glibc_common
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/external/guice/guice_munged_srcs^.output . $GITHUB_WORKSPACE/artifacts/external/guice/guice_munged_srcs^
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/external/guice/guice^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/guice/guice^linux_glibc_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_guice.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/guice/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_05 platform_external_guice.tar.zst --clobber

du -ah -d1 platform_external_guice*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_guice.tar.zst" ]; then
  echo "Compressing platform/external/guice -> platform_external_guice.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_guice.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/guice/ .
fi
