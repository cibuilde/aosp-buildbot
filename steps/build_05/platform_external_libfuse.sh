...
clone_depth_platform platform/external/libfuse
...
echo "building libfuse^android_x86_64_sdk_static_apex30"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_05.ninja libfuse,android_x86_64_sdk_static_apex30
mkdir -p $GITHUB_WORKSPACE/artifacts/external/libfuse/libfuse^android_x86_64_sdk_static_apex30
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/external/libfuse/libfuse^android_x86_64_sdk_static_apex30.output . $GITHUB_WORKSPACE/artifacts/external/libfuse/libfuse^android_x86_64_sdk_static_apex30

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_libfuse.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/libfuse/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_05 platform_external_libfuse.tar.zst --clobber

du -ah -d1 platform_external_libfuse*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_libfuse.tar.zst" ]; then
  echo "Compressing platform/external/libfuse -> platform_external_libfuse.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_libfuse.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/libfuse/ .
fi
