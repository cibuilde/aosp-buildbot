...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_05.ninja libLLVM_android,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/external/llvm/libLLVM_android^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/external/llvm/libLLVM_android^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/llvm/libLLVM_android^android_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_llvm.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/llvm/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_05 platform_external_llvm.tar.zst --clobber
