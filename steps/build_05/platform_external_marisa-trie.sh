...
clone_depth_platform platform/external/marisa-trie
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_05.ninja marisa-trie,android_x86_64_sdk_static_apex30
mkdir -p $GITHUB_WORKSPACE/artifacts/external/marisa-trie/marisa-trie^android_x86_64_sdk_static_apex30
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/external/marisa-trie/marisa-trie^android_x86_64_sdk_static_apex30.output . $GITHUB_WORKSPACE/artifacts/external/marisa-trie/marisa-trie^android_x86_64_sdk_static_apex30

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_marisa-trie.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/marisa-trie/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_05 platform_external_marisa-trie.tar.zst --clobber

du -ah -d1 platform_external_marisa-trie*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_marisa-trie.tar.zst" ]; then
  echo "Compressing platform/external/marisa-trie -> platform_external_marisa-trie.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_marisa-trie.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/marisa-trie/ .
fi
