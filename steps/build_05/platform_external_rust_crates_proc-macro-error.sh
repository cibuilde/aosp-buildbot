...
clone_depth_platform platform/external/rust/crates/proc-macro-error
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_05.ninja libproc_macro_error,linux_glibc_x86_64_rlib_rlib-std
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/external/rust/crates/proc-macro-error/libproc_macro_error^linux_glibc_x86_64_rlib_rlib-std.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/proc-macro-error/libproc_macro_error^linux_glibc_x86_64_rlib_rlib-std

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_rust_crates_proc-macro-error.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/rust/crates/proc-macro-error/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_05 platform_external_rust_crates_proc-macro-error.tar.zst --clobber

du -ah -d1 platform_external_rust_crates_proc-macro-error*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_rust_crates_proc-macro-error.tar.zst" ]; then
  echo "Compressing platform/external/rust/crates/proc-macro-error -> platform_external_rust_crates_proc-macro-error.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_rust_crates_proc-macro-error.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/rust/crates/proc-macro-error/ .
fi
