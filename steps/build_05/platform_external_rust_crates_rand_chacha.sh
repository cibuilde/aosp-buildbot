...
clone_depth_platform platform/external/rust/crates/rand_chacha
...
echo "building librand_chacha^android_x86_64_rlib_rlib-std"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_05.ninja librand_chacha,android_x86_64_rlib_rlib-std
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/rand_chacha/librand_chacha^android_x86_64_rlib_rlib-std
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/external/rust/crates/rand_chacha/librand_chacha^android_x86_64_rlib_rlib-std.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/rand_chacha/librand_chacha^android_x86_64_rlib_rlib-std

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_rust_crates_rand_chacha.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/rust/crates/rand_chacha/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_05 platform_external_rust_crates_rand_chacha.tar.zst --clobber

du -ah -d1 platform_external_rust_crates_rand_chacha*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_rust_crates_rand_chacha.tar.zst" ]; then
  echo "Compressing platform/external/rust/crates/rand_chacha -> platform_external_rust_crates_rand_chacha.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_rust_crates_rand_chacha.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/rust/crates/rand_chacha/ .
fi
