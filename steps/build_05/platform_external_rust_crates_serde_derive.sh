...
clone_depth_platform platform/external/rust/crates/serde_derive
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_05.ninja libserde_derive,linux_glibc_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/serde_derive/libserde_derive^linux_glibc_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/external/rust/crates/serde_derive/libserde_derive^linux_glibc_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/serde_derive/libserde_derive^linux_glibc_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_rust_crates_serde_derive.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/rust/crates/serde_derive/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_05 platform_external_rust_crates_serde_derive.tar.zst --clobber

du -ah -d1 platform_external_rust_crates_serde_derive*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_rust_crates_serde_derive.tar.zst" ]; then
  echo "Compressing platform/external/rust/crates/serde_derive -> platform_external_rust_crates_serde_derive.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_rust_crates_serde_derive.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/rust/crates/serde_derive/ .
fi
