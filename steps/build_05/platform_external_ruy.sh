...
clone_depth_platform platform/external/ruy
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_05.ninja libruy_static,android_x86_64_sdk_static_apex30
mkdir -p $GITHUB_WORKSPACE/artifacts/external/ruy/libruy_static^android_x86_64_sdk_static_apex30
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/external/ruy/libruy_static^android_x86_64_sdk_static_apex30.output . $GITHUB_WORKSPACE/artifacts/external/ruy/libruy_static^android_x86_64_sdk_static_apex30

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_ruy.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/ruy/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_05 platform_external_ruy.tar.zst --clobber

du -ah -d1 platform_external_ruy*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_ruy.tar.zst" ]; then
  echo "Compressing platform/external/ruy -> platform_external_ruy.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_ruy.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/ruy/ .
fi
