...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_05.ninja coex-table-parser,
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/modules/Wifi/service/coex-table-parser/coex-table-parser^
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/packages/modules/Wifi/coex-table-parser^.output . $GITHUB_WORKSPACE/artifacts/packages/modules/Wifi/service/coex-table-parser/coex-table-parser^
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_05 platform_packages_modules_Wifi.tar.zst --clobber
