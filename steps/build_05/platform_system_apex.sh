...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/system/apex/conv_apex_manifest^linux_glibc_x86_64_PY2.output . $GITHUB_WORKSPACE/artifacts/system/apex/apexer/conv_apex_manifest^linux_glibc_x86_64_PY2
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/system/apex/apexer^linux_glibc_x86_64_PY2.output . $GITHUB_WORKSPACE/artifacts/system/apex/apexer/apexer^linux_glibc_x86_64_PY2
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/system/apex/apex_compression_tool^linux_glibc_x86_64_PY3.output . $GITHUB_WORKSPACE/artifacts/system/apex/tools/apex_compression_tool^linux_glibc_x86_64_PY3
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_05/platform/system/apex/apex-info-list^.output . $GITHUB_WORKSPACE/artifacts/system/apex/apexd/apex-info-list^

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_system_apex.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/system/apex/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_05 platform_system_apex.tar.zst --clobber
