...
cd $GITHUB_WORKSPACE/
tar cfJ system_linkerconfig.tar.zst -C $GITHUB_WORKSPACE/artifacts/system/linkerconfig/ .
...
if [ ! -f "$GITHUB_WORKSPACE/cache/system_linkerconfig.tar.zst" ]; then
  ...
  tar cfJ $GITHUB_WORKSPACE/cache/system_linkerconfig.tar.zst -C $GITHUB_WORKSPACE/aosp/system/linkerconfig/ .
fi
