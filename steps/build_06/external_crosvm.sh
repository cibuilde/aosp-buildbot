...
rsync -a -r $GITHUB_WORKSPACE/artifacts/external/crosvm/gpu_display/gpu_display_client_protocol_headers^/ .
rsync -a -r $GITHUB_WORKSPACE/artifacts/external/crosvm/gpu_display/gpu_display_protocol_sources^/ .
...
ninja -f $GITHUB_WORKSPACE/steps/build_06.ninja libdisplay_wl,android_x86_64_static_apex10000
mkdir -p $GITHUB_WORKSPACE/artifacts/external/crosvm/gpu_display/libdisplay_wl^android_x86_64_static_apex10000
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_06/external/crosvm/libdisplay_wl^android_x86_64_static_apex10000.output . $GITHUB_WORKSPACE/artifacts/external/crosvm/gpu_display/libdisplay_wl^android_x86_64_static_apex10000

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_crosvm.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/crosvm/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_06 external_crosvm.tar.zst --clobber
