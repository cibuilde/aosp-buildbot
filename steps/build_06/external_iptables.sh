...
rsync -a -r $GITHUB_WORKSPACE/artifacts/external/iptables/extensions/libext_init^/ .
rsync -a -r $GITHUB_WORKSPACE/artifacts/external/iptables/extensions/libext_srcs^/ .

echo "building libext^android_x86_64_static"
ninja -f $GITHUB_WORKSPACE/steps/build_06.ninja libext,android_x86_64_static
mkdir -p $GITHUB_WORKSPACE/artifacts/external/iptables/extensions/libext^android_x86_64_static
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_06/external/iptables/libext^android_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/external/iptables/extensions/libext^android_x86_64_static

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_iptables.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/iptables/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_06 external_iptables.tar.zst --clobber
