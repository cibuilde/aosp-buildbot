...
rsync -a -r $GITHUB_WORKSPACE/artifacts/external/rust/crates/codespan-reporting/libcodespan_reporting^linux_glibc_x86_64_rlib_rlib-std/ .
...
rsync -a -r $GITHUB_WORKSPACE/artifacts/external/rust/cxx/gen/cmd/libcxxbridge_cmd^linux_glibc_x86_64_rlib_rlib-std/ .

echo "building cxxbridge^linux_glibc_x86_64"
ninja -f $GITHUB_WORKSPACE/steps/build_06.ninja cxxbridge,linux_glibc_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/cxx/gen/cmd/cxxbridge^linux_glibc_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_06/external/rust/cxx/cxxbridge^linux_glibc_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/rust/cxx/gen/cmd/cxxbridge^linux_glibc_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_rust_cxx.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/rust/cxx/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_06 external_rust_cxx.tar.zst --clobber
