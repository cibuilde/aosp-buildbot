...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_06.ninja statslog-telephony-java-gen,
mkdir -p $GITHUB_WORKSPACE/artifacts/frameworks/opt/telephony/statslog-telephony-java-gen^
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_06/frameworks/opt/telephony/statslog-telephony-java-gen^.output . $GITHUB_WORKSPACE/artifacts/frameworks/opt/telephony/statslog-telephony-java-gen^
python3 $GITHUB_WORKSPACE/copy_symlink.py $GITHUB_WORKSPACE/steps/outputs_06/frameworks/opt/telephony/statslog-telephony-java-gen^.output $GITHUB_WORKSPACE/artifacts/frameworks/opt/telephony/statslog-telephony-java-gen^ $GITHUB_WORKSPACE/artifacts/frameworks/opt/telephony/statslog-telephony-java-gen^/addition_copy_files.output
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_06 frameworks_opt_telephony.tar.zst --clobber
