...
clone_depth_platform platform/external/auto
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_06/platform/external/auto/libauto_value_plugin^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/auto/value/libauto_value_plugin^linux_glibc_common
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_06/platform/external/auto/auto_value_plugin^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/auto/value/auto_value_plugin^linux_glibc_common

echo "building auto_value_memoized_extension_plugin^linux_glibc_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_06.ninja auto_value_memoized_extension_plugin,linux_glibc_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/auto/value/auto_value_memoized_extension_plugin^linux_glibc_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_06/platform/external/auto/auto_value_memoized_extension_plugin^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/auto/value/auto_value_memoized_extension_plugin^linux_glibc_common
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_06/platform/external/auto/auto_factory_plugin^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/auto/factory/auto_factory_plugin^linux_glibc_common
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_06/platform/external/auto/auto_annotation_plugin^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/auto/value/auto_annotation_plugin^linux_glibc_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_auto.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/auto/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_06 platform_external_auto.tar.zst --clobber

du -ah -d1 platform_external_auto*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_auto.tar.zst" ]; then
  echo "Compressing platform/external/auto -> platform_external_auto.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_auto.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/auto/ .
fi
