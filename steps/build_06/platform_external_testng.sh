...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_06.ninja testng,linux_glibc_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/testng/testng^linux_glibc_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_06/platform/external/testng/testng^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/testng/testng^linux_glibc_common
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_06 platform_external_testng.tar.zst --clobber
