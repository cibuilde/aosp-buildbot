...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_06/platform/libcore/libjavacore^android_x86_x86_64_shared_apex31.output . $GITHUB_WORKSPACE/artifacts/libcore/libjavacore^android_x86_x86_64_shared_apex31
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_06/platform/libcore/libjavacore^android_x86_64_shared_apex31.output . $GITHUB_WORKSPACE/artifacts/libcore/libjavacore^android_x86_64_shared_apex31

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_libcore.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/libcore/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_06 platform_libcore.tar.zst --clobber
