...
clone_depth_platform platform/packages/apps/DocumentsUI
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_06.ninja statslog-docsui-java-gen,
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/apps/DocumentsUI/statslog-docsui-java-gen^
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_06/platform/packages/apps/DocumentsUI/statslog-docsui-java-gen^.output . $GITHUB_WORKSPACE/artifacts/packages/apps/DocumentsUI/statslog-docsui-java-gen^

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_apps_DocumentsUI.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/apps/DocumentsUI/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_06 platform_packages_apps_DocumentsUI.tar.zst --clobber

du -ah -d1 platform_packages_apps_DocumentsUI*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_apps_DocumentsUI.tar.zst" ]; then
  echo "Compressing platform/packages/apps/DocumentsUI -> platform_packages_apps_DocumentsUI.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_apps_DocumentsUI.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/apps/DocumentsUI/ .
fi
