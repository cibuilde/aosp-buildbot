...
clone_depth_platform platform/packages/apps/Messaging
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_06/platform/packages/apps/Messaging/libgiftranscode^android_x86_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/packages/apps/Messaging/jni/libgiftranscode^android_x86_x86_64_shared
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_06/platform/packages/apps/Messaging/libgiftranscode^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/packages/apps/Messaging/jni/libgiftranscode^android_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_apps_Messaging.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/apps/Messaging/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_06 platform_packages_apps_Messaging.tar.zst --clobber

du -ah -d1 platform_packages_apps_Messaging*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_apps_Messaging.tar.zst" ]; then
  echo "Compressing platform/packages/apps/Messaging -> platform_packages_apps_Messaging.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_apps_Messaging.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/apps/Messaging/ .
fi
