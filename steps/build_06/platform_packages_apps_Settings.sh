...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_06.ninja statslog-settings-java-gen,
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/apps/Settings/statslog-settings-java-gen^
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_06/platform/packages/apps/Settings/statslog-settings-java-gen^.output . $GITHUB_WORKSPACE/artifacts/packages/apps/Settings/statslog-settings-java-gen^
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_06 platform_packages_apps_Settings.tar.zst --clobber
