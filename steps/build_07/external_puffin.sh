...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_07.ninja libpuffpatch,android_recovery_x86_64_static
mkdir -p $GITHUB_WORKSPACE/artifacts/external/puffin/libpuffpatch^android_recovery_x86_64_static
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_07/external/puffin/libpuffpatch^android_recovery_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/external/puffin/libpuffpatch^android_recovery_x86_64_static

echo "building libpuffpatch^android_x86_64_static"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_07.ninja libpuffpatch,android_x86_64_static
mkdir -p $GITHUB_WORKSPACE/artifacts/external/puffin/libpuffpatch^android_x86_64_static
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_07/external/puffin/libpuffpatch^android_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/external/puffin/libpuffpatch^android_x86_64_static

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_puffin.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/puffin/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_07 external_puffin.tar.zst --clobber
