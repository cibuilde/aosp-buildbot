...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_07.ninja libslab,android_x86_64_dylib
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/slab/libslab^android_x86_64_dylib
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_07/external/rust/crates/slab/libslab^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/slab/libslab^android_x86_64_dylib
python3 $GITHUB_WORKSPACE/copy_symlink.py $GITHUB_WORKSPACE/steps/outputs_07/external/rust/crates/slab/libslab^android_x86_64_dylib.output $GITHUB_WORKSPACE/artifacts/external/rust/crates/slab/libslab^android_x86_64_dylib $GITHUB_WORKSPACE/artifacts/external/rust/crates/slab/libslab^android_x86_64_dylib/addition_copy_files.output

echo "building libslab^android_x86_64_dylib_apex10000"
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_07.ninja libslab,android_x86_64_dylib_apex10000
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/slab/libslab^android_x86_64_dylib_apex10000
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_07/external/rust/crates/slab/libslab^android_x86_64_dylib_apex10000.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/slab/libslab^android_x86_64_dylib_apex10000
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_07 external_rust_crates_slab.tar.zst --clobber
