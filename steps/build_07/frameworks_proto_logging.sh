...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_07.ninja statslog.cpp,
mkdir -p $GITHUB_WORKSPACE/artifacts/frameworks/proto_logging/stats/stats_log_api_gen/statslog.cpp^
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_07/frameworks/proto_logging/statslog.cpp^.output . $GITHUB_WORKSPACE/artifacts/frameworks/proto_logging/stats/stats_log_api_gen/statslog.cpp^

echo "building statslog.h^"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_07.ninja statslog.h,
mkdir -p $GITHUB_WORKSPACE/artifacts/frameworks/proto_logging/stats/stats_log_api_gen/statslog.h^
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_07/frameworks/proto_logging/statslog.h^.output . $GITHUB_WORKSPACE/artifacts/frameworks/proto_logging/stats/stats_log_api_gen/statslog.h^
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_07 frameworks_proto_logging.tar.zst --clobber
