...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_07.ninja statslog-secure-element-java-gen,
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/apps/SecureElement/statslog-secure-element-java-gen^
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_07/packages/apps/SecureElement/statslog-secure-element-java-gen^.output . $GITHUB_WORKSPACE/artifacts/packages/apps/SecureElement/statslog-secure-element-java-gen^
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_07 packages_apps_SecureElement.tar.zst --clobber
