...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_07/platform/external/avb/avbctl^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/avb/avbctl^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_avb.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/avb/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_07 platform_external_avb.tar.zst --clobber
