...
clone_depth_platform platform/external/curl
...
rsync -a -r $GITHUB_WORKSPACE/downloads/external/curl/libcurl^android_vendor.31_x86_64_static/ .
rsync -a -r $GITHUB_WORKSPACE/downloads/external/curl/libcurl^android_vendor.31_x86_x86_64_static/ .
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_07.ninja libcurl,android_vendor.31_x86_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/external/curl/libcurl^android_vendor.31_x86_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_07/platform/external/curl/libcurl^android_vendor.31_x86_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/curl/libcurl^android_vendor.31_x86_x86_64_shared

echo "building libcurl^android_vendor.31_x86_64_shared"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_07.ninja libcurl,android_vendor.31_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/external/curl/libcurl^android_vendor.31_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_07/platform/external/curl/libcurl^android_vendor.31_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/curl/libcurl^android_vendor.31_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_curl.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/curl/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_07 platform_external_curl.tar.zst --clobber

du -ah -d1 platform_external_curl*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_curl.tar.zst" ]; then
  echo "Compressing platform/external/curl -> platform_external_curl.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_curl.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/curl/ .
fi
