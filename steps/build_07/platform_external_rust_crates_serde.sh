...
clone_depth_platform platform/external/rust/crates/serde
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_07.ninja libserde,android_x86_64_dylib_apex10000
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/serde/libserde^android_x86_64_dylib_apex10000
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_07/platform/external/rust/crates/serde/libserde^android_x86_64_dylib_apex10000.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/serde/libserde^android_x86_64_dylib_apex10000

echo "building libserde^android_x86_64_dylib"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_07.ninja libserde,android_x86_64_dylib
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/serde/libserde^android_x86_64_dylib
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_07/platform/external/rust/crates/serde/libserde^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/serde/libserde^android_x86_64_dylib

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_rust_crates_serde.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/rust/crates/serde/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_07 platform_external_rust_crates_serde.tar.zst --clobber

du -ah -d1 platform_external_rust_crates_serde*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_rust_crates_serde.tar.zst" ]; then
  echo "Compressing platform/external/rust/crates/serde -> platform_external_rust_crates_serde.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_rust_crates_serde.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/rust/crates/serde/ .
fi
