...
clone_depth_platform platform/external/turbine
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_07.ninja turbine,linux_glibc_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/turbine/turbine^linux_glibc_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_07/platform/external/turbine/turbine^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/turbine/turbine^linux_glibc_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_turbine.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/turbine/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_07 platform_external_turbine.tar.zst --clobber

du -ah -d1 platform_external_turbine*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_turbine.tar.zst" ]; then
  echo "Compressing platform/external/turbine -> platform_external_turbine.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_turbine.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/turbine/ .
fi
