...
clone_depth_platform packages/modules/ExtServices
clone_depth_platform packages/modules/NeuralNetworks
clone_depth_platform platform/packages/modules/ExtServices
...
rsync -a -r $GITHUB_WORKSPACE/downloads/external/fft2d/libfft2d^android_x86_64_sdk_static_apex30/ .
...
rsync -a -r $GITHUB_WORKSPACE/downloads/packages/modules/ExtServices/native/libextservices^android_x86_64_sdk_static_apex30/ .
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_07.ninja libextservices_jni,android_x86_64_sdk_shared_apex30
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/modules/ExtServices/jni/libextservices_jni^android_x86_64_sdk_shared_apex30
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_07/platform/packages/modules/ExtServices/libextservices_jni^android_x86_64_sdk_shared_apex30.output . $GITHUB_WORKSPACE/artifacts/packages/modules/ExtServices/jni/libextservices_jni^android_x86_64_sdk_shared_apex30

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_modules_ExtServices.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/modules/ExtServices/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_07 platform_packages_modules_ExtServices.tar.zst --clobber

du -ah -d1 platform_packages_modules_ExtServices*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/packages_modules_ExtServices.tar.zst" ]; then
  echo "Compressing packages/modules/ExtServices -> packages_modules_ExtServices.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/packages_modules_ExtServices.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/packages/modules/ExtServices/ .
fi
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_modules_ExtServices.tar.zst" ]; then
  echo "Compressing platform/packages/modules/ExtServices -> platform_packages_modules_ExtServices.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_modules_ExtServices.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/modules/ExtServices/ .
fi
