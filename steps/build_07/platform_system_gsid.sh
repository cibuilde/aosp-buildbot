...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_07.ninja libgsi,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/system/gsid/libgsi^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_07/platform/system/gsid/libgsi^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/gsid/libgsi^android_x86_64_shared

echo "building libgsi^android_recovery_x86_64_shared"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_07.ninja libgsi,android_recovery_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/system/gsid/libgsi^android_recovery_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_07/platform/system/gsid/libgsi^android_recovery_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/gsid/libgsi^android_recovery_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_system_gsid.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/system/gsid/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_07 platform_system_gsid.tar.zst --clobber
