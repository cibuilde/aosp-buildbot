...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_07.ninja auditctl,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/logging/logd/auditctl^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_07/platform/system/logging/auditctl^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/logging/logd/auditctl^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_system_logging.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/system/logging/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_07 platform_system_logging.tar.zst --clobber
