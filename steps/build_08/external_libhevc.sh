...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja libhevcdec,android_vendor.31_x86_x86_64_static_cfi
mkdir -p $GITHUB_WORKSPACE/artifacts/external/libhevc/libhevcdec^android_vendor.31_x86_x86_64_static_cfi
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/external/libhevc/libhevcdec^android_vendor.31_x86_x86_64_static_cfi.output . $GITHUB_WORKSPACE/artifacts/external/libhevc/libhevcdec^android_vendor.31_x86_x86_64_static_cfi

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_libhevc.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/libhevc/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_08 external_libhevc.tar.zst --clobber
