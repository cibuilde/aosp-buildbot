...
rsync -a -r $GITHUB_WORKSPACE/artifacts/bionic/libc/crtbegin_dynamic^android_vendor.31_x86_64/ .
rsync -a -r $GITHUB_WORKSPACE/artifacts/bionic/libc/crtbegin_dynamic^android_x86_64/ .
rsync -a -r $GITHUB_WORKSPACE/artifacts/bionic/libc/crtend_android^android_vendor.31_x86_64/ .
...
rsync -a -r $GITHUB_WORKSPACE/artifacts/external/one-true-awk/awkgram.tab.c^/ .
rsync -a -r $GITHUB_WORKSPACE/artifacts/external/one-true-awk/awkgram.tab.h^/ .
rsync -a -r $GITHUB_WORKSPACE/artifacts/external/one-true-awk/proctab.c^/ .

echo "building awk^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja awk,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/external/one-true-awk/awk^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/external/one-true-awk/awk^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/one-true-awk/awk^android_x86_64

echo "building awk_vendor^android_vendor.31_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja awk_vendor,android_vendor.31_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/external/one-true-awk/awk_vendor^android_vendor.31_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/external/one-true-awk/awk_vendor^android_vendor.31_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/one-true-awk/awk_vendor^android_vendor.31_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_one-true-awk.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/one-true-awk/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_08 external_one-true-awk.tar.zst --clobber
