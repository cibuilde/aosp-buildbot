...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja libgetrandom,android_x86_64_dylib
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/getrandom/libgetrandom^android_x86_64_dylib
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/external/rust/crates/getrandom/libgetrandom^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/getrandom/libgetrandom^android_x86_64_dylib
python3 $GITHUB_WORKSPACE/copy_symlink.py $GITHUB_WORKSPACE/steps/outputs_08/external/rust/crates/getrandom/libgetrandom^android_x86_64_dylib.output $GITHUB_WORKSPACE/artifacts/external/rust/crates/getrandom/libgetrandom^android_x86_64_dylib $GITHUB_WORKSPACE/artifacts/external/rust/crates/getrandom/libgetrandom^android_x86_64_dylib/addition_copy_files.output
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_08 external_rust_crates_getrandom.tar.zst --clobber
