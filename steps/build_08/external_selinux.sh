...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja libselinux_bindgen,android_x86_64_source
mkdir -p $GITHUB_WORKSPACE/artifacts/external/selinux/libselinux/libselinux_bindgen^android_x86_64_source
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/external/selinux/libselinux_bindgen^android_x86_64_source.output . $GITHUB_WORKSPACE/artifacts/external/selinux/libselinux/libselinux_bindgen^android_x86_64_source

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_selinux.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/selinux/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_08 external_selinux.tar.zst --clobber
