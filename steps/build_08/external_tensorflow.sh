...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja libtflite_kernel_utils,android_vendor.31_x86_64_static
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/external/tensorflow/libtflite_kernel_utils^android_vendor.31_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/external/tensorflow/tensorflow/lite/kernels/libtflite_kernel_utils^android_vendor.31_x86_64_static

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_tensorflow.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/tensorflow/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_08 external_tensorflow.tar.zst --clobber
