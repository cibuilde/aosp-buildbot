...
clone_depth_platform platform/external/android-clat
...
echo "building clatd^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja clatd,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/external/android-clat/clatd^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/platform/external/android-clat/clatd^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/android-clat/clatd^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_android-clat.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/android-clat/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_08 platform_external_android-clat.tar.zst --clobber

du -ah -d1 platform_external_android-clat*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_android-clat.tar.zst" ]; then
  echo "Compressing platform/external/android-clat -> platform_external_android-clat.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_android-clat.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/android-clat/ .
fi
