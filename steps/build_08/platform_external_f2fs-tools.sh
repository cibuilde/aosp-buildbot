...
clone_depth_platform platform/external/f2fs-tools
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja sload_f2fs,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/external/f2fs-tools/sload_f2fs^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/platform/external/f2fs-tools/sload_f2fs^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/f2fs-tools/sload_f2fs^android_x86_64

echo "building sload_f2fs^android_recovery_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja sload_f2fs,android_recovery_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/external/f2fs-tools/sload_f2fs^android_recovery_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/platform/external/f2fs-tools/sload_f2fs^android_recovery_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/f2fs-tools/sload_f2fs^android_recovery_x86_64

echo "building make_f2fs^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja make_f2fs,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/external/f2fs-tools/make_f2fs^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/platform/external/f2fs-tools/make_f2fs^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/f2fs-tools/make_f2fs^android_x86_64

echo "building make_f2fs^android_recovery_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja make_f2fs,android_recovery_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/external/f2fs-tools/make_f2fs^android_recovery_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/platform/external/f2fs-tools/make_f2fs^android_recovery_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/f2fs-tools/make_f2fs^android_recovery_x86_64

echo "building fsck.f2fs^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja fsck.f2fs,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/external/f2fs-tools/fsck.f2fs^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/platform/external/f2fs-tools/fsck.f2fs^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/f2fs-tools/fsck.f2fs^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_f2fs-tools.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/f2fs-tools/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_08 platform_external_f2fs-tools.tar.zst --clobber

du -ah -d1 platform_external_f2fs-tools*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_f2fs-tools.tar.zst" ]; then
  echo "Compressing platform/external/f2fs-tools -> platform_external_f2fs-tools.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_f2fs-tools.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/f2fs-tools/ .
fi
