...
clone_depth_platform platform/external/mtpd
...
echo "building mtpd^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja mtpd,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/external/mtpd/mtpd^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/platform/external/mtpd/mtpd^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/mtpd/mtpd^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_mtpd.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/mtpd/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_08 platform_external_mtpd.tar.zst --clobber

du -ah -d1 platform_external_mtpd*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_mtpd.tar.zst" ]; then
  echo "Compressing platform/external/mtpd -> platform_external_mtpd.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_mtpd.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/mtpd/ .
fi
