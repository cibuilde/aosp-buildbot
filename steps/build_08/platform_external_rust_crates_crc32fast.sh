...
clone_depth_platform platform/external/rust/crates/crc32fast
...
echo "building libcrc32fast^android_x86_64_dylib"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja libcrc32fast,android_x86_64_dylib
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/crc32fast/libcrc32fast^android_x86_64_dylib
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/platform/external/rust/crates/crc32fast/libcrc32fast^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/crc32fast/libcrc32fast^android_x86_64_dylib

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_rust_crates_crc32fast.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/rust/crates/crc32fast/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_08 platform_external_rust_crates_crc32fast.tar.zst --clobber

du -ah -d1 platform_external_rust_crates_crc32fast*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_rust_crates_crc32fast.tar.zst" ]; then
  echo "Compressing platform/external/rust/crates/crc32fast -> platform_external_rust_crates_crc32fast.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_rust_crates_crc32fast.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/rust/crates/crc32fast/ .
fi
