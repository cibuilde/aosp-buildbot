...
clone_depth_platform platform/external/rust/crates/macaddr
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja libmacaddr,android_x86_64_dylib
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/macaddr/libmacaddr^android_x86_64_dylib
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/platform/external/rust/crates/macaddr/libmacaddr^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/macaddr/libmacaddr^android_x86_64_dylib

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_rust_crates_macaddr.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/rust/crates/macaddr/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_08 platform_external_rust_crates_macaddr.tar.zst --clobber

du -ah -d1 platform_external_rust_crates_macaddr*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_rust_crates_macaddr.tar.zst" ]; then
  echo "Compressing platform/external/rust/crates/macaddr -> platform_external_rust_crates_macaddr.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_rust_crates_macaddr.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/rust/crates/macaddr/ .
fi
