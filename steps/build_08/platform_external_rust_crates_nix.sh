...
clone_depth_platform platform/external/rust/crates/nix
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja libnix,android_x86_64_dylib_apex10000
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/nix/libnix^android_x86_64_dylib_apex10000
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/platform/external/rust/crates/nix/libnix^android_x86_64_dylib_apex10000.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/nix/libnix^android_x86_64_dylib_apex10000

echo "building libnix^android_x86_64_dylib"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja libnix,android_x86_64_dylib
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/nix/libnix^android_x86_64_dylib
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/platform/external/rust/crates/nix/libnix^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/nix/libnix^android_x86_64_dylib

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_rust_crates_nix.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/rust/crates/nix/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_08 platform_external_rust_crates_nix.tar.zst --clobber

du -ah -d1 platform_external_rust_crates_nix*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_rust_crates_nix.tar.zst" ]; then
  echo "Compressing platform/external/rust/crates/nix -> platform_external_rust_crates_nix.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_rust_crates_nix.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/rust/crates/nix/ .
fi
