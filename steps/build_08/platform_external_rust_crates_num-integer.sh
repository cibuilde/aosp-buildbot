...
clone_depth_platform platform/external/rust/crates/num-integer
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/platform/external/rust/crates/num-integer/libnum_integer^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/num-integer/libnum_integer^android_x86_64_dylib

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_rust_crates_num-integer.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/rust/crates/num-integer/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_08 platform_external_rust_crates_num-integer.tar.zst --clobber

du -ah -d1 platform_external_rust_crates_num-integer*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_rust_crates_num-integer.tar.zst" ]; then
  echo "Compressing platform/external/rust/crates/num-integer -> platform_external_rust_crates_num-integer.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_rust_crates_num-integer.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/rust/crates/num-integer/ .
fi
