...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja libcxx,android_x86_64_rlib_dylib-std
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/cxx/libcxx^android_x86_64_rlib_dylib-std
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/platform/external/rust/cxx/libcxx^android_x86_64_rlib_dylib-std.output . $GITHUB_WORKSPACE/artifacts/external/rust/cxx/libcxx^android_x86_64_rlib_dylib-std
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_08 platform_external_rust_cxx.tar.zst --clobber
