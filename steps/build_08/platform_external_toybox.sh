...
clone_depth_platform platform/external/toybox
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/platform/external/toybox/toybox_vendor^android_vendor.31_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/toybox/toybox_vendor^android_vendor.31_x86_64
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/platform/external/toybox/toybox^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/toybox/toybox^android_x86_64
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/platform/external/toybox/toybox^android_recovery_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/toybox/toybox^android_recovery_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_toybox.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/toybox/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_08 platform_external_toybox.tar.zst --clobber

du -ah -d1 platform_external_toybox*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_toybox.tar.zst" ]; then
  echo "Compressing platform/external/toybox -> platform_external_toybox.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_toybox.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/toybox/ .
fi
