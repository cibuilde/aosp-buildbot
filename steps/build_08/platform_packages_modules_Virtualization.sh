...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja microdroid_ramdisk-5.10,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/modules/Virtualization/microdroid/microdroid_ramdisk-5.10^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/platform/packages/modules/Virtualization/microdroid_ramdisk-5.10^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/modules/Virtualization/microdroid/microdroid_ramdisk-5.10^android_common

echo "building microdroid_boot-5.10^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja microdroid_boot-5.10,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/modules/Virtualization/microdroid/microdroid_boot-5.10^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/platform/packages/modules/Virtualization/microdroid_boot-5.10^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/packages/modules/Virtualization/microdroid/microdroid_boot-5.10^android_x86_64
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_08 platform_packages_modules_Virtualization.tar.zst --clobber
