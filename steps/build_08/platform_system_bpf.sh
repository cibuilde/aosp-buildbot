...
clone_depth_platform platform/system/bpf
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja libbpf_android,android_x86_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/system/bpf/libbpf_android/libbpf_android^android_x86_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/platform/system/bpf/libbpf_android^android_x86_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/bpf/libbpf_android/libbpf_android^android_x86_x86_64_shared

echo "building libbpf_android^android_x86_64_shared"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja libbpf_android,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/system/bpf/libbpf_android/libbpf_android^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/platform/system/bpf/libbpf_android^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/bpf/libbpf_android/libbpf_android^android_x86_64_shared

echo "building bpfloader^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_08.ninja bpfloader,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/bpf/bpfloader/bpfloader^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_08/platform/system/bpf/bpfloader^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/bpf/bpfloader/bpfloader^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_system_bpf.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/system/bpf/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_08 platform_system_bpf.tar.zst --clobber

du -ah -d1 platform_system_bpf*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_system_bpf.tar.zst" ]; then
  echo "Compressing platform/system/bpf -> platform_system_bpf.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_system_bpf.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/system/bpf/ .
fi
