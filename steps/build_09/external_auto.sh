...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_09.ninja auto_service_plugin,linux_glibc_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/auto/service/auto_service_plugin^linux_glibc_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_09/external/auto/auto_service_plugin^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/auto/service/auto_service_plugin^linux_glibc_common

echo "building libauto_value_plugin^linux_glibc_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_09.ninja libauto_value_plugin,linux_glibc_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/auto/value/libauto_value_plugin^linux_glibc_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_09/external/auto/libauto_value_plugin^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/auto/value/libauto_value_plugin^linux_glibc_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_auto.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/auto/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_09 external_auto.tar.zst --clobber
