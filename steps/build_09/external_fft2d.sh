...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_09.ninja libfft2d,android_x86_64_sdk_static_apex30
mkdir -p $GITHUB_WORKSPACE/artifacts/external/fft2d/libfft2d^android_x86_64_sdk_static_apex30
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_09/external/fft2d/libfft2d^android_x86_64_sdk_static_apex30.output . $GITHUB_WORKSPACE/artifacts/external/fft2d/libfft2d^android_x86_64_sdk_static_apex30

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_fft2d.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/fft2d/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_09 external_fft2d.tar.zst --clobber
