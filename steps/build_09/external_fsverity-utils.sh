...
rsync -a -r $GITHUB_WORKSPACE/artifacts/external/fsverity-utils/libfsverity^android_x86_64_static/ .
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_09.ninja libfsverity,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/external/fsverity-utils/libfsverity^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_09/external/fsverity-utils/libfsverity^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/fsverity-utils/libfsverity^android_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_fsverity-utils.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/fsverity-utils/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_09 external_fsverity-utils.tar.zst --clobber
