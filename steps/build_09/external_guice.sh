...
rsync -a -r $GITHUB_WORKSPACE/artifacts/external/guice/guice_munged_srcs^/ .
rsync -a -r $GITHUB_WORKSPACE/artifacts/external/jsr330/jsr330^linux_glibc_common/ .

echo "building guice^linux_glibc_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_09.ninja guice,linux_glibc_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/guice/guice^linux_glibc_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_09/external/guice/guice^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/guice/guice^linux_glibc_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_guice.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/guice/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_09 external_guice.tar.zst --clobber
