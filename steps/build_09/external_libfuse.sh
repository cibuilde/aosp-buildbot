...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_09.ninja libfuse,android_x86_64_sdk_static_apex30
mkdir -p $GITHUB_WORKSPACE/artifacts/external/libfuse/libfuse^android_x86_64_sdk_static_apex30
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_09/external/libfuse/libfuse^android_x86_64_sdk_static_apex30.output . $GITHUB_WORKSPACE/artifacts/external/libfuse/libfuse^android_x86_64_sdk_static_apex30

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_libfuse.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/libfuse/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_09 external_libfuse.tar.zst --clobber
