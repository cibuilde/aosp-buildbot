...
rsync -a -r $GITHUB_WORKSPACE/artifacts/external/libopus/libopus^android_x86_64_static_apex29/ .

echo "building libopus^android_x86_64_shared_apex29"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_09.ninja libopus,android_x86_64_shared_apex29
mkdir -p $GITHUB_WORKSPACE/artifacts/external/libopus/libopus^android_x86_64_shared_apex29
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_09/external/libopus/libopus^android_x86_64_shared_apex29.output . $GITHUB_WORKSPACE/artifacts/external/libopus/libopus^android_x86_64_shared_apex29

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_libopus.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/libopus/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_09 external_libopus.tar.zst --clobber
