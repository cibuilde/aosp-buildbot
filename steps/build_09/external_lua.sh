...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_09/external/lua/liblua^android_x86_64_sdk_static_apex30.output . $GITHUB_WORKSPACE/artifacts/external/lua/liblua^android_x86_64_sdk_static_apex30

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_lua.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/lua/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_09 external_lua.tar.zst --clobber
