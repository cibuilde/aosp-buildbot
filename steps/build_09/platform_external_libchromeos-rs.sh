...
clone_depth_platform platform/external/libchromeos-rs
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_09.ninja liblibchromeos,android_x86_64_rlib_rlib-std_apex10000
mkdir -p $GITHUB_WORKSPACE/artifacts/external/libchromeos-rs/liblibchromeos^android_x86_64_rlib_rlib-std_apex10000
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_09/platform/external/libchromeos-rs/liblibchromeos^android_x86_64_rlib_rlib-std_apex10000.output . $GITHUB_WORKSPACE/artifacts/external/libchromeos-rs/liblibchromeos^android_x86_64_rlib_rlib-std_apex10000

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_libchromeos-rs.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/libchromeos-rs/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_09 platform_external_libchromeos-rs.tar.zst --clobber

du -ah -d1 platform_external_libchromeos-rs*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_libchromeos-rs.tar.zst" ]; then
  echo "Compressing platform/external/libchromeos-rs -> platform_external_libchromeos-rs.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_libchromeos-rs.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/libchromeos-rs/ .
fi
