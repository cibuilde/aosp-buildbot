...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_09.ninja libenv_logger,android_x86_64_dylib_apex10000
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/env_logger/libenv_logger^android_x86_64_dylib_apex10000
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_09/platform/external/rust/crates/env_logger/libenv_logger^android_x86_64_dylib_apex10000.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/env_logger/libenv_logger^android_x86_64_dylib_apex10000

echo "building libenv_logger^android_x86_64_dylib"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_09.ninja libenv_logger,android_x86_64_dylib
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/env_logger/libenv_logger^android_x86_64_dylib
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_09/platform/external/rust/crates/env_logger/libenv_logger^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/env_logger/libenv_logger^android_x86_64_dylib
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_09 platform_external_rust_crates_env_logger.tar.zst --clobber
