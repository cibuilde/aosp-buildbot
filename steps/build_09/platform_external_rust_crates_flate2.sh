...
clone_depth_platform platform/external/rust/crates/flate2
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_09.ninja libflate2,android_x86_64_dylib
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/flate2/libflate2^android_x86_64_dylib
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_09/platform/external/rust/crates/flate2/libflate2^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/flate2/libflate2^android_x86_64_dylib

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_rust_crates_flate2.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/rust/crates/flate2/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_09 platform_external_rust_crates_flate2.tar.zst --clobber

du -ah -d1 platform_external_rust_crates_flate2*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_rust_crates_flate2.tar.zst" ]; then
  echo "Compressing platform/external/rust/crates/flate2 -> platform_external_rust_crates_flate2.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_rust_crates_flate2.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/rust/crates/flate2/ .
fi
