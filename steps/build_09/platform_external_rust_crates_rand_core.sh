...
clone_depth_platform platform/external/rust/crates/rand_core
...
echo "building librand_core^android_x86_64_dylib"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_09.ninja librand_core,android_x86_64_dylib
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/rand_core/librand_core^android_x86_64_dylib
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_09/platform/external/rust/crates/rand_core/librand_core^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/rand_core/librand_core^android_x86_64_dylib

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_rust_crates_rand_core.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/rust/crates/rand_core/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_09 platform_external_rust_crates_rand_core.tar.zst --clobber

du -ah -d1 platform_external_rust_crates_rand_core*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_rust_crates_rand_core.tar.zst" ]; then
  echo "Compressing platform/external/rust/crates/rand_core -> platform_external_rust_crates_rand_core.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_rust_crates_rand_core.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/rust/crates/rand_core/ .
fi
