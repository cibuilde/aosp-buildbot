...
clone_depth_platform platform/system/hwservicemanager
...
rsync -a -r $GITHUB_WORKSPACE/downloads/system/hwservicemanager/libhwservicemanager^android_x86_64_static/ .
rsync -a -r $GITHUB_WORKSPACE/downloads/system/hwservicemanager/libtokenmanager^android_x86_64_static/ .
...
echo "building hwservicemanager^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_09.ninja hwservicemanager,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/hwservicemanager/hwservicemanager^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_09/platform/system/hwservicemanager/hwservicemanager^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/hwservicemanager/hwservicemanager^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_system_hwservicemanager.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/system/hwservicemanager/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_09 platform_system_hwservicemanager.tar.zst --clobber

du -ah -d1 platform_system_hwservicemanager*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_system_hwservicemanager.tar.zst" ]; then
  echo "Compressing platform/system/hwservicemanager -> platform_system_hwservicemanager.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_system_hwservicemanager.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/system/hwservicemanager/ .
fi
