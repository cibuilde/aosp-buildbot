...
rsync -a -r $GITHUB_WORKSPACE/artifacts/system/memory/libion/libion^android_x86_64_static_apex29/ .

echo "building libion^android_x86_64_shared_apex29"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_09.ninja libion,android_x86_64_shared_apex29
mkdir -p $GITHUB_WORKSPACE/artifacts/system/memory/libion/libion^android_x86_64_shared_apex29
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_09/system/memory/libion/libion^android_x86_64_shared_apex29.output . $GITHUB_WORKSPACE/artifacts/system/memory/libion/libion^android_x86_64_shared_apex29

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_memory_libion.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/memory/libion/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_09 system_memory_libion.tar.zst --clobber
