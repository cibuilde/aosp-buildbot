...
rsync -a -r $GITHUB_WORKSPACE/artifacts/system/tools/hidl/metadata/hidl_metadata_in_cpp^linux_glibc_x86_64/ .

echo "building libhidlmetadata^linux_glibc_x86_64_static"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_09.ninja libhidlmetadata,linux_glibc_x86_64_static
mkdir -p $GITHUB_WORKSPACE/artifacts/system/tools/hidl/metadata/libhidlmetadata^linux_glibc_x86_64_static
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_09/system/tools/hidl/libhidlmetadata^linux_glibc_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/system/tools/hidl/metadata/libhidlmetadata^linux_glibc_x86_64_static

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_tools_hidl.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/tools/hidl/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_09 system_tools_hidl.tar.zst --clobber
