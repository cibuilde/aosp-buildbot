...
rsync -a -r $GITHUB_WORKSPACE/artifacts/tools/dexter/slicer/slicer^android_x86_64_static/ .

echo "building slicer^android_x86_64_shared"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_09.ninja slicer,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/tools/dexter/slicer/slicer^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_09/tools/dexter/slicer^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/tools/dexter/slicer/slicer^android_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf tools_dexter.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/tools/dexter/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_09 tools_dexter.tar.zst --clobber
