...
rsync -a -r $GITHUB_WORKSPACE/artifacts/external/auto/android-annotation-stubs/auto_android_annotation_stubs^linux_glibc_common/ .
...
rsync -a -r $GITHUB_WORKSPACE/artifacts/external/auto/value/auto_value_extension^linux_glibc_common/ .
rsync -a -r $GITHUB_WORKSPACE/artifacts/external/auto/value/libauto_value_plugin^linux_glibc_common/ .
rsync -a -r $GITHUB_WORKSPACE/artifacts/external/error_prone/error_prone_annotations^linux_glibc_common/ .
rsync -a -r $GITHUB_WORKSPACE/artifacts/external/escapevelocity/escapevelocity^linux_glibc_common/ .
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_10.ninja auto_annotation_plugin,linux_glibc_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/auto/value/auto_annotation_plugin^linux_glibc_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_10/external/auto/auto_annotation_plugin^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/auto/value/auto_annotation_plugin^linux_glibc_common

echo "building auto_value_memoized_extension_plugin^linux_glibc_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_10.ninja auto_value_memoized_extension_plugin,linux_glibc_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/auto/value/auto_value_memoized_extension_plugin^linux_glibc_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_10/external/auto/auto_value_memoized_extension_plugin^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/auto/value/auto_value_memoized_extension_plugin^linux_glibc_common

echo "building auto_value_plugin^linux_glibc_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_10.ninja auto_value_plugin,linux_glibc_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/auto/value/auto_value_plugin^linux_glibc_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_10/external/auto/auto_value_plugin^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/auto/value/auto_value_plugin^linux_glibc_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_auto.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/auto/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_10 external_auto.tar.zst --clobber
