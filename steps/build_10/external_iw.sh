...
rsync -a -r $GITHUB_WORKSPACE/artifacts/external/iw/iw_version^/ .
...
rsync -a -r $GITHUB_WORKSPACE/artifacts/external/libnl/libnl^android_x86_64_shared/ .

echo "building iw^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_10.ninja iw,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/external/iw/iw^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_10/external/iw/iw^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/iw/iw^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_iw.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/iw/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_10 external_iw.tar.zst --clobber
