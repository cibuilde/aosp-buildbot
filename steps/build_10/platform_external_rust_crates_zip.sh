...
clone_depth_platform platform/external/rust/crates/zip
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_10/platform/external/rust/crates/zip/libzip^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/zip/libzip^android_x86_64_dylib

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_rust_crates_zip.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/rust/crates/zip/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_10 platform_external_rust_crates_zip.tar.zst --clobber

du -ah -d1 platform_external_rust_crates_zip*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_rust_crates_zip.tar.zst" ]; then
  echo "Compressing platform/external/rust/crates/zip -> platform_external_rust_crates_zip.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_rust_crates_zip.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/rust/crates/zip/ .
fi
