...
clone_depth_platform platform/external/vm_tools/p9
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_10.ninja libp9,android_x86_64_rlib_rlib-std_apex10000
mkdir -p $GITHUB_WORKSPACE/artifacts/external/vm_tools/p9/libp9^android_x86_64_rlib_rlib-std_apex10000
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_10/platform/external/vm_tools/p9/libp9^android_x86_64_rlib_rlib-std_apex10000.output . $GITHUB_WORKSPACE/artifacts/external/vm_tools/p9/libp9^android_x86_64_rlib_rlib-std_apex10000

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_vm_tools_p9.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/vm_tools/p9/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_10 platform_external_vm_tools_p9.tar.zst --clobber

du -ah -d1 platform_external_vm_tools_p9*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_vm_tools_p9.tar.zst" ]; then
  echo "Compressing platform/external/vm_tools/p9 -> platform_external_vm_tools_p9.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_vm_tools_p9.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/vm_tools/p9/ .
fi
