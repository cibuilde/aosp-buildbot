...
rsync -a -r $GITHUB_WORKSPACE/downloads/external/mdnsresponder/libmdnssd^android_x86_64_shared/ .
...
rsync -a -r $GITHUB_WORKSPACE/downloads/packages/modules/DnsResolver/libnetd_resolv^android_x86_64_shared_current/ .
...
rsync -a -r $GITHUB_WORKSPACE/downloads/system/core/libqtaguid/libqtaguid^android_x86_64_shared/ .
...
rsync -a -r $GITHUB_WORKSPACE/downloads/system/hardware/interfaces/net/netd/1.1/android.system.net.netd@1.1^android_x86_64_shared/ .
...
rsync -a -r $GITHUB_WORKSPACE/downloads/system/netd/server/libnetd_server^android_x86_64_static_cfi/ .
...
rsync -a -r $GITHUB_WORKSPACE/downloads/system/netd/server/netd_aidl_interface-V7-cpp^android_x86_64_shared/ .
...
rsync -a -r $GITHUB_WORKSPACE/downloads/system/netd/server/oemnetd_aidl_interface-cpp^android_x86_64_shared/ .

echo "building netd^android_x86_64_cfi"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_10.ninja netd,android_x86_64_cfi
mkdir -p $GITHUB_WORKSPACE/artifacts/system/netd/server/netd^android_x86_64_cfi
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_10/platform/system/netd/netd^android_x86_64_cfi.output . $GITHUB_WORKSPACE/artifacts/system/netd/server/netd^android_x86_64_cfi

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_system_netd.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/system/netd/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_10 platform_system_netd.tar.zst --clobber
