...
rsync -a -r $GITHUB_WORKSPACE/downloads/hardware/interfaces/nfc/1.2/android.hardware.nfc@1.2^android_x86_64_shared/ .
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_10/platform/system/nfc/libnfc-nci^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/nfc/src/libnfc-nci^android_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_system_nfc.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/system/nfc/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_10 platform_system_nfc.tar.zst --clobber
