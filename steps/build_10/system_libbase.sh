...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_10.ninja libbase_ndk,android_x86_64_sdk_static_apex30
mkdir -p $GITHUB_WORKSPACE/artifacts/system/libbase/libbase_ndk^android_x86_64_sdk_static_apex30
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_10/system/libbase/libbase_ndk^android_x86_64_sdk_static_apex30.output . $GITHUB_WORKSPACE/artifacts/system/libbase/libbase_ndk^android_x86_64_sdk_static_apex30

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_libbase.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/libbase/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_10 system_libbase.tar.zst --clobber
