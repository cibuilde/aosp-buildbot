...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_11.ninja libfuse,android_x86_64_sdk_shared_apex30
mkdir -p $GITHUB_WORKSPACE/artifacts/external/libfuse/libfuse^android_x86_64_sdk_shared_apex30
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_11/external/libfuse/libfuse^android_x86_64_sdk_shared_apex30.output . $GITHUB_WORKSPACE/artifacts/external/libfuse/libfuse^android_x86_64_sdk_shared_apex30

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_libfuse.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/libfuse/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_11 external_libfuse.tar.zst --clobber
