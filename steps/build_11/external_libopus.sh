...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_11.ninja libopus,android_vendor.31_x86_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/external/libopus/libopus^android_vendor.31_x86_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_11/external/libopus/libopus^android_vendor.31_x86_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/libopus/libopus^android_vendor.31_x86_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_libopus.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/libopus/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_11 external_libopus.tar.zst --clobber
