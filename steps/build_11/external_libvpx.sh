...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_11.ninja libvpx,android_vendor.31_x86_x86_64_shared_cfi
mkdir -p $GITHUB_WORKSPACE/artifacts/external/libvpx/libvpx^android_vendor.31_x86_x86_64_shared_cfi
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_11/external/libvpx/libvpx^android_vendor.31_x86_x86_64_shared_cfi.output . $GITHUB_WORKSPACE/artifacts/external/libvpx/libvpx^android_vendor.31_x86_x86_64_shared_cfi

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_libvpx.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/libvpx/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_11 external_libvpx.tar.zst --clobber
