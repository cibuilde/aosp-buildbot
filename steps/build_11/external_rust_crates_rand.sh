...
echo "Preparing for external/rust/crates/rand"
...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_11.ninja librand,android_x86_64_dylib
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/rand/librand^android_x86_64_dylib
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_11/external/rust/crates/rand/librand^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/rand/librand^android_x86_64_dylib
python3 $GITHUB_WORKSPACE/copy_symlink.py $GITHUB_WORKSPACE/steps/outputs_11/external/rust/crates/rand/librand^android_x86_64_dylib.output $GITHUB_WORKSPACE/artifacts/external/rust/crates/rand/librand^android_x86_64_dylib $GITHUB_WORKSPACE/artifacts/external/rust/crates/rand/librand^android_x86_64_dylib/addition_copy_files.output
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_11 external_rust_crates_rand.tar.zst --clobber
