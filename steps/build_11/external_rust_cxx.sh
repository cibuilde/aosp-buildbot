...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_11.ninja libcxx,android_x86_64_rlib_rlib-std
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/cxx/libcxx^android_x86_64_rlib_rlib-std
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_11/external/rust/cxx/libcxx^android_x86_64_rlib_rlib-std.output . $GITHUB_WORKSPACE/artifacts/external/rust/cxx/libcxx^android_x86_64_rlib_rlib-std

echo "building libcxx^android_x86_x86_64_rlib_rlib-std"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_11.ninja libcxx,android_x86_x86_64_rlib_rlib-std
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/cxx/libcxx^android_x86_x86_64_rlib_rlib-std
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_11/external/rust/cxx/libcxx^android_x86_x86_64_rlib_rlib-std.output . $GITHUB_WORKSPACE/artifacts/external/rust/cxx/libcxx^android_x86_x86_64_rlib_rlib-std

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_rust_cxx.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/rust/cxx/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_11 external_rust_cxx.tar.zst --clobber
