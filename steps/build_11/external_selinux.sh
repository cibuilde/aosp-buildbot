...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_11.ninja libselinux_bindgen,android_x86_64_rlib_rlib-std
mkdir -p $GITHUB_WORKSPACE/artifacts/external/selinux/libselinux/libselinux_bindgen^android_x86_64_rlib_rlib-std
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_11/external/selinux/libselinux_bindgen^android_x86_64_rlib_rlib-std.output . $GITHUB_WORKSPACE/artifacts/external/selinux/libselinux/libselinux_bindgen^android_x86_64_rlib_rlib-std

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_selinux.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/selinux/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_11 external_selinux.tar.zst --clobber
