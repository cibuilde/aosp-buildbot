...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_11.ninja contextualcards,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/apps/Settings/contextualcards^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_11/packages/apps/Settings/contextualcards^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/apps/Settings/contextualcards^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf packages_apps_Settings.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/packages/apps/Settings/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_11 packages_apps_Settings.tar.zst --clobber
