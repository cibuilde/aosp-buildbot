...
clone_depth_platform platform/system/connectivity/wificond
...
rsync -a -r $GITHUB_WORKSPACE/downloads/frameworks/opt/net/wifi/libwifi_system_iface/libwifi-system-iface^android_x86_64_shared/ .
...
rsync -a -r $GITHUB_WORKSPACE/downloads/system/connectivity/wificond/libwificond^android_x86_64_static/ .
...
rsync -a -r $GITHUB_WORKSPACE/downloads/system/hardware/interfaces/wifi/keystore/1.0/android.system.wifi.keystore@1.0^android_x86_64_shared/ .
...
rsync -a -r $GITHUB_WORKSPACE/downloads/system/hardware/interfaces/wifi/keystore/1.0/default/libwifikeystorehal^android_x86_64_static/ .
...
rsync -a -r $GITHUB_WORKSPACE/downloads/system/security/keystore2/aidl/android.security.legacykeystore-ndk_platform^android_x86_64_shared/ .

echo "building wificond^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_11.ninja wificond,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/connectivity/wificond/wificond^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_11/platform/system/connectivity/wificond/wificond^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/connectivity/wificond/wificond^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_system_connectivity_wificond.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/system/connectivity/wificond/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_11 platform_system_connectivity_wificond.tar.zst --clobber

du -ah -d1 platform_system_connectivity_wificond*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_system_connectivity_wificond.tar.zst" ]; then
  echo "Compressing platform/system/connectivity/wificond -> platform_system_connectivity_wificond.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_system_connectivity_wificond.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/system/connectivity/wificond/ .
fi
