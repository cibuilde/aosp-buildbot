...
rsync -a -r $GITHUB_WORKSPACE/downloads/hardware/interfaces/gatekeeper/1.0/android.hardware.gatekeeper@1.0^android_x86_64_shared/ .
...
rsync -a -r $GITHUB_WORKSPACE/downloads/system/core/gatekeeperd/libgatekeeper_aidl^android_x86_64_shared/ .
...
rsync -a -r $GITHUB_WORKSPACE/downloads/system/gatekeeper/libgatekeeper^android_x86_64_shared/ .
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_11/platform/system/core/remount^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/core/fs_mgr/remount^android_x86_64
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_11/platform/system/core/gatekeeperd^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/core/gatekeeperd/gatekeeperd^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_system_core.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/system/core/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_11 platform_system_core.tar.zst --clobber
