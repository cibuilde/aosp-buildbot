...
rsync -a -r $GITHUB_WORKSPACE/downloads/packages/modules/DnsResolver/dnsresolver_aidl_interface-V7-cpp-source^/ .
rsync -a -r $GITHUB_WORKSPACE/downloads/packages/modules/DnsResolver/dnsresolver_aidl_interface-V7-cpp^android_x86_64_shared/ .
...
rsync -a -r $GITHUB_WORKSPACE/downloads/system/netd/server/netd_aidl_interface-V6-cpp^android_x86_64_shared/ .
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_11/platform/system/netd/ndc^android_x86_64_cfi.output . $GITHUB_WORKSPACE/artifacts/system/netd/server/ndc^android_x86_64_cfi

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_system_netd.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/system/netd/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_11 platform_system_netd.tar.zst --clobber
