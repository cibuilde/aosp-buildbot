...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_11.ninja libkeymaster_messages,android_vendor.31_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/system/keymaster/libkeymaster_messages^android_vendor.31_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_11/system/keymaster/libkeymaster_messages^android_vendor.31_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/keymaster/libkeymaster_messages^android_vendor.31_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_keymaster.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/keymaster/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_11 system_keymaster.tar.zst --clobber
