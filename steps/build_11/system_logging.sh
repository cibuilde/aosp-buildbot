...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_11.ninja liblog,android_vendor_ramdisk_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/system/logging/liblog/liblog^android_vendor_ramdisk_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_11/system/logging/liblog^android_vendor_ramdisk_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/logging/liblog/liblog^android_vendor_ramdisk_x86_64_shared

echo "building liblog_event_list_bindgen^android_x86_64_rlib_rlib-std"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_11.ninja liblog_event_list_bindgen,android_x86_64_rlib_rlib-std
mkdir -p $GITHUB_WORKSPACE/artifacts/system/logging/liblog/liblog_event_list_bindgen^android_x86_64_rlib_rlib-std
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_11/system/logging/liblog_event_list_bindgen^android_x86_64_rlib_rlib-std.output . $GITHUB_WORKSPACE/artifacts/system/logging/liblog/liblog_event_list_bindgen^android_x86_64_rlib_rlib-std

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_logging.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/logging/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_11 system_logging.tar.zst --clobber
