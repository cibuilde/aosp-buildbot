...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_12.ninja libcurl,android_vendor.31_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/external/curl/libcurl^android_vendor.31_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_12/external/curl/libcurl^android_vendor.31_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/curl/libcurl^android_vendor.31_x86_64_shared

echo "building libcurl^android_vendor.31_x86_x86_64_shared"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_12.ninja libcurl,android_vendor.31_x86_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/external/curl/libcurl^android_vendor.31_x86_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_12/external/curl/libcurl^android_vendor.31_x86_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/curl/libcurl^android_vendor.31_x86_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_curl.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/curl/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_12 external_curl.tar.zst --clobber
