...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_12.ninja dagger2-compiler-lib,linux_glibc_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/dagger2/dagger2-compiler-lib^linux_glibc_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_12/external/dagger2/dagger2-compiler-lib^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/external/dagger2/dagger2-compiler-lib^linux_glibc_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_dagger2.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/dagger2/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_12 external_dagger2.tar.zst --clobber
