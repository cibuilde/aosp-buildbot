...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_12.ninja libcppbor_external,android_vendor.31_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/external/libcppbor/libcppbor_external^android_vendor.31_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_12/external/libcppbor/libcppbor_external^android_vendor.31_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/libcppbor/libcppbor_external^android_vendor.31_x86_64_shared

echo "building libcppbor_external^android_x86_64_shared"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_12.ninja libcppbor_external,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/external/libcppbor/libcppbor_external^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_12/external/libcppbor/libcppbor_external^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/libcppbor/libcppbor_external^android_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_libcppbor.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/libcppbor/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_12 external_libcppbor.tar.zst --clobber
