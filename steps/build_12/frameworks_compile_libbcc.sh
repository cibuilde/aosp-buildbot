...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_12.ninja libbcc,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/frameworks/compile/libbcc/lib/libbcc^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_12/frameworks/compile/libbcc/libbcc^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/frameworks/compile/libbcc/lib/libbcc^android_x86_64_shared
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_12 frameworks_compile_libbcc.tar.zst --clobber
