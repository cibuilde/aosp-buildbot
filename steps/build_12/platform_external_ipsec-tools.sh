...
rsync -a -r $GITHUB_WORKSPACE/downloads/external/ipsec-tools/libipsec^android_x86_64_static/ .
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_12.ninja racoon,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/external/ipsec-tools/racoon^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_12/platform/external/ipsec-tools/racoon^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/ipsec-tools/racoon^android_x86_64
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_12 platform_external_ipsec-tools.tar.zst --clobber
