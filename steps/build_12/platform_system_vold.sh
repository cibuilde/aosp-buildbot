...
clone_depth_platform platform/system/vold
...
rsync -a -r $GITHUB_WORKSPACE/downloads/hardware/interfaces/health/storage/aidl/android.hardware.health.storage-V1-ndk_platform^android_x86_64_shared/ .
rsync -a -r $GITHUB_WORKSPACE/downloads/hardware/interfaces/health/storage/1.0/android.hardware.health.storage@1.0^android_x86_64_shared/ .
...
rsync -a -r $GITHUB_WORKSPACE/downloads/system/core/libdiskconfig/libdiskconfig^android_x86_64_shared/ .
...
rsync -a -r $GITHUB_WORKSPACE/downloads/system/extras/f2fs_utils/libf2fs_sparseblock^android_x86_64_shared/ .
...
rsync -a -r $GITHUB_WORKSPACE/downloads/system/security/keystore2/aidl/android.security.maintenance-ndk_platform^android_x86_64_shared/ .
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_12/platform/system/vold/wait_for_keymaster^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/vold/wait_for_keymaster^android_x86_64
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_12/platform/system/vold/vold^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/vold/vold^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_system_vold.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/system/vold/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_12 platform_system_vold.tar.zst --clobber

du -ah -d1 platform_system_vold*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_system_vold.tar.zst" ]; then
  echo "Compressing platform/system/vold -> platform_system_vold.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_system_vold.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/system/vold/ .
fi
