...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_12.ninja libbase,android_vendor_ramdisk_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/system/libbase/libbase^android_vendor_ramdisk_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_12/system/libbase/libbase^android_vendor_ramdisk_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/libbase/libbase^android_vendor_ramdisk_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_libbase.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/libbase/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_12 system_libbase.tar.zst --clobber
