...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_12.ninja libfmq,android_x86_64_shared_apex29
mkdir -p $GITHUB_WORKSPACE/artifacts/system/libfmq/libfmq^android_x86_64_shared_apex29
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_12/system/libfmq/libfmq^android_x86_64_shared_apex29.output . $GITHUB_WORKSPACE/artifacts/system/libfmq/libfmq^android_x86_64_shared_apex29

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_libfmq.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/libfmq/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_12 system_libfmq.tar.zst --clobber
