...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_12.ninja libhidlbase,android_x86_64_shared_apex29
mkdir -p $GITHUB_WORKSPACE/artifacts/system/libhidl/libhidlbase^android_x86_64_shared_apex29
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_12/system/libhidl/libhidlbase^android_x86_64_shared_apex29.output . $GITHUB_WORKSPACE/artifacts/system/libhidl/libhidlbase^android_x86_64_shared_apex29

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_libhidl.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/libhidl/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_12 system_libhidl.tar.zst --clobber
