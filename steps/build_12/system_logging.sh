...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_12.ninja auditctl,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/logging/logd/auditctl^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_12/system/logging/auditctl^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/logging/logd/auditctl^android_x86_64

echo "building liblog_event_list^android_x86_64_rlib_rlib-std"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_12.ninja liblog_event_list,android_x86_64_rlib_rlib-std
mkdir -p $GITHUB_WORKSPACE/artifacts/system/logging/rust/liblog_event_list^android_x86_64_rlib_rlib-std
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_12/system/logging/liblog_event_list^android_x86_64_rlib_rlib-std.output . $GITHUB_WORKSPACE/artifacts/system/logging/rust/liblog_event_list^android_x86_64_rlib_rlib-std

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_logging.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/logging/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_12 system_logging.tar.zst --clobber
