...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_12.ninja class2nonsdklist,linux_glibc_common
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_12/tools/platform-compat/class2nonsdklist^linux_glibc_common.output . $GITHUB_WORKSPACE/artifacts/tools/platform-compat/java/com/android/class2nonsdklist/class2nonsdklist^linux_glibc_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf tools_platform-compat.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/tools/platform-compat/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_12 tools_platform-compat.tar.zst --clobber
