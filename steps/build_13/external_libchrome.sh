...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_13.ninja libchrome,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/external/libchrome/libchrome^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_13/external/libchrome/libchrome^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/libchrome/libchrome^android_x86_64_shared

echo "building libchrome^android_x86_x86_64_shared"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_13.ninja libchrome,android_x86_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/external/libchrome/libchrome^android_x86_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_13/external/libchrome/libchrome^android_x86_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/libchrome/libchrome^android_x86_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_libchrome.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/libchrome/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_13 external_libchrome.tar.zst --clobber
