...
clone_depth_platform platform/external/kotlinx.atomicfu
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_13/platform/external/kotlinx.atomicfu/kotlinx_atomicfu^android_common.output . $GITHUB_WORKSPACE/artifacts/external/kotlinx.atomicfu/kotlinx_atomicfu^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_kotlinx.atomicfu.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/kotlinx.atomicfu/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_13 platform_external_kotlinx.atomicfu.tar.zst --clobber

du -ah -d1 platform_external_kotlinx.atomicfu*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_kotlinx.atomicfu.tar.zst" ]; then
  echo "Compressing platform/external/kotlinx.atomicfu -> platform_external_kotlinx.atomicfu.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_kotlinx.atomicfu.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/kotlinx.atomicfu/ .
fi
