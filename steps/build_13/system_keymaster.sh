...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_13.ninja libcppcose_rkp,android_vendor.31_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/system/keymaster/libcppcose_rkp^android_vendor.31_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_13/system/keymaster/libcppcose_rkp^android_vendor.31_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/keymaster/libcppcose_rkp^android_vendor.31_x86_64_shared

echo "building libcppcose_rkp^android_x86_64_shared"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_13.ninja libcppcose_rkp,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/system/keymaster/libcppcose_rkp^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_13/system/keymaster/libcppcose_rkp^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/keymaster/libcppcose_rkp^android_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_keymaster.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/keymaster/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_13 system_keymaster.tar.zst --clobber
