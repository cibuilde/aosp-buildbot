...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_13.ninja ziptool,android_recovery_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/libziparchive/ziptool^android_recovery_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_13/system/libziparchive/ziptool^android_recovery_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/libziparchive/ziptool^android_recovery_x86_64

echo "building ziptool^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_13.ninja ziptool,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/libziparchive/ziptool^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_13/system/libziparchive/ziptool^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/libziparchive/ziptool^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_libziparchive.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/libziparchive/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_13 system_libziparchive.tar.zst --clobber
