...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_14.ninja clatd,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/external/android-clat/clatd^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_14/external/android-clat/clatd^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/android-clat/clatd^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_android-clat.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/android-clat/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_14 external_android-clat.tar.zst --clobber

du -ah -d1 external_android-clat*.tar.zst | sort -h
