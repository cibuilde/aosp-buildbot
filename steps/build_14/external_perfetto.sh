...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_14.ninja heapprofd_client,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/external/perfetto/heapprofd_client^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_14/external/perfetto/heapprofd_client^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/perfetto/heapprofd_client^android_x86_64_shared

echo "building heapprofd_client^android_x86_x86_64_shared"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_14.ninja heapprofd_client,android_x86_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/external/perfetto/heapprofd_client^android_x86_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_14/external/perfetto/heapprofd_client^android_x86_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/perfetto/heapprofd_client^android_x86_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_perfetto.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/perfetto/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_14 external_perfetto.tar.zst --clobber
