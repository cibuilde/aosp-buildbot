...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_14.ninja libbitflags,android_x86_64_dylib
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/bitflags/libbitflags^android_x86_64_dylib
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_14/external/rust/crates/bitflags/libbitflags^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/bitflags/libbitflags^android_x86_64_dylib

echo "building libbitflags^android_x86_64_dylib_apex10000"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_14.ninja libbitflags,android_x86_64_dylib_apex10000
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/bitflags/libbitflags^android_x86_64_dylib_apex10000
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_14/external/rust/crates/bitflags/libbitflags^android_x86_64_dylib_apex10000.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/bitflags/libbitflags^android_x86_64_dylib_apex10000
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_14 external_rust_crates_bitflags.tar.zst --clobber
