...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_14.ninja libbyteorder,android_x86_64_dylib
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/byteorder/libbyteorder^android_x86_64_dylib
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_14/external/rust/crates/byteorder/libbyteorder^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/byteorder/libbyteorder^android_x86_64_dylib
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_14 external_rust_crates_byteorder.tar.zst --clobber

du -ah -d1 external_rust_crates_byteorder*.tar.zst | sort -h
