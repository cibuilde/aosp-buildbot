...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_14.ninja libhashlink,android_x86_64_rlib_rlib-std
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/hashlink/libhashlink^android_x86_64_rlib_rlib-std
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_14/external/rust/crates/hashlink/libhashlink^android_x86_64_rlib_rlib-std.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/hashlink/libhashlink^android_x86_64_rlib_rlib-std
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_14 external_rust_crates_hashlink.tar.zst --clobber
