...
clone_depth_platform platform/external/rappor
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_14.ninja rappor,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rappor/rappor^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_14/platform/external/rappor/rappor^android_common.output . $GITHUB_WORKSPACE/artifacts/external/rappor/rappor^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_rappor.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/rappor/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_14 platform_external_rappor.tar.zst --clobber

du -ah -d1 platform_external_rappor*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_rappor.tar.zst" ]; then
  echo "Compressing platform/external/rappor -> platform_external_rappor.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_rappor.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/rappor/ .
fi
