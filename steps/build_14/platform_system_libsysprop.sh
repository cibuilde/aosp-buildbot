...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_14.ninja PlatformProperties_public,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/system/libsysprop/srcs/PlatformProperties_public^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_14/platform/system/libsysprop/PlatformProperties_public^android_common.output . $GITHUB_WORKSPACE/artifacts/system/libsysprop/srcs/PlatformProperties_public^android_common

echo "building PlatformProperties^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_14.ninja PlatformProperties,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/system/libsysprop/srcs/PlatformProperties^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_14/platform/system/libsysprop/PlatformProperties^android_common.output . $GITHUB_WORKSPACE/artifacts/system/libsysprop/srcs/PlatformProperties^android_common
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_14 platform_system_libsysprop.tar.zst --clobber
