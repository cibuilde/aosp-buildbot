...
rsync -a -r $GITHUB_WORKSPACE/downloads/system/extras/squashfs_utils/libsquashfs_utils^android_recovery_x86_64_shared/ .
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_14.ninja libfec,android_recovery_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/system/extras/libfec/libfec^android_recovery_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_14/system/extras/libfec^android_recovery_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/extras/libfec/libfec^android_recovery_x86_64_shared

echo "building libfec^android_x86_64_shared"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_14.ninja libfec,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/system/extras/libfec/libfec^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_14/system/extras/libfec^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/extras/libfec/libfec^android_x86_64_shared

echo "building libsimpleperf_profcollect_bindgen^android_x86_64_rlib_dylib-std"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_14.ninja libsimpleperf_profcollect_bindgen,android_x86_64_rlib_dylib-std
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_14/system/extras/libsimpleperf_profcollect_bindgen^android_x86_64_rlib_dylib-std.output . $GITHUB_WORKSPACE/artifacts/system/extras/simpleperf/libsimpleperf_profcollect_bindgen^android_x86_64_rlib_dylib-std

echo "building preopt2cachename^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_14.ninja preopt2cachename,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/extras/preopt2cachename/preopt2cachename^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_14/system/extras/preopt2cachename^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/extras/preopt2cachename/preopt2cachename^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_extras.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/extras/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_14 system_extras.tar.zst --clobber
