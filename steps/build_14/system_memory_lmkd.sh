...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_14.ninja lmkd,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/memory/lmkd/lmkd^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_14/system/memory/lmkd/lmkd^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/memory/lmkd/lmkd^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_memory_lmkd.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/memory/lmkd/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_14 system_memory_lmkd.tar.zst --clobber
