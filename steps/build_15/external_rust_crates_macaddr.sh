...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_15.ninja libmacaddr,android_x86_64_dylib
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/macaddr/libmacaddr^android_x86_64_dylib
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_15/external/rust/crates/macaddr/libmacaddr^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/macaddr/libmacaddr^android_x86_64_dylib
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_15 external_rust_crates_macaddr.tar.zst --clobber
