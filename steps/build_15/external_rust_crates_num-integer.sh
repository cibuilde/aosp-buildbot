...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_15.ninja libnum_integer,android_x86_64_dylib
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/num-integer/libnum_integer^android_x86_64_dylib
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_15/external/rust/crates/num-integer/libnum_integer^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/num-integer/libnum_integer^android_x86_64_dylib
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_15 external_rust_crates_num-integer.tar.zst --clobber
