...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_15.ninja libcxx,android_x86_64_rlib_dylib-std
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/cxx/libcxx^android_x86_64_rlib_dylib-std
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_15/external/rust/cxx/libcxx^android_x86_64_rlib_dylib-std.output . $GITHUB_WORKSPACE/artifacts/external/rust/cxx/libcxx^android_x86_64_rlib_dylib-std

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_rust_cxx.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/rust/cxx/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_15 external_rust_cxx.tar.zst --clobber
