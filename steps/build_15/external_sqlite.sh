...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_15.ninja sqlite3,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/external/sqlite/dist/sqlite3^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_15/external/sqlite/sqlite3^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/sqlite/dist/sqlite3^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_sqlite.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/sqlite/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_15 external_sqlite.tar.zst --clobber
