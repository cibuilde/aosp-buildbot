...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_15.ninja art-module-intra-core-api-stubs-system-modules,android_common
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_15/libcore/art-module-intra-core-api-stubs-system-modules^android_common.output . $GITHUB_WORKSPACE/artifacts/libcore/mmodules/intracoreapi/art-module-intra-core-api-stubs-system-modules^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf libcore.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/libcore/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_15 libcore.tar.zst --clobber
