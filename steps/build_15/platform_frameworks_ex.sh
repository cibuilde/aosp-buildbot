...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_15/platform/frameworks/ex/libframesequence^android_x86_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/frameworks/ex/framesequence/jni/libframesequence^android_x86_x86_64_shared
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_15/platform/frameworks/ex/libframesequence^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/frameworks/ex/framesequence/jni/libframesequence^android_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_frameworks_ex.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/frameworks/ex/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_15 platform_frameworks_ex.tar.zst --clobber
