...
clone_depth_platform platform/frameworks/opt/net/voip
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_15.ninja librtp_jni,android_x86_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/frameworks/opt/net/voip/src/jni/rtp/librtp_jni^android_x86_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_15/platform/frameworks/opt/net/voip/librtp_jni^android_x86_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/frameworks/opt/net/voip/src/jni/rtp/librtp_jni^android_x86_x86_64_shared

echo "building librtp_jni^android_x86_64_shared"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_15.ninja librtp_jni,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/frameworks/opt/net/voip/src/jni/rtp/librtp_jni^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_15/platform/frameworks/opt/net/voip/librtp_jni^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/frameworks/opt/net/voip/src/jni/rtp/librtp_jni^android_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_frameworks_opt_net_voip.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/frameworks/opt/net/voip/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_15 platform_frameworks_opt_net_voip.tar.zst --clobber

du -ah -d1 platform_frameworks_opt_net_voip*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_frameworks_opt_net_voip.tar.zst" ]; then
  echo "Compressing platform/frameworks/opt/net/voip -> platform_frameworks_opt_net_voip.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_frameworks_opt_net_voip.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/frameworks/opt/net/voip/ .
fi
