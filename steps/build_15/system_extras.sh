...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_15.ninja libsimpleperf_profcollect_rust,android_x86_64_rlib_dylib-std
mkdir -p $GITHUB_WORKSPACE/artifacts/system/extras/simpleperf/libsimpleperf_profcollect_rust^android_x86_64_rlib_dylib-std
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_15/system/extras/libsimpleperf_profcollect_rust^android_x86_64_rlib_dylib-std.output . $GITHUB_WORKSPACE/artifacts/system/extras/simpleperf/libsimpleperf_profcollect_rust^android_x86_64_rlib_dylib-std

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_extras.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/extras/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_15 system_extras.tar.zst --clobber
