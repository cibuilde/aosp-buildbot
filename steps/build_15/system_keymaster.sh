...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_15.ninja libsoft_attestation_cert,android_vendor.31_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/system/keymaster/libsoft_attestation_cert^android_vendor.31_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_15/system/keymaster/libsoft_attestation_cert^android_vendor.31_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/keymaster/libsoft_attestation_cert^android_vendor.31_x86_64_shared

echo "building libsoft_attestation_cert^android_x86_64_shared"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_15.ninja libsoft_attestation_cert,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/system/keymaster/libsoft_attestation_cert^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_15/system/keymaster/libsoft_attestation_cert^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/keymaster/libsoft_attestation_cert^android_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_keymaster.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/keymaster/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_15 system_keymaster.tar.zst --clobber
