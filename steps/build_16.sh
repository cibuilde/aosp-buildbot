...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_14 --pattern art.tar.zst --output art-14.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/art
tar xf $GITHUB_WORKSPACE/art-14.tar.zst -C $GITHUB_WORKSPACE/downloads/art/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_09 --pattern external_bcc.tar.zst --output external_bcc-09.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/bcc
tar xf $GITHUB_WORKSPACE/external_bcc-09.tar.zst -C $GITHUB_WORKSPACE/downloads/external/bcc/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_11 --pattern external_cblas.tar.zst --output external_cblas-11.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/cblas
tar xf $GITHUB_WORKSPACE/external_cblas-11.tar.zst -C $GITHUB_WORKSPACE/downloads/external/cblas/

gh release --repo cibuilde/aosp-buildbot download android12-gsi_15 --pattern external_conscrypt.tar.zst --output external_conscrypt-15.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/conscrypt
tar xf $GITHUB_WORKSPACE/external_conscrypt-15.tar.zst -C $GITHUB_WORKSPACE/downloads/external/conscrypt/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_12 --pattern external_e2fsprogs.tar.zst --output external_e2fsprogs-12.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/e2fsprogs
tar xf $GITHUB_WORKSPACE/external_e2fsprogs-12.tar.zst -C $GITHUB_WORKSPACE/downloads/external/e2fsprogs/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_14 --pattern external_e2fsprogs.tar.zst --output external_e2fsprogs-14.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/e2fsprogs
tar xf $GITHUB_WORKSPACE/external_e2fsprogs-14.tar.zst -C $GITHUB_WORKSPACE/downloads/external/e2fsprogs/

gh release --repo cibuilde/aosp-buildbot download android12-gsi_15 --pattern external_e2fsprogs.tar.zst --output external_e2fsprogs-15.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/e2fsprogs
tar xf $GITHUB_WORKSPACE/external_e2fsprogs-15.tar.zst -C $GITHUB_WORKSPACE/downloads/external/e2fsprogs/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_08 --pattern external_gemmlowp.tar.zst --output external_gemmlowp-08.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/gemmlowp
tar xf $GITHUB_WORKSPACE/external_gemmlowp-08.tar.zst -C $GITHUB_WORKSPACE/downloads/external/gemmlowp/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_15 --pattern external_harfbuzz_ng.tar.zst --output external_harfbuzz_ng-15.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/harfbuzz_ng
tar xf $GITHUB_WORKSPACE/external_harfbuzz_ng-15.tar.zst -C $GITHUB_WORKSPACE/downloads/external/harfbuzz_ng/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_08 --pattern external_scrypt.tar.zst --output external_scrypt-08.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/scrypt
tar xf $GITHUB_WORKSPACE/external_scrypt-08.tar.zst -C $GITHUB_WORKSPACE/downloads/external/scrypt/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_15 --pattern hardware_libhardware_legacy.tar.zst --output hardware_libhardware_legacy-15.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/hardware/libhardware_legacy
tar xf $GITHUB_WORKSPACE/hardware_libhardware_legacy-15.tar.zst -C $GITHUB_WORKSPACE/downloads/hardware/libhardware_legacy/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_15 --pattern system_bpf.tar.zst --output system_bpf-15.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/system/bpf
tar xf $GITHUB_WORKSPACE/system_bpf-15.tar.zst -C $GITHUB_WORKSPACE/downloads/system/bpf/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_11 --pattern system_gatekeeper.tar.zst --output system_gatekeeper-11.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/system/gatekeeper
tar xf $GITHUB_WORKSPACE/system_gatekeeper-11.tar.zst -C $GITHUB_WORKSPACE/downloads/system/gatekeeper/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_15 --pattern system_hardware_interfaces.tar.zst --output system_hardware_interfaces-15.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/system/hardware/interfaces
tar xf $GITHUB_WORKSPACE/system_hardware_interfaces-15.tar.zst -C $GITHUB_WORKSPACE/downloads/system/hardware/interfaces/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_15 --pattern system_keymaster.tar.zst --output system_keymaster-15.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/system/keymaster
tar xf $GITHUB_WORKSPACE/system_keymaster-15.tar.zst -C $GITHUB_WORKSPACE/downloads/system/keymaster/
...
time source steps/build_16/art.sh
time source steps/build_16/bootable_recovery.sh
time source steps/build_16/external_conscrypt.sh
time source steps/build_16/external_crosvm.sh
time source steps/build_16/external_e2fsprogs.sh
time source steps/build_16/external_icu.sh
time source steps/build_16/external_libbrillo.sh
time source steps/build_16/external_rust_crates_chrono.sh
time source steps/build_16/external_rust_crates_env_logger.sh
time source steps/build_16/external_rust_crates_flate2.sh
time source steps/build_16/external_rust_crates_futures-util.sh
time source steps/build_16/external_rust_crates_rand_core.sh
time source steps/build_16/external_rust_crates_structopt.sh
time source steps/build_16/external_vm_tools_p9.sh
time source steps/build_16/frameworks_av.sh
time source steps/build_16/frameworks_base.sh
time source steps/build_16/frameworks_hardware_interfaces.sh
time source steps/build_16/frameworks_minikin.sh
time source steps/build_16/frameworks_native.sh
time source steps/build_16/frameworks_rs.sh
time source steps/build_16/hardware_interfaces.sh
time source steps/build_16/hardware_libhardware.sh
time source steps/build_16/hardware_libhardware_legacy.sh
time source steps/build_16/libcore.sh
time source steps/build_16/packages_modules_DnsResolver.sh
time source steps/build_16/packages_modules_NeuralNetworks.sh
time source steps/build_16/prebuilts_sdk.sh
time source steps/build_16/system_apex.sh
time source steps/build_16/system_bpf.sh
time source steps/build_16/system_bt.sh
time source steps/build_16/system_connectivity_wificond.sh
time source steps/build_16/system_core.sh
time source steps/build_16/system_extras.sh
time source steps/build_16/system_gsid.sh
time source steps/build_16/system_hardware_interfaces.sh
time source steps/build_16/system_iorap.sh
time source steps/build_16/system_keymaster.sh
time source steps/build_16/system_libhidl.sh
time source steps/build_16/system_memory_libmemtrack.sh
time source steps/build_16/system_netd.sh
time source steps/build_16/system_security.sh
time source steps/build_16/system_tools_aidl.sh
time source steps/build_16/system_update_engine.sh
