...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_16.ninja uncrypt,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/bootable/recovery/uncrypt/uncrypt^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_16/bootable/recovery/uncrypt^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/bootable/recovery/uncrypt/uncrypt^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf bootable_recovery.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/bootable/recovery/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_16 bootable_recovery.tar.zst --clobber
