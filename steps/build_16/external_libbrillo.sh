...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_16.ninja libbrillo-binder,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/external/libbrillo/libbrillo-binder^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_16/external/libbrillo/libbrillo-binder^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/libbrillo/libbrillo-binder^android_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_libbrillo.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/libbrillo/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_16 external_libbrillo.tar.zst --clobber
