...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_16.ninja libflate2,android_x86_64_dylib
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/flate2/libflate2^android_x86_64_dylib
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_16/external/rust/crates/flate2/libflate2^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/flate2/libflate2^android_x86_64_dylib
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_16 external_rust_crates_flate2.tar.zst --clobber
