...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_16.ninja librand_core,android_x86_64_dylib
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/rand_core/librand_core^android_x86_64_dylib
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_16/external/rust/crates/rand_core/librand_core^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/rand_core/librand_core^android_x86_64_dylib
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_16 external_rust_crates_rand_core.tar.zst --clobber
