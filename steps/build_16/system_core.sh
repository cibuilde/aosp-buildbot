...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_16.ninja init_second_stage,android_recovery_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/core/init/init_second_stage^android_recovery_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_16/system/core/init_second_stage^android_recovery_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/core/init/init_second_stage^android_recovery_x86_64

echo "building init_second_stage^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_16.ninja init_second_stage,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/core/init/init_second_stage^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_16/system/core/init_second_stage^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/core/init/init_second_stage^android_x86_64

echo "building libbinderwrapper^android_x86_64_shared"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_16.ninja libbinderwrapper,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/system/core/libbinderwrapper/libbinderwrapper^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_16/system/core/libbinderwrapper^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/core/libbinderwrapper/libbinderwrapper^android_x86_64_shared

echo "building libgatekeeper_aidl^android_x86_64_shared"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_16.ninja libgatekeeper_aidl,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/system/core/gatekeeperd/libgatekeeper_aidl^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_16/system/core/libgatekeeper_aidl^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/core/gatekeeperd/libgatekeeper_aidl^android_x86_64_shared

echo "building libsnapshot^android_x86_64_static"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_16.ninja libsnapshot,android_x86_64_static
mkdir -p $GITHUB_WORKSPACE/artifacts/system/core/fs_mgr/libsnapshot/libsnapshot^android_x86_64_static
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_16/system/core/libsnapshot^android_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/system/core/fs_mgr/libsnapshot/libsnapshot^android_x86_64_static

echo "building libstoraged^android_x86_64_static"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_16.ninja libstoraged,android_x86_64_static
mkdir -p $GITHUB_WORKSPACE/artifacts/system/core/storaged/libstoraged^android_x86_64_static
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_16/system/core/libstoraged^android_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/system/core/storaged/libstoraged^android_x86_64_static

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_core.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/core/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_16 system_core.tar.zst --clobber
