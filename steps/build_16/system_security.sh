...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_16.ninja libkeystore-attestation-application-id,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/system/security/keystore/libkeystore-attestation-application-id^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_16/system/security/libkeystore-attestation-application-id^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/security/keystore/libkeystore-attestation-application-id^android_x86_64_shared

echo "building libkeystore-wifi-hidl^android_vendor.31_x86_64_shared"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_16.ninja libkeystore-wifi-hidl,android_vendor.31_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/system/security/keystore/libkeystore-wifi-hidl^android_vendor.31_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_16/system/security/libkeystore-wifi-hidl^android_vendor.31_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/security/keystore/libkeystore-wifi-hidl^android_vendor.31_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_security.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/security/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_16 system_security.tar.zst --clobber
