...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_16.ninja libupdate_engine_stable-V1-cpp,android_x86_64_static
mkdir -p $GITHUB_WORKSPACE/artifacts/system/update_engine/stable/libupdate_engine_stable-V1-cpp^android_x86_64_static
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_16/system/update_engine/libupdate_engine_stable-V1-cpp^android_x86_64_static.output . $GITHUB_WORKSPACE/artifacts/system/update_engine/stable/libupdate_engine_stable-V1-cpp^android_x86_64_static

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_update_engine.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/update_engine/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_16 system_update_engine.tar.zst --clobber
