...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_17.ninja libandroid_logger,android_x86_64_dylib
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/android_logger/libandroid_logger^android_x86_64_dylib
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_17/external/rust/crates/android_logger/libandroid_logger^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/android_logger/libandroid_logger^android_x86_64_dylib

echo "building libandroid_logger^android_x86_64_dylib_apex10000"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_17.ninja libandroid_logger,android_x86_64_dylib_apex10000
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/android_logger/libandroid_logger^android_x86_64_dylib_apex10000
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_17/external/rust/crates/android_logger/libandroid_logger^android_x86_64_dylib_apex10000.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/android_logger/libandroid_logger^android_x86_64_dylib_apex10000
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_17 external_rust_crates_android_logger.tar.zst --clobber
