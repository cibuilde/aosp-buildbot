...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_17.ninja libzip,android_x86_64_dylib
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/zip/libzip^android_x86_64_dylib
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_17/external/rust/crates/zip/libzip^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/zip/libzip^android_x86_64_dylib
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_17 external_rust_crates_zip.tar.zst --clobber
