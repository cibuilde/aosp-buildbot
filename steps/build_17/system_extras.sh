...
rsync -a -r $GITHUB_WORKSPACE/downloads/system/extras/partition_tools/aidl/liblpdump_interface-cpp^android_x86_64_static/ .

echo "building liblpdump_interface-cpp^android_x86_64_shared"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_17.ninja liblpdump_interface-cpp,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/system/extras/partition_tools/aidl/liblpdump_interface-cpp^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_17/system/extras/liblpdump_interface-cpp^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/extras/partition_tools/aidl/liblpdump_interface-cpp^android_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_extras.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/extras/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_17 system_extras.tar.zst --clobber
