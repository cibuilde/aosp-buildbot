...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_17.ninja gsi_aidl_interface-cpp,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/system/gsid/gsi_aidl_interface-cpp^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_17/system/gsid/gsi_aidl_interface-cpp^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/gsid/gsi_aidl_interface-cpp^android_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_gsid.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/gsid/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_17 system_gsid.tar.zst --clobber
