...
rsync -a -r $GITHUB_WORKSPACE/downloads/system/keymaster/lib_android_keymaster_keymint_utils^android_vendor.31_x86_64_shared/ .
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_17.ninja libkeymint,android_vendor.31_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/system/keymaster/libkeymint^android_vendor.31_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_17/system/keymaster/libkeymint^android_vendor.31_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/keymaster/libkeymint^android_vendor.31_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_keymaster.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/keymaster/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_17 system_keymaster.tar.zst --clobber
