...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_10 --pattern external_curl.tar.zst --output external_curl-10.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/curl
tar xf $GITHUB_WORKSPACE/external_curl-10.tar.zst -C $GITHUB_WORKSPACE/downloads/external/curl/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_11 --pattern external_mdnsresponder.tar.zst --output external_mdnsresponder-11.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/mdnsresponder
tar xf $GITHUB_WORKSPACE/external_mdnsresponder-11.tar.zst -C $GITHUB_WORKSPACE/downloads/external/mdnsresponder/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_09 --pattern external_sonic.tar.zst --output external_sonic-09.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/sonic
tar xf $GITHUB_WORKSPACE/external_sonic-09.tar.zst -C $GITHUB_WORKSPACE/downloads/external/sonic/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_16 --pattern packages_modules_DnsResolver.tar.zst --output packages_modules_DnsResolver-16.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/packages/modules/DnsResolver
tar xf $GITHUB_WORKSPACE/packages_modules_DnsResolver-16.tar.zst -C $GITHUB_WORKSPACE/downloads/packages/modules/DnsResolver/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_17 --pattern system_core.tar.zst --output system_core-17.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/system/core
tar xf $GITHUB_WORKSPACE/system_core-17.tar.zst -C $GITHUB_WORKSPACE/downloads/system/core/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_17 --pattern system_extras.tar.zst --output system_extras-17.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/system/extras
tar xf $GITHUB_WORKSPACE/system_extras-17.tar.zst -C $GITHUB_WORKSPACE/downloads/system/extras/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_17 --pattern system_gsid.tar.zst --output system_gsid-17.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/system/gsid
tar xf $GITHUB_WORKSPACE/system_gsid-17.tar.zst -C $GITHUB_WORKSPACE/downloads/system/gsid/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_11 --pattern system_hardware_interfaces.tar.zst --output system_hardware_interfaces-11.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/system/hardware/interfaces
tar xf $GITHUB_WORKSPACE/system_hardware_interfaces-11.tar.zst -C $GITHUB_WORKSPACE/downloads/system/hardware/interfaces/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_17 --pattern system_iorap.tar.zst --output system_iorap-17.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/system/iorap
tar xf $GITHUB_WORKSPACE/system_iorap-17.tar.zst -C $GITHUB_WORKSPACE/downloads/system/iorap/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_17 --pattern system_keymaster.tar.zst --output system_keymaster-17.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/system/keymaster
tar xf $GITHUB_WORKSPACE/system_keymaster-17.tar.zst -C $GITHUB_WORKSPACE/downloads/system/keymaster/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_08 --pattern system_update_engine.tar.zst --output system_update_engine-08.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/system/update_engine
tar xf $GITHUB_WORKSPACE/system_update_engine-08.tar.zst -C $GITHUB_WORKSPACE/downloads/system/update_engine/

gh release --repo cibuilde/aosp-buildbot download android12-gsi_17 --pattern system_update_engine.tar.zst --output system_update_engine-17.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/system/update_engine
tar xf $GITHUB_WORKSPACE/system_update_engine-17.tar.zst -C $GITHUB_WORKSPACE/downloads/system/update_engine/
...
time source steps/build_18/bootable_recovery.sh
time source steps/build_18/device_google_cuttlefish.sh
time source steps/build_18/external_crosvm.sh
time source steps/build_18/external_rust_crates_futures.sh
time source steps/build_18/external_rust_crates_rand.sh
time source steps/build_18/frameworks_av.sh
time source steps/build_18/frameworks_base.sh
time source steps/build_18/frameworks_hardware_interfaces.sh
time source steps/build_18/frameworks_native.sh
time source steps/build_18/hardware_interfaces.sh
time source steps/build_18/libcore.sh
time source steps/build_18/packages_modules_DnsResolver.sh
time source steps/build_18/prebuilts_sdk.sh
time source steps/build_18/system_apex.sh
time source steps/build_18/system_bt.sh
time source steps/build_18/system_core.sh
time source steps/build_18/system_extras.sh
time source steps/build_18/system_gsid.sh
time source steps/build_18/system_hardware_interfaces.sh
time source steps/build_18/system_iorap.sh
time source steps/build_18/system_memory_libmemtrack.sh
time source steps/build_18/system_netd.sh
time source steps/build_18/system_security.sh
time source steps/build_18/system_tools_hidl.sh
time source steps/build_18/system_update_engine.sh
time source steps/build_18/system_vold.sh
