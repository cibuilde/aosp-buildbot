...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_18.ninja librand,android_x86_64_dylib
mkdir -p $GITHUB_WORKSPACE/artifacts/external/rust/crates/rand/librand^android_x86_64_dylib
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_18/external/rust/crates/rand/librand^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/external/rust/crates/rand/librand^android_x86_64_dylib
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_18 external_rust_crates_rand.tar.zst --clobber
