...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_18.ninja update_engine,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/update_engine/update_engine^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_18/system/update_engine/update_engine^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/update_engine/update_engine^android_x86_64

echo "building update_engine_sideload^android_recovery_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_18.ninja update_engine_sideload,android_recovery_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/update_engine/update_engine_sideload^android_recovery_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_18/system/update_engine/update_engine_sideload^android_recovery_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/update_engine/update_engine_sideload^android_recovery_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_update_engine.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/update_engine/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_18 system_update_engine.tar.zst --clobber
