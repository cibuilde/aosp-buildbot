...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_19.ninja update_verifier,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/bootable/recovery/update_verifier/update_verifier^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_19/bootable/recovery/update_verifier^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/bootable/recovery/update_verifier/update_verifier^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf bootable_recovery.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/bootable/recovery/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_19 bootable_recovery.tar.zst --clobber
