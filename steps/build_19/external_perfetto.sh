...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_19.ninja libperfetto_android_internal,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/external/perfetto/libperfetto_android_internal^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_19/external/perfetto/libperfetto_android_internal^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/external/perfetto/libperfetto_android_internal^android_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_perfetto.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/perfetto/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_19 external_perfetto.tar.zst --clobber
