...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_19.ninja ike-internals,android_common_apex30
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/modules/IPsec/ike-internals^android_common_apex30
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_19/platform/packages/modules/IPsec/ike-internals^android_common_apex30.output . $GITHUB_WORKSPACE/artifacts/packages/modules/IPsec/ike-internals^android_common_apex30

echo "building ike-internals^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_19.ninja ike-internals,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/modules/IPsec/ike-internals^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_19/platform/packages/modules/IPsec/ike-internals^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/modules/IPsec/ike-internals^android_common
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_19 platform_packages_modules_IPsec.tar.zst --clobber
