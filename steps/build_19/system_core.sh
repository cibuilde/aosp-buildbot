...
rsync -a -r $GITHUB_WORKSPACE/downloads/system/core/fs_mgr/libfs_mgr_binder^android_x86_64_shared/ .
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_19.ninja charger,android_recovery_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/core/healthd/charger^android_recovery_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_19/system/core/charger^android_recovery_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/core/healthd/charger^android_recovery_x86_64

echo "building charger^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_19.ninja charger,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/core/healthd/charger^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_19/system/core/charger^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/core/healthd/charger^android_x86_64

echo "building clean_scratch_files^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_19.ninja clean_scratch_files,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/core/fs_mgr/clean_scratch_files^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_19/system/core/clean_scratch_files^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/core/fs_mgr/clean_scratch_files^android_x86_64

echo "building remount^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_19.ninja remount,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/core/fs_mgr/remount^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_19/system/core/remount^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/core/fs_mgr/remount^android_x86_64

echo "building set-verity-state^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_19.ninja set-verity-state,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/core/set-verity-state/set-verity-state^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_19/system/core/set-verity-state^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/core/set-verity-state/set-verity-state^android_x86_64

echo "building snapshotctl^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_19.ninja snapshotctl,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/core/fs_mgr/libsnapshot/snapshotctl^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_19/system/core/snapshotctl^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/core/fs_mgr/libsnapshot/snapshotctl^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_core.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/core/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_19 system_core.tar.zst --clobber
