...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_19.ninja bootctl,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/extras/bootctl/bootctl^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_19/system/extras/bootctl^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/extras/bootctl/bootctl^android_x86_64

echo "building profcollectd_aidl_interface-rust^android_x86_64_dylib"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_19.ninja profcollectd_aidl_interface-rust,android_x86_64_dylib
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_19/system/extras/profcollectd_aidl_interface-rust^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/system/extras/profcollectd/libprofcollectd/profcollectd_aidl_interface-rust^android_x86_64_dylib

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_extras.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/extras/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_19 system_extras.tar.zst --clobber
