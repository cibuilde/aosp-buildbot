...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_19.ninja ndc,android_x86_64_cfi
mkdir -p $GITHUB_WORKSPACE/artifacts/system/netd/server/ndc^android_x86_64_cfi
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_19/system/netd/ndc^android_x86_64_cfi.output . $GITHUB_WORKSPACE/artifacts/system/netd/server/ndc^android_x86_64_cfi

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_netd.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/netd/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_19 system_netd.tar.zst --clobber
