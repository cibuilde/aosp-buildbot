...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_20.ninja libnfc_nci_jni,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/apps/Nfc/nci/jni/libnfc_nci_jni^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_20/packages/apps/Nfc/libnfc_nci_jni^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/packages/apps/Nfc/nci/jni/libnfc_nci_jni^android_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf packages_apps_Nfc.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/packages/apps/Nfc/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_20 packages_apps_Nfc.tar.zst --clobber
