...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_20.ninja gatekeeperd,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/core/gatekeeperd/gatekeeperd^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_20/system/core/gatekeeperd^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/core/gatekeeperd/gatekeeperd^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_core.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/core/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_20 system_core.tar.zst --clobber
