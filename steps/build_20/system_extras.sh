...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_20.ninja libprofcollectd,android_x86_64_dylib
mkdir -p $GITHUB_WORKSPACE/artifacts/system/extras/profcollectd/libprofcollectd/libprofcollectd^android_x86_64_dylib
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_20/system/extras/libprofcollectd^android_x86_64_dylib.output . $GITHUB_WORKSPACE/artifacts/system/extras/profcollectd/libprofcollectd/libprofcollectd^android_x86_64_dylib

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_extras.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/extras/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_20 system_extras.tar.zst --clobber
