...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_20.ninja libkeymint,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/system/keymaster/libkeymint^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_20/system/keymaster/libkeymint^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/keymaster/libkeymint^android_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_keymaster.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/keymaster/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_20 system_keymaster.tar.zst --clobber
