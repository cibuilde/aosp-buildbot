...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_20.ninja android.security.maintenance-ndk_platform,android_x86_64_shared
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_20/system/security/android.security.maintenance-ndk_platform^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/security/keystore2/aidl/android.security.maintenance-ndk_platform^android_x86_64_shared

echo "building credstore^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_20.ninja credstore,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/security/identity/credstore^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_20/system/security/credstore^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/security/identity/credstore^android_x86_64

echo "building keystore_cli_v2^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_20.ninja keystore_cli_v2,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/security/keystore/keystore_cli_v2^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_20/system/security/keystore_cli_v2^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/security/keystore/keystore_cli_v2^android_x86_64

echo "building libkeystore-engine^android_x86_64_shared"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_20.ninja libkeystore-engine,android_x86_64_shared
mkdir -p $GITHUB_WORKSPACE/artifacts/system/security/keystore-engine/libkeystore-engine^android_x86_64_shared
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_20/system/security/libkeystore-engine^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/system/security/keystore-engine/libkeystore-engine^android_x86_64_shared

echo "building libkeystore2_apc_compat-rust^android_x86_64_rlib_rlib-std"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_20.ninja libkeystore2_apc_compat-rust,android_x86_64_rlib_rlib-std
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_20/system/security/libkeystore2_apc_compat-rust^android_x86_64_rlib_rlib-std.output . $GITHUB_WORKSPACE/artifacts/system/security/keystore2/apc_compat/libkeystore2_apc_compat-rust^android_x86_64_rlib_rlib-std

echo "building odsign^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_20.ninja odsign,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/security/ondevice-signing/odsign^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_20/system/security/odsign^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/security/ondevice-signing/odsign^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_security.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/security/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_20 system_security.tar.zst --clobber
