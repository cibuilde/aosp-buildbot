...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_21.ninja racoon,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/external/ipsec-tools/racoon^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_21/external/ipsec-tools/racoon^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/external/ipsec-tools/racoon^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_ipsec-tools.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/ipsec-tools/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_21 external_ipsec-tools.tar.zst --clobber
