...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_21.ninja profcollectctl,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/extras/profcollectd/profcollectctl^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_21/system/extras/profcollectctl^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/extras/profcollectd/profcollectctl^android_x86_64

echo "building profcollectd^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_21.ninja profcollectd,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/extras/profcollectd/profcollectd^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_21/system/extras/profcollectd^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/extras/profcollectd/profcollectd^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_extras.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/extras/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_21 system_extras.tar.zst --clobber
