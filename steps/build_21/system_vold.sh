...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_21.ninja vold,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/vold/vold^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_21/system/vold/vold^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/vold/vold^android_x86_64

echo "building wait_for_keymaster^android_x86_64"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_21.ninja wait_for_keymaster,android_x86_64
mkdir -p $GITHUB_WORKSPACE/artifacts/system/vold/wait_for_keymaster^android_x86_64
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_21/system/vold/wait_for_keymaster^android_x86_64.output . $GITHUB_WORKSPACE/artifacts/system/vold/wait_for_keymaster^android_x86_64

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_vold.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/vold/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_21 system_vold.tar.zst --clobber

du -ah -d1 system_vold*.tar.zst | sort -h
