...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_22.ninja conscrypt.module.platform.api.stubs.source,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/conscrypt/conscrypt.module.platform.api.stubs.source^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_22/external/conscrypt/conscrypt.module.platform.api.stubs.source^android_common.output . $GITHUB_WORKSPACE/artifacts/external/conscrypt/conscrypt.module.platform.api.stubs.source^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_conscrypt.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/conscrypt/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_22 external_conscrypt.tar.zst --clobber
