...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_22.ninja libgooglecamerahal,android_vendor.31_x86_64_shared
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_22/hardware/google/camera/libgooglecamerahal^android_vendor.31_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/hardware/google/camera/common/hal/google_camera_hal/libgooglecamerahal^android_vendor.31_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf hardware_google_camera.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/hardware/google/camera/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_22 hardware_google_camera.tar.zst --clobber
