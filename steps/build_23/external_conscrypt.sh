...
echo "building conscrypt.module.platform.api.stubs^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_23.ninja conscrypt.module.platform.api.stubs,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/conscrypt/conscrypt.module.platform.api.stubs^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_23/external/conscrypt/conscrypt.module.platform.api.stubs^android_common.output . $GITHUB_WORKSPACE/artifacts/external/conscrypt/conscrypt.module.platform.api.stubs^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_conscrypt.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/conscrypt/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_23 external_conscrypt.tar.zst --clobber
