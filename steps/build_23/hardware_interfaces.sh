...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_23.ninja android.hardware.camera.provider@2.7,android_x86_64_shared
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_23/hardware/interfaces/android.hardware.camera.provider@2.7^android_x86_64_shared.output . $GITHUB_WORKSPACE/artifacts/hardware/interfaces/camera/provider/2.7/android.hardware.camera.provider@2.7^android_x86_64_shared

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf hardware_interfaces.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/hardware/interfaces/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_23 hardware_interfaces.tar.zst --clobber
