...
echo "Preparing for packages/apps/Launcher3"
...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_23.ninja launcher_log_protos_lite,android_common
...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_23.ninja launcher_quickstep_log_protos_lite,android_common
...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_23.ninja LauncherPluginLib,android_common
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_23 packages_apps_Launcher3.tar.zst --clobber

du -ah -d1 packages_apps_Launcher3*.tar.zst | sort -h
