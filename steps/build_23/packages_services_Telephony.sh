...
echo "Preparing for packages/services/Telephony"
...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_23.ninja ecc-protos-lite,android_common
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_23 packages_services_Telephony.tar.zst --clobber

du -ah -d1 packages_services_Telephony*.tar.zst | sort -h
