...
clone_depth_platform platform/external/okio
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_23.ninja okio-lib,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/okio/okio-lib^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_23/platform/external/okio/okio-lib^android_common.output . $GITHUB_WORKSPACE/artifacts/external/okio/okio-lib^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_okio.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/okio/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_23 platform_external_okio.tar.zst --clobber

du -ah -d1 platform_external_okio*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_okio.tar.zst" ]; then
  echo "Compressing platform/external/okio -> platform_external_okio.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_okio.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/okio/ .
fi
