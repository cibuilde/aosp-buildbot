...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_23/platform/frameworks/av/mediatranscoding_aidl_interface-java^android_common_apex29.output . $GITHUB_WORKSPACE/artifacts/frameworks/av/media/libmediatranscoding/mediatranscoding_aidl_interface-java^android_common_apex29

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_frameworks_av.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/frameworks/av/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_23 platform_frameworks_av.tar.zst --clobber
