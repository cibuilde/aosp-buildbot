...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_23.ninja android-ex-camera2-utils,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/frameworks/ex/camera2/utils/android-ex-camera2-utils^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_23/platform/frameworks/ex/android-ex-camera2-utils^android_common.output . $GITHUB_WORKSPACE/artifacts/frameworks/ex/camera2/utils/android-ex-camera2-utils^android_common

echo "building android-ex-camera2-portability^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_23.ninja android-ex-camera2-portability,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/frameworks/ex/camera2/portability/android-ex-camera2-portability^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_23/platform/frameworks/ex/android-ex-camera2-portability^android_common.output . $GITHUB_WORKSPACE/artifacts/frameworks/ex/camera2/portability/android-ex-camera2-portability^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_frameworks_ex.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/frameworks/ex/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_23 platform_frameworks_ex.tar.zst --clobber
