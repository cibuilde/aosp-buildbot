...
clone_depth_platform platform/packages/apps/PhoneCommon
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_23.ninja com.android.phone.common-lib,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/apps/PhoneCommon/com.android.phone.common-lib^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_23/platform/packages/apps/PhoneCommon/com.android.phone.common-lib^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/apps/PhoneCommon/com.android.phone.common-lib^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_apps_PhoneCommon.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/apps/PhoneCommon/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_23 platform_packages_apps_PhoneCommon.tar.zst --clobber

du -ah -d1 platform_packages_apps_PhoneCommon*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_apps_PhoneCommon.tar.zst" ]; then
  echo "Compressing platform/packages/apps/PhoneCommon -> platform_packages_apps_PhoneCommon.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_apps_PhoneCommon.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/apps/PhoneCommon/ .
fi
