...
clone_depth_platform platform/packages/modules/Scheduling
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_23.ninja scheduling-statsd,android_common_apex10000
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/modules/Scheduling/service/scheduling-statsd^android_common_apex10000
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_23/platform/packages/modules/Scheduling/scheduling-statsd^android_common_apex10000.output . $GITHUB_WORKSPACE/artifacts/packages/modules/Scheduling/service/scheduling-statsd^android_common_apex10000

echo "building com.android.scheduling-bootclasspath-fragment^android_common_apex10000"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_23.ninja com.android.scheduling-bootclasspath-fragment,android_common_apex10000
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_23/platform/packages/modules/Scheduling/com.android.scheduling-bootclasspath-fragment^android_common_apex10000.output . $GITHUB_WORKSPACE/artifacts/packages/modules/Scheduling/apex/com.android.scheduling-bootclasspath-fragment^android_common_apex10000

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_modules_Scheduling.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/modules/Scheduling/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_23 platform_packages_modules_Scheduling.tar.zst --clobber

du -ah -d1 platform_packages_modules_Scheduling*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_modules_Scheduling.tar.zst" ]; then
  echo "Compressing platform/packages/modules/Scheduling -> platform_packages_modules_Scheduling.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_modules_Scheduling.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/modules/Scheduling/ .
fi
