...
clone_depth_platform platform/packages/services/Telephony
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_23/platform/packages/services/Telephony/ecc-protos-lite^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/services/Telephony/ecc/conversion_toolset_v1/proto/ecc-protos-lite^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_services_Telephony.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/services/Telephony/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_23 platform_packages_services_Telephony.tar.zst --clobber

du -ah -d1 platform_packages_services_Telephony*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_services_Telephony.tar.zst" ]; then
  echo "Compressing platform/packages/services/Telephony -> platform_packages_services_Telephony.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_services_Telephony.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/services/Telephony/ .
fi
