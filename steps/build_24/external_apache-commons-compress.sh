...
echo "Preparing for external/apache-commons-compress"
...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_24.ninja apache-commons-compress,android_common
...
cd $GITHUB_WORKSPACE/
tar -cf external_apache-commons-compress.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/apache-commons-compress/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_24 external_apache-commons-compress.tar.zst --clobber

du -ah -d1 external_apache-commons-compress*.tar.zst | sort -h
