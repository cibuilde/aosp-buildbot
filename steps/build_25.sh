...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_11 --pattern external_cn-cbor.tar.zst --output external_cn-cbor-11.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/cn-cbor
tar xf $GITHUB_WORKSPACE/external_cn-cbor-11.tar.zst -C $GITHUB_WORKSPACE/downloads/external/cn-cbor/

gh release --repo cibuilde/aosp-buildbot download android12-gsi_09 --pattern external_compiler-rt.tar.zst --output external_compiler-rt-09.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/compiler-rt
tar xf $GITHUB_WORKSPACE/external_compiler-rt-09.tar.zst -C $GITHUB_WORKSPACE/downloads/external/compiler-rt/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_24 --pattern external_crosvm.tar.zst --output external_crosvm-24.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/crosvm
tar xf $GITHUB_WORKSPACE/external_crosvm-24.tar.zst -C $GITHUB_WORKSPACE/downloads/external/crosvm/

gh release --repo cibuilde/aosp-buildbot download android12-gsi_12 --pattern external_curl.tar.zst --output external_curl-12.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/curl
tar xf $GITHUB_WORKSPACE/external_curl-12.tar.zst -C $GITHUB_WORKSPACE/downloads/external/curl/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_24 --pattern external_icu.tar.zst --output external_icu-24.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/icu
tar xf $GITHUB_WORKSPACE/external_icu-24.tar.zst -C $GITHUB_WORKSPACE/downloads/external/icu/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_11 --pattern external_libevent.tar.zst --output external_libevent-11.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/libevent
tar xf $GITHUB_WORKSPACE/external_libevent-11.tar.zst -C $GITHUB_WORKSPACE/downloads/external/libevent/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_12 --pattern external_libldac.tar.zst --output external_libldac-12.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/libldac
tar xf $GITHUB_WORKSPACE/external_libldac-12.tar.zst -C $GITHUB_WORKSPACE/downloads/external/libldac/

gh release --repo cibuilde/aosp-buildbot download android12-gsi_11 --pattern external_libnl.tar.zst --output external_libnl-11.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/libnl
tar xf $GITHUB_WORKSPACE/external_libnl-11.tar.zst -C $GITHUB_WORKSPACE/downloads/external/libnl/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_11 --pattern external_libpng.tar.zst --output external_libpng-11.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/libpng
tar xf $GITHUB_WORKSPACE/external_libpng-11.tar.zst -C $GITHUB_WORKSPACE/downloads/external/libpng/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_11 --pattern external_lz4.tar.zst --output external_lz4-11.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/lz4
tar xf $GITHUB_WORKSPACE/external_lz4-11.tar.zst -C $GITHUB_WORKSPACE/downloads/external/lz4/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_09 --pattern external_pcre.tar.zst --output external_pcre-09.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/pcre
tar xf $GITHUB_WORKSPACE/external_pcre-09.tar.zst -C $GITHUB_WORKSPACE/downloads/external/pcre/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_11 --pattern external_piex.tar.zst --output external_piex-11.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/external/piex
tar xf $GITHUB_WORKSPACE/external_piex-11.tar.zst -C $GITHUB_WORKSPACE/downloads/external/piex/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_17 --pattern frameworks_rs.tar.zst --output frameworks_rs-17.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/frameworks/rs
tar xf $GITHUB_WORKSPACE/frameworks_rs-17.tar.zst -C $GITHUB_WORKSPACE/downloads/frameworks/rs/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_12 --pattern packages_modules_StatsD.tar.zst --output packages_modules_StatsD-12.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/packages/modules/StatsD
tar xf $GITHUB_WORKSPACE/packages_modules_StatsD-12.tar.zst -C $GITHUB_WORKSPACE/downloads/packages/modules/StatsD/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_14 --pattern system_media.tar.zst --output system_media-14.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/system/media
tar xf $GITHUB_WORKSPACE/system_media-14.tar.zst -C $GITHUB_WORKSPACE/downloads/system/media/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_16 --pattern system_memory_libmemtrack.tar.zst --output system_memory_libmemtrack-16.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/system/memory/libmemtrack
tar xf $GITHUB_WORKSPACE/system_memory_libmemtrack-16.tar.zst -C $GITHUB_WORKSPACE/downloads/system/memory/libmemtrack/
...
gh release --repo cibuilde/aosp-buildbot download android12-gsi_11 --pattern system_tools_mkbootimg.tar.zst --output system_tools_mkbootimg-11.tar.zst
mkdir -p $GITHUB_WORKSPACE/downloads/system/tools/mkbootimg
tar xf $GITHUB_WORKSPACE/system_tools_mkbootimg-11.tar.zst -C $GITHUB_WORKSPACE/downloads/system/tools/mkbootimg/
...
time source steps/build_25/external_conscrypt.sh
time source steps/build_25/external_crosvm.sh
time source steps/build_25/external_icu.sh
time source steps/build_25/frameworks_av.sh
time source steps/build_25/frameworks_native.sh
time source steps/build_25/frameworks_proto_logging.sh
time source steps/build_25/libcore.sh
time source steps/build_25/packages_modules_vndk.sh
time source steps/build_25/packages_providers_TelephonyProvider.sh
time source steps/build_25/prebuilts_sdk.sh
time source steps/build_25/system_security.sh
