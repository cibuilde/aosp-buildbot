...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_25.ninja crosvm,android_x86_64_apex10000
mkdir -p $GITHUB_WORKSPACE/artifacts/external/crosvm/crosvm^android_x86_64_apex10000
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_25/external/crosvm/crosvm^android_x86_64_apex10000.output . $GITHUB_WORKSPACE/artifacts/external/crosvm/crosvm^android_x86_64_apex10000

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_crosvm.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/crosvm/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_25 external_crosvm.tar.zst --clobber
