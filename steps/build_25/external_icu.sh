...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_25.ninja i18n.module.public.api.stubs,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/icu/android_icu4j/i18n.module.public.api.stubs^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_25/external/icu/i18n.module.public.api.stubs^android_common.output . $GITHUB_WORKSPACE/artifacts/external/icu/android_icu4j/i18n.module.public.api.stubs^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_icu.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/icu/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_25 external_icu.tar.zst --clobber
