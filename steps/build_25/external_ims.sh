...
echo "Preparing for external/ims"
...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_25.ninja ucepresencelib,android_common
...
cd $GITHUB_WORKSPACE/
tar -cf external_ims.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/ims/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_25 external_ims.tar.zst --clobber

du -ah -d1 external_ims*.tar.zst | sort -h
