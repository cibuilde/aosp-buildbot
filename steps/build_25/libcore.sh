...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_25.ninja legacy-core-platform-api-stubs-system-modules,android_common
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_25/libcore/legacy-core-platform-api-stubs-system-modules^android_common.output . $GITHUB_WORKSPACE/artifacts/libcore/mmodules/core_platform_api/legacy-core-platform-api-stubs-system-modules^android_common

echo "building stable-core-platform-api-stubs-system-modules^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_25.ninja stable-core-platform-api-stubs-system-modules,android_common
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_25/libcore/stable-core-platform-api-stubs-system-modules^android_common.output . $GITHUB_WORKSPACE/artifacts/libcore/mmodules/core_platform_api/stable-core-platform-api-stubs-system-modules^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf libcore.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/libcore/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_25 libcore.tar.zst --clobber
