...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_25.ninja com.android.vndk.current,android_common_image
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/modules/vndk/apex/com.android.vndk.current^android_common_image
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_25/packages/modules/vndk/com.android.vndk.current^android_common_image.output . $GITHUB_WORKSPACE/artifacts/packages/modules/vndk/apex/com.android.vndk.current^android_common_image

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf packages_modules_vndk.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/packages/modules/vndk/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_25 packages_modules_vndk.tar.zst --clobber

du -ah -d1 packages_modules_vndk*.tar.zst | sort -h
