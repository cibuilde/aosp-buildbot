...
clone_depth_platform platform/packages/modules/CellBroadcastService
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_25.ninja CellBroadcastServiceModule,android_common_apex30
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_25/platform/packages/modules/CellBroadcastService/CellBroadcastServiceModule^android_common_apex30.output . $GITHUB_WORKSPACE/artifacts/packages/modules/CellBroadcastService/CellBroadcastServiceModule^android_common_apex30

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_modules_CellBroadcastService.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/modules/CellBroadcastService/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_25 platform_packages_modules_CellBroadcastService.tar.zst --clobber

du -ah -d1 platform_packages_modules_CellBroadcastService*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_modules_CellBroadcastService.tar.zst" ]; then
  echo "Compressing platform/packages/modules/CellBroadcastService -> platform_packages_modules_CellBroadcastService.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_modules_CellBroadcastService.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/modules/CellBroadcastService/ .
fi
