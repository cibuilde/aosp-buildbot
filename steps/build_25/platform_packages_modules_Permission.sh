...
clone_depth_platform platform/packages/modules/Permission
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_25.ninja service-permission-shared,android_common_apex30
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/modules/Permission/service/service-permission-shared^android_common_apex30
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_25/platform/packages/modules/Permission/service-permission-shared^android_common_apex30.output . $GITHUB_WORKSPACE/artifacts/packages/modules/Permission/service/service-permission-shared^android_common_apex30

echo "building service-permission-shared^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_25.ninja service-permission-shared,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/modules/Permission/service/service-permission-shared^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_25/platform/packages/modules/Permission/service-permission-shared^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/modules/Permission/service/service-permission-shared^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_modules_Permission.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/modules/Permission/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_25 platform_packages_modules_Permission.tar.zst --clobber

du -ah -d1 platform_packages_modules_Permission*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_modules_Permission.tar.zst" ]; then
  echo "Compressing platform/packages/modules/Permission -> platform_packages_modules_Permission.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_modules_Permission.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/modules/Permission/ .
fi
