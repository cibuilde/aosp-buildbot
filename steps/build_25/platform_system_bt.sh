...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_25.ninja bluetooth-protos-lite,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/system/bt/gd/proto/bluetooth-protos-lite^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_25/platform/system/bt/bluetooth-protos-lite^android_common.output . $GITHUB_WORKSPACE/artifacts/system/bt/gd/proto/bluetooth-protos-lite^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_system_bt.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/system/bt/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_25 platform_system_bt.tar.zst --clobber
