...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_27.ninja core-current-stubs-for-system-modules,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/libcore/core-current-stubs-for-system-modules^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_27/libcore/core-current-stubs-for-system-modules^android_common.output . $GITHUB_WORKSPACE/artifacts/libcore/core-current-stubs-for-system-modules^android_common

echo "building core-module-lib-stubs-system-modules^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_27.ninja core-module-lib-stubs-system-modules,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/libcore/core-module-lib-stubs-system-modules^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_27/libcore/core-module-lib-stubs-system-modules^android_common.output . $GITHUB_WORKSPACE/artifacts/libcore/core-module-lib-stubs-system-modules^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf libcore.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/libcore/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_27 libcore.tar.zst --clobber
