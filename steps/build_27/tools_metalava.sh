...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_27.ninja private-stub-annotations-jar,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/tools/metalava/private-stub-annotations-jar^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_27/tools/metalava/private-stub-annotations-jar^android_common.output . $GITHUB_WORKSPACE/artifacts/tools/metalava/private-stub-annotations-jar^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf tools_metalava.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/tools/metalava/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_27 tools_metalava.tar.zst --clobber
