...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_29.ninja bouncycastle_ike_digests,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/bouncycastle/bouncycastle_ike_digests^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_29/external/bouncycastle/bouncycastle_ike_digests^android_common.output . $GITHUB_WORKSPACE/artifacts/external/bouncycastle/bouncycastle_ike_digests^android_common

echo "building bouncycastle_ike_digests^android_common_apex30"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_29.ninja bouncycastle_ike_digests,android_common_apex30
mkdir -p $GITHUB_WORKSPACE/artifacts/external/bouncycastle/bouncycastle_ike_digests^android_common_apex30
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_29/external/bouncycastle/bouncycastle_ike_digests^android_common_apex30.output . $GITHUB_WORKSPACE/artifacts/external/bouncycastle/bouncycastle_ike_digests^android_common_apex30

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_bouncycastle.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/bouncycastle/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_29 external_bouncycastle.tar.zst --clobber
