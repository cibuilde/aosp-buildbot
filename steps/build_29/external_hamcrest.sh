...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_29.ninja hamcrest,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/hamcrest/hamcrest-core/hamcrest^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_29/external/hamcrest/hamcrest^android_common.output . $GITHUB_WORKSPACE/artifacts/external/hamcrest/hamcrest-core/hamcrest^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_hamcrest.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/hamcrest/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_29 external_hamcrest.tar.zst --clobber
