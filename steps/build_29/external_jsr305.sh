...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_29.ninja jsr305,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/jsr305/jsr305^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_29/external/jsr305/jsr305^android_common.output . $GITHUB_WORKSPACE/artifacts/external/jsr305/jsr305^android_common

echo "building jsr305^android_common_apex29"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_29.ninja jsr305,android_common_apex29
mkdir -p $GITHUB_WORKSPACE/artifacts/external/jsr305/jsr305^android_common_apex29
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_29/external/jsr305/jsr305^android_common_apex29.output . $GITHUB_WORKSPACE/artifacts/external/jsr305/jsr305^android_common_apex29

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_jsr305.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/jsr305/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_29 external_jsr305.tar.zst --clobber
