...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_29.ninja framework-annotations-lib,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/frameworks/libs/modules-utils/java/framework-annotations-lib^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_29/frameworks/libs/modules-utils/framework-annotations-lib^android_common.output . $GITHUB_WORKSPACE/artifacts/frameworks/libs/modules-utils/java/framework-annotations-lib^android_common
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_29 frameworks_libs_modules-utils.tar.zst --clobber
