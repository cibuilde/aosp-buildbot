...
rsync -a -r $GITHUB_WORKSPACE/downloads/packages/modules/StatsD/framework/statslog-statsd-java-gen^/ .
...
echo "building framework-statsd^android_common_apex30"
prebuilts/build-tools/linux-x86/bin/ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_29.ninja framework-statsd,android_common_apex30
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/modules/StatsD/framework/framework-statsd^android_common_apex30
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_29/packages/modules/StatsD/framework-statsd^android_common_apex30.output . $GITHUB_WORKSPACE/artifacts/packages/modules/StatsD/framework/framework-statsd^android_common_apex30
python3 $GITHUB_WORKSPACE/copy_symlink.py $GITHUB_WORKSPACE/steps/outputs_29/packages/modules/StatsD/framework-statsd^android_common_apex30.output $GITHUB_WORKSPACE/artifacts/packages/modules/StatsD/framework/framework-statsd^android_common_apex30 $GITHUB_WORKSPACE/artifacts/packages/modules/StatsD/framework/framework-statsd^android_common_apex30/addition_copy_files.output

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf packages_modules_StatsD.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/packages/modules/StatsD/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_29 packages_modules_StatsD.tar.zst --clobber
