...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_29.ninja sysprop-library-stub-platform,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/system/tools/sysprop/sysprop-library-stub-platform^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_29/system/tools/sysprop/sysprop-library-stub-platform^android_common.output . $GITHUB_WORKSPACE/artifacts/system/tools/sysprop/sysprop-library-stub-platform^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_tools_sysprop.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/tools/sysprop/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_29 system_tools_sysprop.tar.zst --clobber
