...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_29.ninja app-compat-annotations,android_common
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_29/tools/platform-compat/app-compat-annotations^android_common.output . $GITHUB_WORKSPACE/artifacts/tools/platform-compat/java/android/compat/annotation/app-compat-annotations^android_common

echo "building unsupportedappusage^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_29.ninja unsupportedappusage,android_common
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_29/tools/platform-compat/unsupportedappusage^android_common.output . $GITHUB_WORKSPACE/artifacts/tools/platform-compat/java/android/compat/annotation/unsupportedappusage^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf tools_platform-compat.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/tools/platform-compat/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_29 tools_platform-compat.tar.zst --clobber
