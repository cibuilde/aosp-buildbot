...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_30.ninja dagger2,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/dagger2/dagger2^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_30/external/dagger2/dagger2^android_common.output . $GITHUB_WORKSPACE/artifacts/external/dagger2/dagger2^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_dagger2.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/dagger2/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_30 external_dagger2.tar.zst --clobber
