...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_30.ninja junit,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/junit/junit^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_30/external/junit/junit^android_common.output . $GITHUB_WORKSPACE/artifacts/external/junit/junit^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_junit.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/junit/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_30 external_junit.tar.zst --clobber
