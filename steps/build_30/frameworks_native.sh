...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_30.ninja SurfaceFlingerProperties,android_common
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_30/frameworks/native/SurfaceFlingerProperties^android_common.output . $GITHUB_WORKSPACE/artifacts/frameworks/native/services/surfaceflinger/sysprop/SurfaceFlingerProperties^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf frameworks_native.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/frameworks/native/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_30 frameworks_native.tar.zst --clobber
