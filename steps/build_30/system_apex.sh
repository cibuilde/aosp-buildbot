...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_30.ninja com.android.sysprop.apex,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/system/apex/apexd/sysprop/com.android.sysprop.apex^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_30/system/apex/com.android.sysprop.apex^android_common.output . $GITHUB_WORKSPACE/artifacts/system/apex/apexd/sysprop/com.android.sysprop.apex^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_apex.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/apex/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_30 system_apex.tar.zst --clobber
