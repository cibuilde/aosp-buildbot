...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_30.ninja PlatformProperties,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/system/libsysprop/srcs/PlatformProperties^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_30/system/libsysprop/PlatformProperties^android_common.output . $GITHUB_WORKSPACE/artifacts/system/libsysprop/srcs/PlatformProperties^android_common

echo "building PlatformProperties_public^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_30.ninja PlatformProperties_public,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/system/libsysprop/srcs/PlatformProperties_public^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_30/system/libsysprop/PlatformProperties_public^android_common.output . $GITHUB_WORKSPACE/artifacts/system/libsysprop/srcs/PlatformProperties_public^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf system_libsysprop.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/system/libsysprop/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_30 system_libsysprop.tar.zst --clobber
