...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_31.ninja icing-java-proto-lite,android_common_apex10000
mkdir -p $GITHUB_WORKSPACE/artifacts/external/icing/proto/icing-java-proto-lite^android_common_apex10000
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_31/external/icing/icing-java-proto-lite^android_common_apex10000.output . $GITHUB_WORKSPACE/artifacts/external/icing/proto/icing-java-proto-lite^android_common_apex10000

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf external_icing.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/external/icing/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_31 external_icing.tar.zst --clobber
