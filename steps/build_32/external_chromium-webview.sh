...
echo "Preparing for external/chromium-webview"
...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja webview,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/chromium-webview/webview^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/external/chromium-webview/webview^android_common.output . $GITHUB_WORKSPACE/artifacts/external/chromium-webview/webview^android_common
python3 $GITHUB_WORKSPACE/copy_symlink.py $GITHUB_WORKSPACE/steps/outputs_32/external/chromium-webview/webview^android_common.output $GITHUB_WORKSPACE/artifacts/external/chromium-webview/webview^android_common $GITHUB_WORKSPACE/artifacts/external/chromium-webview/webview^android_common/addition_copy_files.output
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 external_chromium-webview.tar.zst --clobber

du -ah -d1 external_chromium-webview*.tar.zst | sort -h
