...
echo "Preparing for packages/apps/Browser2"
...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja Browser2,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/apps/Browser2/Browser2^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/packages/apps/Browser2/Browser2^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/apps/Browser2/Browser2^android_common
python3 $GITHUB_WORKSPACE/copy_symlink.py $GITHUB_WORKSPACE/steps/outputs_32/packages/apps/Browser2/Browser2^android_common.output $GITHUB_WORKSPACE/artifacts/packages/apps/Browser2/Browser2^android_common $GITHUB_WORKSPACE/artifacts/packages/apps/Browser2/Browser2^android_common/addition_copy_files.output
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 packages_apps_Browser2.tar.zst --clobber

du -ah -d1 packages_apps_Browser2*.tar.zst | sort -h
