...
echo "Preparing for packages/apps/HTMLViewer"
...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja HTMLViewer,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/apps/HTMLViewer/HTMLViewer^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/packages/apps/HTMLViewer/HTMLViewer^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/apps/HTMLViewer/HTMLViewer^android_common
python3 $GITHUB_WORKSPACE/copy_symlink.py $GITHUB_WORKSPACE/steps/outputs_32/packages/apps/HTMLViewer/HTMLViewer^android_common.output $GITHUB_WORKSPACE/artifacts/packages/apps/HTMLViewer/HTMLViewer^android_common $GITHUB_WORKSPACE/artifacts/packages/apps/HTMLViewer/HTMLViewer^android_common/addition_copy_files.output
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 packages_apps_HTMLViewer.tar.zst --clobber

du -ah -d1 packages_apps_HTMLViewer*.tar.zst | sort -h
