...
echo "Preparing for packages/apps/Music"
...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja Music,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/apps/Music/Music^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/packages/apps/Music/Music^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/apps/Music/Music^android_common
python3 $GITHUB_WORKSPACE/copy_symlink.py $GITHUB_WORKSPACE/steps/outputs_32/packages/apps/Music/Music^android_common.output $GITHUB_WORKSPACE/artifacts/packages/apps/Music/Music^android_common $GITHUB_WORKSPACE/artifacts/packages/apps/Music/Music^android_common/addition_copy_files.output
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 packages_apps_Music.tar.zst --clobber

du -ah -d1 packages_apps_Music*.tar.zst | sort -h
