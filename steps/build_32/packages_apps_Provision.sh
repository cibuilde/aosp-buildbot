...
echo "Preparing for packages/apps/Provision"
...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja Provision,android_common
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 packages_apps_Provision.tar.zst --clobber

du -ah -d1 packages_apps_Provision*.tar.zst | sort -h
