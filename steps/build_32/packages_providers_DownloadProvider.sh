...
echo "Preparing for packages/providers/DownloadProvider"
...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja DownloadProvider,android_common
...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja DownloadProviderUi,android_common
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 packages_providers_DownloadProvider.tar.zst --clobber

du -ah -d1 packages_providers_DownloadProvider*.tar.zst | sort -h
