...
echo "Preparing for packages/providers/TelephonyProvider"
...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja TelephonyProvider,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/providers/TelephonyProvider/TelephonyProvider^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/packages/providers/TelephonyProvider/TelephonyProvider^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/providers/TelephonyProvider/TelephonyProvider^android_common
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 packages_providers_TelephonyProvider.tar.zst --clobber

du -ah -d1 packages_providers_TelephonyProvider*.tar.zst | sort -h
