...
echo "Preparing for packages/screensavers/PhotoTable"
...
prebuilts/build-tools/linux-x86/bin/ninja -j $(nproc) -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja PhotoTable,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/screensavers/PhotoTable/PhotoTable^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/packages/screensavers/PhotoTable/PhotoTable^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/screensavers/PhotoTable/PhotoTable^android_common
python3 $GITHUB_WORKSPACE/copy_symlink.py $GITHUB_WORKSPACE/steps/outputs_32/packages/screensavers/PhotoTable/PhotoTable^android_common.output $GITHUB_WORKSPACE/artifacts/packages/screensavers/PhotoTable/PhotoTable^android_common $GITHUB_WORKSPACE/artifacts/packages/screensavers/PhotoTable/PhotoTable^android_common/addition_copy_files.output
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 packages_screensavers_PhotoTable.tar.zst --clobber

du -ah -d1 packages_screensavers_PhotoTable*.tar.zst | sort -h
