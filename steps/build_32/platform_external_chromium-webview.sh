...
clone_depth_platform platform/external/chromium-webview
...
echo "building webview^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja webview,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/external/chromium-webview/webview^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/platform/external/chromium-webview/webview^android_common.output . $GITHUB_WORKSPACE/artifacts/external/chromium-webview/webview^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_external_chromium-webview.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/external/chromium-webview/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 platform_external_chromium-webview.tar.zst --clobber

du -ah -d1 platform_external_chromium-webview*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_external_chromium-webview.tar.zst" ]; then
  echo "Compressing platform/external/chromium-webview -> platform_external_chromium-webview.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_external_chromium-webview.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/external/chromium-webview/ .
fi
