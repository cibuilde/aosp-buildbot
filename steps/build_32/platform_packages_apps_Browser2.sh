...
clone_depth_platform platform/packages/apps/Browser2
...
rsync -a -r $GITHUB_WORKSPACE/downloads/frameworks/base/test-mock/android.test.mock^android_common/ .
rsync -a -r $GITHUB_WORKSPACE/downloads/frameworks/base/test-runner/android.test.runner^android_common/ .
...
echo "building Browser2^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja Browser2,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/apps/Browser2/Browser2^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/platform/packages/apps/Browser2/Browser2^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/apps/Browser2/Browser2^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_apps_Browser2.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/apps/Browser2/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 platform_packages_apps_Browser2.tar.zst --clobber

du -ah -d1 platform_packages_apps_Browser2*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_apps_Browser2.tar.zst" ]; then
  echo "Compressing platform/packages/apps/Browser2 -> platform_packages_apps_Browser2.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_apps_Browser2.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/apps/Browser2/ .
fi
