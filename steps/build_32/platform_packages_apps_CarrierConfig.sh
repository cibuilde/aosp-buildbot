...
clone_depth_platform platform/packages/apps/CarrierConfig
...
echo "building CarrierConfig^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja CarrierConfig,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/apps/CarrierConfig/CarrierConfig^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/platform/packages/apps/CarrierConfig/CarrierConfig^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/apps/CarrierConfig/CarrierConfig^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_apps_CarrierConfig.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/apps/CarrierConfig/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 platform_packages_apps_CarrierConfig.tar.zst --clobber

du -ah -d1 platform_packages_apps_CarrierConfig*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_apps_CarrierConfig.tar.zst" ]; then
  echo "Compressing platform/packages/apps/CarrierConfig -> platform_packages_apps_CarrierConfig.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_apps_CarrierConfig.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/apps/CarrierConfig/ .
fi
