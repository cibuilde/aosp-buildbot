...
clone_depth_platform packages/apps/CertInstaller
clone_depth_platform platform/packages/apps/CertInstaller
...
rsync -a -r $GITHUB_WORKSPACE/downloads/external/bouncycastle/bouncycastle-unbundled^android_common/ .
...
echo "building CertInstaller^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja CertInstaller,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/apps/CertInstaller/CertInstaller^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/platform/packages/apps/CertInstaller/CertInstaller^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/apps/CertInstaller/CertInstaller^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_apps_CertInstaller.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/apps/CertInstaller/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 platform_packages_apps_CertInstaller.tar.zst --clobber

du -ah -d1 platform_packages_apps_CertInstaller*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/packages_apps_CertInstaller.tar.zst" ]; then
  echo "Compressing packages/apps/CertInstaller -> packages_apps_CertInstaller.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/packages_apps_CertInstaller.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/packages/apps/CertInstaller/ .
fi
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_apps_CertInstaller.tar.zst" ]; then
  echo "Compressing platform/packages/apps/CertInstaller -> platform_packages_apps_CertInstaller.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_apps_CertInstaller.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/apps/CertInstaller/ .
fi
