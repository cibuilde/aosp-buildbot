...
clone_depth_platform platform/packages/apps/EmergencyInfo
...
rsync -a -r $GITHUB_WORKSPACE/downloads/packages/apps/EmergencyInfo/EmergencyGestureAction/EmergencyGestureAction^android_common/ .
rsync -a -r $GITHUB_WORKSPACE/downloads/packages/apps/EmergencyInfo/EmergencyGestureContentProvider/EmergencyGestureContentProvider^android_common/ .
rsync -a -r $GITHUB_WORKSPACE/downloads/packages/apps/EmergencyInfo/EmergencyInfo-lib^android_common/ .
...
rsync -a -r $GITHUB_WORKSPACE/downloads/prebuilts/sdk/current/androidx-legacy/androidx.legacy_legacy-support-v13^android_common/ .
...
echo "building EmergencyInfo^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja EmergencyInfo,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/apps/EmergencyInfo/EmergencyInfo^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/platform/packages/apps/EmergencyInfo/EmergencyInfo^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/apps/EmergencyInfo/EmergencyInfo^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_apps_EmergencyInfo.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/apps/EmergencyInfo/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 platform_packages_apps_EmergencyInfo.tar.zst --clobber

du -ah -d1 platform_packages_apps_EmergencyInfo*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_apps_EmergencyInfo.tar.zst" ]; then
  echo "Compressing platform/packages/apps/EmergencyInfo -> platform_packages_apps_EmergencyInfo.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_apps_EmergencyInfo.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/apps/EmergencyInfo/ .
fi
