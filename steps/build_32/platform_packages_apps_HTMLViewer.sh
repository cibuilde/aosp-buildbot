...
clone_depth_platform packages/apps/HTMLViewer
clone_depth_platform platform/packages/apps/HTMLViewer
...
echo "building HTMLViewer^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja HTMLViewer,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/apps/HTMLViewer/HTMLViewer^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/platform/packages/apps/HTMLViewer/HTMLViewer^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/apps/HTMLViewer/HTMLViewer^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_apps_HTMLViewer.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/apps/HTMLViewer/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 platform_packages_apps_HTMLViewer.tar.zst --clobber

du -ah -d1 platform_packages_apps_HTMLViewer*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/packages_apps_HTMLViewer.tar.zst" ]; then
  echo "Compressing packages/apps/HTMLViewer -> packages_apps_HTMLViewer.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/packages_apps_HTMLViewer.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/packages/apps/HTMLViewer/ .
fi
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_apps_HTMLViewer.tar.zst" ]; then
  echo "Compressing platform/packages/apps/HTMLViewer -> platform_packages_apps_HTMLViewer.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_apps_HTMLViewer.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/apps/HTMLViewer/ .
fi
