...
clone_depth_platform packages/apps/MusicFX
clone_depth_platform platform/packages/apps/MusicFX
...
echo "building MusicFX^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja MusicFX,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/apps/MusicFX/MusicFX^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/platform/packages/apps/MusicFX/MusicFX^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/apps/MusicFX/MusicFX^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_apps_MusicFX.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/apps/MusicFX/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 platform_packages_apps_MusicFX.tar.zst --clobber

du -ah -d1 platform_packages_apps_MusicFX*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/packages_apps_MusicFX.tar.zst" ]; then
  echo "Compressing packages/apps/MusicFX -> packages_apps_MusicFX.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/packages_apps_MusicFX.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/packages/apps/MusicFX/ .
fi
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_apps_MusicFX.tar.zst" ]; then
  echo "Compressing platform/packages/apps/MusicFX -> platform_packages_apps_MusicFX.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_apps_MusicFX.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/apps/MusicFX/ .
fi
