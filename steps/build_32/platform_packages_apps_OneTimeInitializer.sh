...
clone_depth_platform platform/packages/apps/OneTimeInitializer
...
echo "building OneTimeInitializer^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja OneTimeInitializer,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/apps/OneTimeInitializer/OneTimeInitializer^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/platform/packages/apps/OneTimeInitializer/OneTimeInitializer^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/apps/OneTimeInitializer/OneTimeInitializer^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_apps_OneTimeInitializer.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/apps/OneTimeInitializer/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 platform_packages_apps_OneTimeInitializer.tar.zst --clobber

du -ah -d1 platform_packages_apps_OneTimeInitializer*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_apps_OneTimeInitializer.tar.zst" ]; then
  echo "Compressing platform/packages/apps/OneTimeInitializer -> platform_packages_apps_OneTimeInitializer.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_apps_OneTimeInitializer.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/apps/OneTimeInitializer/ .
fi
