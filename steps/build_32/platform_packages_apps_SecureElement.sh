...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja SecureElement,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/apps/SecureElement/SecureElement^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/platform/packages/apps/SecureElement/SecureElement^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/apps/SecureElement/SecureElement^android_common
...
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 platform_packages_apps_SecureElement.tar.zst --clobber
