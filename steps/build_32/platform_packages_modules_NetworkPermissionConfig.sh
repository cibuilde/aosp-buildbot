...
clone_depth_platform platform/packages/modules/NetworkPermissionConfig
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja NetworkPermissionConfig,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/modules/NetworkPermissionConfig/NetworkPermissionConfig^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/platform/packages/modules/NetworkPermissionConfig/NetworkPermissionConfig^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/modules/NetworkPermissionConfig/NetworkPermissionConfig^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_modules_NetworkPermissionConfig.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/modules/NetworkPermissionConfig/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 platform_packages_modules_NetworkPermissionConfig.tar.zst --clobber

du -ah -d1 platform_packages_modules_NetworkPermissionConfig*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_modules_NetworkPermissionConfig.tar.zst" ]; then
  echo "Compressing platform/packages/modules/NetworkPermissionConfig -> platform_packages_modules_NetworkPermissionConfig.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_modules_NetworkPermissionConfig.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/modules/NetworkPermissionConfig/ .
fi
