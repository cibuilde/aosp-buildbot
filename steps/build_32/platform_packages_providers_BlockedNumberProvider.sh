...
clone_depth_platform packages/providers/BlockedNumberProvider
clone_depth_platform platform/packages/providers/BlockedNumberProvider
...
echo "building BlockedNumberProvider^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja BlockedNumberProvider,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/providers/BlockedNumberProvider/BlockedNumberProvider^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/platform/packages/providers/BlockedNumberProvider/BlockedNumberProvider^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/providers/BlockedNumberProvider/BlockedNumberProvider^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_providers_BlockedNumberProvider.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/providers/BlockedNumberProvider/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 platform_packages_providers_BlockedNumberProvider.tar.zst --clobber

du -ah -d1 platform_packages_providers_BlockedNumberProvider*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/packages_providers_BlockedNumberProvider.tar.zst" ]; then
  echo "Compressing packages/providers/BlockedNumberProvider -> packages_providers_BlockedNumberProvider.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/packages_providers_BlockedNumberProvider.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/packages/providers/BlockedNumberProvider/ .
fi
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_providers_BlockedNumberProvider.tar.zst" ]; then
  echo "Compressing platform/packages/providers/BlockedNumberProvider -> platform_packages_providers_BlockedNumberProvider.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_providers_BlockedNumberProvider.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/providers/BlockedNumberProvider/ .
fi
