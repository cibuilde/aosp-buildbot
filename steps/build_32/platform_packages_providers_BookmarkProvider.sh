...
clone_depth_platform platform/packages/providers/BookmarkProvider
...
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/platform/packages/providers/BookmarkProvider/BookmarkProvider^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/providers/BookmarkProvider/BookmarkProvider^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_providers_BookmarkProvider.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/providers/BookmarkProvider/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 platform_packages_providers_BookmarkProvider.tar.zst --clobber

du -ah -d1 platform_packages_providers_BookmarkProvider*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_providers_BookmarkProvider.tar.zst" ]; then
  echo "Compressing platform/packages/providers/BookmarkProvider -> platform_packages_providers_BookmarkProvider.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_providers_BookmarkProvider.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/providers/BookmarkProvider/ .
fi
