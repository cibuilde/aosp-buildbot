...
clone_depth_platform packages/providers/CallLogProvider
clone_depth_platform platform/packages/providers/CallLogProvider
...
echo "building CallLogBackup^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja CallLogBackup,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/providers/CallLogProvider/CallLogBackup^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/platform/packages/providers/CallLogProvider/CallLogBackup^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/providers/CallLogProvider/CallLogBackup^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_providers_CallLogProvider.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/providers/CallLogProvider/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 platform_packages_providers_CallLogProvider.tar.zst --clobber

du -ah -d1 platform_packages_providers_CallLogProvider*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/packages_providers_CallLogProvider.tar.zst" ]; then
  echo "Compressing packages/providers/CallLogProvider -> packages_providers_CallLogProvider.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/packages_providers_CallLogProvider.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/packages/providers/CallLogProvider/ .
fi
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_providers_CallLogProvider.tar.zst" ]; then
  echo "Compressing platform/packages/providers/CallLogProvider -> platform_packages_providers_CallLogProvider.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_providers_CallLogProvider.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/providers/CallLogProvider/ .
fi
