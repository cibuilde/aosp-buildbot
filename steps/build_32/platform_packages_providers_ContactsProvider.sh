...
clone_depth_platform frameworks/opt/vcard
clone_depth_platform libcore
clone_depth_platform packages/providers/ContactsProvider
clone_depth_platform platform/packages/providers/ContactsProvider
...
rsync -a -r $GITHUB_WORKSPACE/downloads/frameworks/opt/vcard/com.android.vcard^android_common/ .
...
echo "building ContactsProvider^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja ContactsProvider,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/providers/ContactsProvider/ContactsProvider^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/platform/packages/providers/ContactsProvider/ContactsProvider^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/providers/ContactsProvider/ContactsProvider^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_providers_ContactsProvider.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/providers/ContactsProvider/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 platform_packages_providers_ContactsProvider.tar.zst --clobber

du -ah -d1 platform_packages_providers_ContactsProvider*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/frameworks_opt_vcard.tar.zst" ]; then
  echo "Compressing frameworks/opt/vcard -> frameworks_opt_vcard.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/frameworks_opt_vcard.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/frameworks/opt/vcard/ .
fi
if [ ! -f "$GITHUB_WORKSPACE/cache/libcore.tar.zst" ]; then
  echo "Compressing libcore -> libcore.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/libcore.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/libcore/ .
fi
if [ ! -f "$GITHUB_WORKSPACE/cache/packages_providers_ContactsProvider.tar.zst" ]; then
  echo "Compressing packages/providers/ContactsProvider -> packages_providers_ContactsProvider.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/packages_providers_ContactsProvider.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/packages/providers/ContactsProvider/ .
fi
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_providers_ContactsProvider.tar.zst" ]; then
  echo "Compressing platform/packages/providers/ContactsProvider -> platform_packages_providers_ContactsProvider.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_providers_ContactsProvider.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/providers/ContactsProvider/ .
fi
