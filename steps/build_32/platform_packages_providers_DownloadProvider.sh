...
clone_depth_platform packages/providers/DownloadProvider
clone_depth_platform platform/packages/providers/DownloadProvider
...
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja DownloadProvider,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/providers/DownloadProvider/DownloadProvider^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/platform/packages/providers/DownloadProvider/DownloadProvider^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/providers/DownloadProvider/DownloadProvider^android_common

echo "building DownloadProviderUi^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja DownloadProviderUi,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/providers/DownloadProvider/ui/DownloadProviderUi^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/platform/packages/providers/DownloadProvider/DownloadProviderUi^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/providers/DownloadProvider/ui/DownloadProviderUi^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_providers_DownloadProvider.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/providers/DownloadProvider/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 platform_packages_providers_DownloadProvider.tar.zst --clobber

du -ah -d1 platform_packages_providers_DownloadProvider*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/packages_providers_DownloadProvider.tar.zst" ]; then
  echo "Compressing packages/providers/DownloadProvider -> packages_providers_DownloadProvider.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/packages_providers_DownloadProvider.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/packages/providers/DownloadProvider/ .
fi
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_providers_DownloadProvider.tar.zst" ]; then
  echo "Compressing platform/packages/providers/DownloadProvider -> platform_packages_providers_DownloadProvider.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_providers_DownloadProvider.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/providers/DownloadProvider/ .
fi
