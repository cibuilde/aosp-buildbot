...
clone_depth_platform packages/providers/TelephonyProvider
clone_depth_platform platform/packages/providers/TelephonyProvider
...
rsync -a -r $GITHUB_WORKSPACE/downloads/packages/providers/TelephonyProvider/proto/telephonyprovider-protos^android_common/ .
...
echo "building TelephonyProvider^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja TelephonyProvider,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/providers/TelephonyProvider/TelephonyProvider^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/platform/packages/providers/TelephonyProvider/TelephonyProvider^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/providers/TelephonyProvider/TelephonyProvider^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_providers_TelephonyProvider.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/providers/TelephonyProvider/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 platform_packages_providers_TelephonyProvider.tar.zst --clobber

du -ah -d1 platform_packages_providers_TelephonyProvider*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/packages_providers_TelephonyProvider.tar.zst" ]; then
  echo "Compressing packages/providers/TelephonyProvider -> packages_providers_TelephonyProvider.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/packages_providers_TelephonyProvider.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/packages/providers/TelephonyProvider/ .
fi
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_providers_TelephonyProvider.tar.zst" ]; then
  echo "Compressing platform/packages/providers/TelephonyProvider -> platform_packages_providers_TelephonyProvider.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_providers_TelephonyProvider.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/providers/TelephonyProvider/ .
fi
