...
clone_depth_platform packages/providers/UserDictionaryProvider
clone_depth_platform platform/packages/providers/UserDictionaryProvider
...
echo "building UserDictionaryProvider^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja UserDictionaryProvider,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/providers/UserDictionaryProvider/UserDictionaryProvider^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/platform/packages/providers/UserDictionaryProvider/UserDictionaryProvider^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/providers/UserDictionaryProvider/UserDictionaryProvider^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_providers_UserDictionaryProvider.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/providers/UserDictionaryProvider/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 platform_packages_providers_UserDictionaryProvider.tar.zst --clobber

du -ah -d1 platform_packages_providers_UserDictionaryProvider*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/packages_providers_UserDictionaryProvider.tar.zst" ]; then
  echo "Compressing packages/providers/UserDictionaryProvider -> packages_providers_UserDictionaryProvider.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/packages_providers_UserDictionaryProvider.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/packages/providers/UserDictionaryProvider/ .
fi
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_providers_UserDictionaryProvider.tar.zst" ]; then
  echo "Compressing platform/packages/providers/UserDictionaryProvider -> platform_packages_providers_UserDictionaryProvider.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_providers_UserDictionaryProvider.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/providers/UserDictionaryProvider/ .
fi
