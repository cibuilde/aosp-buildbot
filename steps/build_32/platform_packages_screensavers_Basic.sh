...
clone_depth_platform packages/screensavers/Basic
clone_depth_platform platform/packages/screensavers/Basic
...
echo "building BasicDreams^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja BasicDreams,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/screensavers/Basic/BasicDreams^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/platform/packages/screensavers/Basic/BasicDreams^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/screensavers/Basic/BasicDreams^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_screensavers_Basic.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/screensavers/Basic/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 platform_packages_screensavers_Basic.tar.zst --clobber

du -ah -d1 platform_packages_screensavers_Basic*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/packages_screensavers_Basic.tar.zst" ]; then
  echo "Compressing packages/screensavers/Basic -> packages_screensavers_Basic.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/packages_screensavers_Basic.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/packages/screensavers/Basic/ .
fi
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_screensavers_Basic.tar.zst" ]; then
  echo "Compressing platform/packages/screensavers/Basic -> platform_packages_screensavers_Basic.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_screensavers_Basic.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/screensavers/Basic/ .
fi
