...
clone_depth_platform platform/packages/screensavers/PhotoTable
...
echo "building PhotoTable^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja PhotoTable,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/screensavers/PhotoTable/PhotoTable^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/platform/packages/screensavers/PhotoTable/PhotoTable^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/screensavers/PhotoTable/PhotoTable^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_screensavers_PhotoTable.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/screensavers/PhotoTable/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 platform_packages_screensavers_PhotoTable.tar.zst --clobber

du -ah -d1 platform_packages_screensavers_PhotoTable*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_screensavers_PhotoTable.tar.zst" ]; then
  echo "Compressing platform/packages/screensavers/PhotoTable -> platform_packages_screensavers_PhotoTable.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_screensavers_PhotoTable.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/screensavers/PhotoTable/ .
fi
