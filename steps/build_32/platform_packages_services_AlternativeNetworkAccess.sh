...
clone_depth_platform packages/services/AlternativeNetworkAccess
clone_depth_platform platform/packages/services/AlternativeNetworkAccess
...
echo "building ONS^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja ONS,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/services/AlternativeNetworkAccess/ONS^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/platform/packages/services/AlternativeNetworkAccess/ONS^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/services/AlternativeNetworkAccess/ONS^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_services_AlternativeNetworkAccess.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/services/AlternativeNetworkAccess/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 platform_packages_services_AlternativeNetworkAccess.tar.zst --clobber

du -ah -d1 platform_packages_services_AlternativeNetworkAccess*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/packages_services_AlternativeNetworkAccess.tar.zst" ]; then
  echo "Compressing packages/services/AlternativeNetworkAccess -> packages_services_AlternativeNetworkAccess.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/packages_services_AlternativeNetworkAccess.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/packages/services/AlternativeNetworkAccess/ .
fi
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_services_AlternativeNetworkAccess.tar.zst" ]; then
  echo "Compressing platform/packages/services/AlternativeNetworkAccess -> platform_packages_services_AlternativeNetworkAccess.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_services_AlternativeNetworkAccess.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/services/AlternativeNetworkAccess/ .
fi
