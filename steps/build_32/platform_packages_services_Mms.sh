...
clone_depth_platform packages/services/Mms
clone_depth_platform platform/packages/services/Mms
...
echo "building MmsService^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_32.ninja MmsService,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/services/Mms/MmsService^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_32/platform/packages/services/Mms/MmsService^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/services/Mms/MmsService^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_services_Mms.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/services/Mms/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_32 platform_packages_services_Mms.tar.zst --clobber

du -ah -d1 platform_packages_services_Mms*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/packages_services_Mms.tar.zst" ]; then
  echo "Compressing packages/services/Mms -> packages_services_Mms.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/packages_services_Mms.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/packages/services/Mms/ .
fi
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_services_Mms.tar.zst" ]; then
  echo "Compressing platform/packages/services/Mms -> platform_packages_services_Mms.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_services_Mms.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/services/Mms/ .
fi
