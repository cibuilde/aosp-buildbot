...
clone_depth_platform external/cbor-java
...
clone_depth_platform platform/packages/apps/RemoteProvisioner
...
rsync -a -r $GITHUB_WORKSPACE/downloads/external/cbor-java/cbor-java^android_common/ .
...
rsync -a -r $GITHUB_WORKSPACE/downloads/system/hardware/interfaces/keystore2/aidl/android.system.keystore2-V1-java^android_common/ .
...
rsync -a -r $GITHUB_WORKSPACE/downloads/system/security/keystore2/aidl/android.security.remoteprovisioning-java^android_common/ .

echo "building RemoteProvisioner^android_common"
ninja -d keepdepfile -f $GITHUB_WORKSPACE/steps/build_33.ninja RemoteProvisioner,android_common
mkdir -p $GITHUB_WORKSPACE/artifacts/packages/apps/RemoteProvisioner/RemoteProvisioner^android_common
rsync -a -r --files-from=$GITHUB_WORKSPACE/steps/outputs_33/platform/packages/apps/RemoteProvisioner/RemoteProvisioner^android_common.output . $GITHUB_WORKSPACE/artifacts/packages/apps/RemoteProvisioner/RemoteProvisioner^android_common

rm -rf out

cd $GITHUB_WORKSPACE/
tar -cf platform_packages_apps_RemoteProvisioner.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/artifacts/platform/packages/apps/RemoteProvisioner/ .
gh release --repo cibuilde/aosp-buildbot upload android12-gsi_33 platform_packages_apps_RemoteProvisioner.tar.zst --clobber

du -ah -d1 platform_packages_apps_RemoteProvisioner*.tar.zst | sort -h
...
if [ ! -f "$GITHUB_WORKSPACE/cache/external_cbor-java.tar.zst" ]; then
  echo "Compressing external/cbor-java -> external_cbor-java.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/external_cbor-java.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/external/cbor-java/ .
fi
...
if [ ! -f "$GITHUB_WORKSPACE/cache/platform_packages_apps_RemoteProvisioner.tar.zst" ]; then
  echo "Compressing platform/packages/apps/RemoteProvisioner -> platform_packages_apps_RemoteProvisioner.tar.zst"
  tar -cf $GITHUB_WORKSPACE/cache/platform_packages_apps_RemoteProvisioner.tar.zst --use-compress-program zstdmt -C $GITHUB_WORKSPACE/aosp/platform/packages/apps/RemoteProvisioner/ .
fi
